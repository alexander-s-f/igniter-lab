...
| **agent coordination foundation** | ✅ (P2) | `coordination::{CoordinationHub, AgentIdentity/AgentKind/AgentStatus, CapsulePool/PoolVisibility, PoolRight, CapsuleRef, PoolGrant, PoolRefusal}` — coordination = **Capability IO applied to a new domain**: one `guard()` boundary = P5 `verify_passport` (WHO + op-class scope) → pool ACL (`owner ‖ developer ‖ explicit PoolGrant`, WHAT-on-WHICH) → `AuditEvent` fact (allowed AND denied) in `__coord_audit__`. Ops: register/create_pool/add_capsule/list_capsules/check_right/grant/transfer_ownership. **CapsuleRef content-addressed** (dedup by blake3 digest). Developer = local root-of-trust (privileged but audited). Schema keeps production-mode reachable (visibility `Production`, transferable ownership, `RuntimeActor`/`vendor:*` actor) but does NOT serve. NO messenger/transfer/federation. VM untouched. (LAB-MACHINE-AGENT-POOLS-P2) |
