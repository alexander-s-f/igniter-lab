...
| **machine → frame projection** | ✅ (FP-P1, headless) | `frame::{project_world, Frame, Camera, write_frame_receipt, RenderHost, SvgRenderHost, JsonRenderHost}` — the INVERSE of wire-to-effect (`fact-to-frame`): the machine produces an observable representation. World = facts in `__world__` (latest-per-key); `project_world` deterministically projects to integer screen coords (fixed perspective). `Frame` carries `world_digest` + `source_receipt_id` lineage; `render_digest()` is host-agnostic. **Frame = a fact** (`__frames__`, causation=lineage) → replayable/auditable bitemporal frame history (time-travel viewer substrate). `RenderHost` is a swappable edge (SVG now / canvas/wgpu later, like an executor). Two replays → byte-identical; fact change → predictable frame change. Pure headless — no GPU/window/network/VM. (LAB-MACHINE-FRAME-PROJECTION-P1) |
...
- `tests/frame_projection_tests.rs` (6) — **machine → frame projection** (FP-P1, headless): project
  world from machine facts → Frame (world_digest + source_receipt_id); deterministic replay
  byte-identical; fact change → predictable frame change (sx 200→250); render host swappable (SVG/
  JSON); frame is a fact (`__frames__`, time-travel history); empty world stable.
