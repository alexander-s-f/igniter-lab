...
fn concurrent_distinct_keys_run_in_parallel() {
    rt().block_on(async {
        ...
        let (report, statuses) = tokio::join!(lp.run_concurrent(&orch, &pol), client_storm(addr, &keys));
        let report = report.unwrap();

        assert!(report.booted);
        assert_eq!(report.requests_served, 6);
        assert!(report.max_in_flight_observed > 1, "the loop genuinely served concurrently");
        assert_eq!(report.max_in_flight_observed, 4, "concurrency is bounded by max_in_flight");
...
fn concurrent_same_key_exactly_one_effect() {
    rt().block_on(async {
        ...
        assert_eq!(report.requests_served, 6);
        assert_eq!(report.max_in_flight_observed, 6, "all six were in flight at once");
        assert!(statuses.iter().all(|s| *s == 200));
        assert_eq!(exec.attempts(), 1, "six concurrent same-key requests perform exactly one effect");
...
fn concurrent_distinct_parallel_same_key_serialized() {
    rt().block_on(async {
        ...
        assert_eq!(exec.applied_count(), 3, "one effect per distinct key, duplicates serialized away");
...
fn concurrent_deterministic_shutdown_no_leak() {
    rt().block_on(async {
        ...
        let (r1, _s) = tokio::join!(lp.run_concurrent(&orch, &pol), client_storm(addr, &["A", "B", "C"]));
        let r1 = r1.unwrap();
        ...
        let (r2, _s) = tokio::join!(lp.run_concurrent(&orch, &pol2), client_storm(addr, &["D", "E"]));
        let r2 = r2.unwrap();
        assert_eq!(r2.requests_served, 2);
        assert!(r2.booted);
        assert_eq!(exec.applied_count(), 5, "five distinct keys across two bounded runs → five effects");
...
fn concurrent_tick_drains_due_retry() {
    rt().block_on(async {
        ...
        enqueue_retry(&receipts, &eclock, &write_req("R1"), "write", &ep.authority_digest(), 3, 0.0).await.unwrap();
        ...
        let (report, _s) = tokio::join!(lp.run_concurrent(&orch, &pol), client_storm(addr, &["E1", "E2"]));
        let report = report.unwrap();

        assert_eq!(report.requests_served, 2);
        assert_eq!(report.ticks_run, 1, "exactly one host-owned tick (no background scheduler)");
        assert_eq!(report.retries_drained, 1, "the due retry intent was drained");
