...
fn commit_lifecycle_business_row_and_pg_receipt() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k1", intent("c1")), RunMode::Live)
            .await
            .unwrap();
        ...
        let hist = store.facts_for(RECEIPTS_STORE, &format!("{CAP}:k1"), None, None).await.unwrap();
        let states: Vec<String> = hist
            .iter()
            .filter_map(|f| f.value.get("state").and_then(|s| s.as_str()).map(|s| s.to_string()))
            .collect();
        assert!(states.contains(&"prepared".to_string()), "prepared gate fact must exist: {states:?}");
        assert!(states.contains(&"committed".to_string()), "committed terminal fact must exist: {states:?}");

        // terminal receipt records correlation + idempotency key, NOT raw SQL / business values.
        let terminal = store.read_as_of(RECEIPTS_STORE, &format!("{CAP}:k1"), f64::MAX).await.unwrap().unwrap();
        ...
        assert!(!receipt_text.contains("select ") && !receipt_text.contains("insert into"), "no raw SQL in receipt");
    });
}
...
fn raw_sql_payload_refused_structurally() {
    rt().block_on(async {
        ...
        assert_eq!(adapter.attempts(), 0, "adapter must never see a raw-SQL payload");
...
fn replay_same_key_same_payload_bypasses_adapter() {
    rt().block_on(async {
        ...
        let a = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("dup", intent("c1")), RunMode::Live).await.unwrap();
        let b = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("dup", intent("c1")), RunMode::Live).await.unwrap();

        assert_eq!(a.state, WriteState::Committed);
        assert_eq!(b.state, WriteState::Committed); // replayed from machine receipt
        assert_eq!(adapter.attempts(), 1, "machine receipt replay never reaches the adapter");
        assert_eq!(adapter.business_row_count(), 1);
    });
}
...
fn same_key_different_payload_refused_before_adapter() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("kx", intent("c1")), RunMode::Live).await.unwrap();
        // same idempotency key, different payload (correlation differs → digest differs).
        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("kx", intent("DIFFERENT")), RunMode::Live)
            .await
            .unwrap();

        assert_eq!(out.state, WriteState::Denied);
        assert!(out.detail.unwrap().contains("different payload"));
        assert_eq!(adapter.attempts(), 1, "second (conflicting) request must not reach the adapter");
        assert_eq!(adapter.business_row_count(), 1);
    });
}
...
fn pg_side_dedup_blocks_second_mutation_when_machine_receipt_lost() {
    rt().block_on(async {
        ...
        let a = run_write_effect(&reg, &store_a, &clock(), &passport(), "write", &write_req("k-lost", intent("c1")), RunMode::Live).await.unwrap();
        assert_eq!(a.state, WriteState::Committed);
        ...
        let b = run_write_effect(&reg, &store_b, &clock(), &passport(), "write", &write_req("k-lost", intent("c1")), RunMode::Live).await.unwrap();

        assert_eq!(b.state, WriteState::Committed);
        assert_eq!(b.result["duplicate"], json!(true), "second hit is a PG-side dedup");
        assert_eq!(adapter.attempts(), 2, "executor reached twice (machine receipt was lost)");
        assert_eq!(adapter.business_row_count(), 1, "but the business mutation happened exactly once");
    });
}
...
fn serialization_failure_is_retryable() {
    rt().block_on(async {
        let adapter = Arc::new(FakePostgresWriteAdapter::new(FakeWriteBehavior::SerializationFailure));
        let reg = registry(adapter.clone());
        let store = receipts();

        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-ser", intent("c1")), RunMode::Live).await.unwrap();
...
fn unknown_is_unknown_with_no_blind_retry() {
    rt().block_on(async {
        ...
        let first = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-unk", intent("c1")), RunMode::Live).await.unwrap();
        ...
        let second = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-unk", intent("c1")), RunMode::Live).await.unwrap();
        assert_eq!(second.state, WriteState::UnknownExternalState);
        assert!(second.detail.unwrap().contains("no blind retry"));
        assert_eq!(adapter.attempts(), 1, "unknown receipt must not trigger a blind re-execution");
    });
}
...
fn constraint_violation_is_permanent() {
    rt().block_on(async {
        let adapter = Arc::new(FakePostgresWriteAdapter::new(FakeWriteBehavior::ConstraintViolation));
        let reg = registry(adapter.clone());
        let store = receipts();

        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-con", intent("c1")), RunMode::Live).await.unwrap();
        assert_eq!(out.state, WriteState::PermanentFailure);
...
fn adapter_denied_maps_to_denied() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-den", intent("c1")), RunMode::Live).await.unwrap();
        assert_eq!(out.state, WriteState::Denied);
        assert_eq!(adapter.business_row_count(), 0);
    });
}
...
fn policy_gates_refuse_before_adapter() {
    rt().block_on(async {
        ...
        let bad_target = json!({"operation": "insert", "target": "secrets", "key": "x", "values": {}});
        let out1 = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-t", bad_target), RunMode::Live).await.unwrap();
        ...
        let out2 = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k-o", bad_op), RunMode::Live).await.unwrap();
        assert_eq!(out2.state, WriteState::Denied);

        assert_eq!(adapter.attempts(), 0, "gate refusals never reach the adapter");
        assert_eq!(adapter.business_row_count(), 0);
    });
}
