...
fn non_allowlisted_host_refused_before_send() {
    rt().block_on(async {
        ...
        let out = ext_exec(t.clone(), no_secrets())
        ...
        assert_eq!(
            t.sends(),
            0,
            "non-allowlisted host must be refused before any send"
        );
    });
}
...
fn allowlisted_https_get_succeeds_with_receipt() {
    rt().block_on(async {
        ...
        let out = run_effect_with_passport(
            ...
            &passport(),
            "read",
            &ereq("GET", &format!("https://{HOST}/ping"), "g1", json!({})),
            RunMode::Live,
        )
        .await
        .unwrap();
...
fn cert_invalid_permanent_tls_dns_retryable() {
    rt().block_on(async {
        ...
        let out = ext_exec(cert, no_secrets())
            .execute(&ereq("GET", &format!("https://{HOST}/x"), "k", json!({})))
            .await;
        assert_eq!(
            out.kind,
            OutcomeKind::PermanentFailure,
            "bad cert is a permanent security failure"
        );
        ...
        for e in [
            ...
            let out = ext_exec(t, no_secrets())
                .execute(&ereq("GET", &format!("https://{HOST}/x"), "k", json!({})))
                .await;
            assert_eq!(
                out.kind,
                OutcomeKind::Retryable,
                "{e:?} did not reach server → retryable"
            );
        }
    });
}
...
fn transport_error_is_an_auditable_receipt() {
    rt().block_on(async {
        ...
        run_effect_with_passport(
            ...
            &passport(),
            "read",
            &ereq("GET", &format!("https://{HOST}/x"), "e1", json!({})),
            RunMode::Live,
        )
        .await
        .unwrap();
        let r = receipts
            .read_as_of(RECEIPTS_STORE, "IO.HttpCapability:e1", f64::MAX)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(
            r.value["outcome_kind"],
            json!("retryable"),
            "transport errors are auditable outcomes"
        );
    });
}
...
fn no_external_post_mutation() {
    rt().block_on(async {
        ...
        assert!(out
            .failure_kind
            .unwrap()
            .contains("external mutation not permitted"));
        assert_eq!(t.sends(), 0);
    });
}
...
fn plain_http_refused_in_external_profile() {
    rt().block_on(async {
        let t = Arc::new(FakeHttpTransport::ok(200, "ok", vec![]));
        let out = ext_exec(t.clone(), no_secrets())
            .execute(&ereq("GET", &format!("http://{HOST}/x"), "k", json!({})))
            .await;
        assert_eq!(out.kind, OutcomeKind::PermanentFailure);
