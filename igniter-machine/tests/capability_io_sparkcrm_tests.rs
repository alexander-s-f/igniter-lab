...
use igniter_machine::http::{
    HttpTransport, MapSecretProvider, SecretProvider, TlsLoopbackHttpTransport,
};
...
fn forward_create_succeeds_receipt_redacts_and_correlates() {
    rt().block_on(async {
        ...
        assert!(
            !r.to_string().contains("test-token-s3cr3t"),
            "auth secret must never be in the receipt"
        );
...
fn replay_does_not_resend() {
    rt().block_on(async {
        ...
        .unwrap();
        assert_eq!(
            st.lock().unwrap().posts,
            1,
            "replay must not POST a second lead"
        );
    });
}
...
fn reconcile_by_correlation_landed_and_not_landed() {
    rt().block_on(async {
        ...
        run_write_effect(
            ...
            &passport(),
            "write",
            &create_req("c5a", "corr-5a"),
            RunMode::Live,
        )
        .await
        .unwrap();
        // the SparkCRM executor IS the correlation resolver (GET /status)
        let r = reconcile_unknown_by_correlation(&receipts, &*exec, &clock(), CAP, "c5a")
            .await
            .unwrap();
        assert_eq!(
            r,
            CorrelationReconcileResult::ResolvedCommitted,
            "server /status confirms it landed"
        );
        ...
        run_write_effect(
            ...
            &passport(),
            "write",
            &create_req("c5b", "corr-5b"),
            RunMode::Live,
        )
        .await
        .unwrap();
        let r2 = reconcile_unknown_by_correlation(&receipts, &*exec2, &clock(), CAP, "c5b")
            .await
            .unwrap();
        assert_eq!(
            r2,
            CorrelationReconcileResult::ResolvedPermanentFailure,
            "server /status 404 → did not land"
        );
    });
}
...
fn rate_limit_retryable_and_enqueues_intent() {
    rt().block_on(async {
        ...
        let intent = receipts
            .read_as_of(RETRY_QUEUE_STORE, "c7", f64::MAX)
            .await
            .unwrap();
...
fn status_taxonomy_4xx_permanent_5xx_unknown() {
    rt().block_on(async {
        ...
        let bad = run_write_effect(
            &reg,
            &receipts,
            &clock(),
            &passport(),
            "write",
            &create_req("c8a", "corr-8a"),
            RunMode::Live,
        )
        .await
        .unwrap();
        ...
        let down = run_write_effect(
            &reg2,
            &receipts,
            &clock(),
            &passport(),
            "write",
            &create_req("c8b", "corr-8b"),
            RunMode::Live,
        )
        .await
        .unwrap();
        assert_eq!(
            ...
            "5xx on a POST → mutation unknown"
        );
    });
}
