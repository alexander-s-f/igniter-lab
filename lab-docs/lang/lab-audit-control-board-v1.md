...
| A19 | Type IR is stringly / name-only soundness holes | Correctness / compiler soundness | Replace stringly type surfaces with an `IgType` enum model. | CLOSED (two slices; tails active) | `lab-igniter-compiler-type-ir-enum-p5-v0.md` (`enum IgType` helper boundary, variant-field generic check fails closed `OOF-KIND2`, SIR byte-identical); `lab-igniter-compiler-user-fn-signature-check-p6-v0.md` (B-U1: user-`def` call arity + parameter-type check at `Expr::Call`, `OOF-TY0`, structural via `IgType`). | Active: `LAB-IGNITER-COMPILER-RECORD-LITERAL-NONINLINE-FIELD-TYPING-P7` (B-U3) and `LAB-IGNITER-COMPILER-CALL-CONTRACT-ARG-TYPING-P8`; stdlib arg-typing later if live gap remains. |
...
| A23 | VM direct source-run / REPL missing | DX | Build source-to-run/REPL as a product DX surface, not as audit safety. | ACTIVE READINESS | Current waves mark `.igapp` runtime implemented but source-run missing. | Active: `LAB-IGNITER-VM-SOURCE-RUN-REPL-READINESS-P1`. |
