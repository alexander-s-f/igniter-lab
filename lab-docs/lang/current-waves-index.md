...
| --- | --- | --- | --- | --- | --- |
| TodoApp API / product hardening | Object create body, host surrogate id, account existence semantics, app error envelope, delete, keyset pagination. Verify in `server/igniter-web/IMPLEMENTED_SURFACE.md`, `examples/todo_postgres_app/API.md`, `scripts/check_todo_product_surface.sh`. | DB-free async HTTP runner and fake Postgres adapter paths: `tests/todo_postgres_async_runner_smoke_tests.rs`, `runtime/igniter-machine/tests/postgres_write_tests.rs`. Real local DB tests exist but are DSN-gated. | Product polish choices such as `{items,next}` pagination envelope and client `limit` shape. | Typed row destructuring, global protocol error envelope, schema migration runner, production DB ownership. | `LAB-TODOAPP-API-PAGINATION-ENVELOPE-READINESS-P*`; `LAB-TODOAPP-API-TYPED-ROWS-READINESS-P*`; `LAB-TODOAPP-API-CONTRACT-EXAMPLES-P*`. |
| IgWeb routing / rendering / ViewArtifact | `scope`, `resource`, nested routes, route-level `via`, context `let`/same-name `guard`, `Render`, `RenderView`, raw response mapping. Verify in `lang/igniter-compiler/src/igweb.rs`, `lang/igniter-compiler/tests/igweb_lowering_tests.rs`, `server/igniter-web/src/lib.rs`. | Todo view/render fixtures prove authored HTML and renderer behavior: `examples/todo_view_app`, `tests/todo_view_app_tests.rs`, `igniter-render-html` tests. | File export and richer layout vocabulary remain design threads. | Public hosting, stable CLI, `.igweb` public language authority, `.igv`/projection dialect canon. | `LAB-IGNITER-WEB-FILE-EXPORT-READINESS-P*`; `LAB-IGNITER-WEB-LAYOUT-VOCAB-P*`; `LAB-IGNITER-WEB-ROUTING-SOURCE-MAP-READINESS-P*`. |
...
| ReadThen / EffectHost runner | `ReadThen { plan, then, carry }` is runner-integrated and bounded; final `InvokeEffect` routes through `MachineEffectHost` in async machine mode. Verify in `server/igniter-web/src/lib.rs`, `src/read_dispatch.rs`, `src/machine_runner.rs`, `scripts/check_implemented_surface.sh`. | DB-free socket/runner tests prove staged reads and final effects without live DB: `readthen_socket_runner_tests`, `igweb_serve_machine_mode_tests`, `todo_postgres_async_runner_smoke_tests`. | Typed row destructuring and nicer staged-read authoring syntax are still design. | Sync mode still observes `InvokeEffect`; no typed row columns in `.ig`; no multi-DSN/cross-DB joins. | `LAB-IGNITER-WEB-TYPED-ROW-DESTRUCTURING-READINESS-P*`; `LAB-IGNITER-WEB-READTHEN-AUTHORING-SYNTAX-P*`; `LAB-IGNITER-WEB-MULTI-DSN-READINESS-P*` only after real product pressure. |
...
Run the next parallel wave around **provenance + typed-row/product polish**, because it connects the
...
2. `LAB-IGNITER-WEB-TYPED-ROW-DESTRUCTURING-READINESS-P*` — unblock Todo pagination envelope and
   future richer ReadThen continuations.
3. `LAB-TODOAPP-API-PAGINATION-ENVELOPE-READINESS-P*` — decide `{items,next}` and client `limit`
   only after typed-row constraints are explicit.
4. `LAB-STDLIB-PROBABILITY-DISTRIBUTIONS-P*` — keep science pressure moving on explicit PRNG state.

Keep these parallel, not merged: provenance/package identity, ReadThen typing, Todo product API, and
