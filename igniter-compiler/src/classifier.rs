...
impl Classifier {
    ...
    pub fn classify(&self, parsed: &SourceFile, sample_input: &serde_json::Value) -> ClassifiedProgram {
        let registry = self.build_assumption_registry(parsed);
        let mut contracts = Vec::new();
        let mut oof_log = Vec::new();

        for contract in &parsed.contracts {
            let mut classified_c = self.classify_contract(parsed, contract, sample_input, &registry);
            oof_log.append(&mut classified_c.oof_log.clone());
            contracts.push(classified_c);
        }

        let type_declarations = parsed.types.iter().map(|t| ClassifiedTypeDecl {
            kind: "type".to_string(),
            name: t.name.clone(),
            fields: t.fields.iter().map(|f| ClassifiedFieldDecl {
                name: f.name.clone(),
        ...
        let pass_result = if oof_log.iter().any(|d| d.rule.starts_with("OOF-") || d.rule.starts_with("E-IO-")) {
            "oof".to_string()
        } else {
            "ok".to_string()
        };
    ...
    fn build_assumption_registry(&self, parsed: &SourceFile) -> HashMap<String, serde_json::Value> {
        ...
        for a in &parsed.assumptions {
            ...
            entry.insert("kind".to_string(), serde_json::Value::String("assumption_entry".to_string()));
            entry.insert("name".to_string(), serde_json::Value::String(a.name.clone()));
            entry.insert("fields".to_string(), serde_json::to_value(&a.fields).unwrap());
            entry.insert("declared_in_module".to_string(), serde_json::to_value(&parsed.module).unwrap());
    ...
    fn classify_contract(
        ...
        for node in &contract.body {
            match node {
                ...
                BodyDecl::Read { name, type_annotation, .. } => {
                    let is_temp = self.is_temporal_type(type_annotation);
                    let fragment = if is_temp { "temporal".to_string() } else { "escape".to_string() };
                    symbol_fragments.insert(name.clone(), if is_temp { "core".to_string() } else { "escape".to_string() });
                    symbol_kinds.insert(name.clone(), if is_temp { "temporal_read".to_string() } else { "read".to_string() });
                }
                ...
                BodyDecl::FoldStream { name, bound, .. } => {
                    ...
                    symbol_fragments.insert(name.clone(), if is_bounded { "core".to_string() } else { "oof".to_string() });
                ...
                BodyDecl::Loop { name, body: loop_body, .. } => {
                    symbol_fragments.insert(name.clone(), "core".to_string());
                ...
                BodyDecl::ServiceLoop { name, body: loop_body, .. } => {
                    symbol_fragments.insert(name.clone(), "escape".to_string());
        ...
        for node in &contract.body {
            if let BodyDecl::Capability { name, type_annotation } = node {
        ...
        for node in &contract.body {
            match node {
                BodyDecl::Compute { expr, .. } | BodyDecl::Snapshot { expr, .. } => {
                    check_expr_io(expr, &capabilities, &effects, &contract.name, is_pure, &mut diagnostics);
                }
                BodyDecl::FoldStream { expr, .. } => {
                    check_expr_io(expr, &capabilities, &effects, &contract.name, is_pure, &mut diagnostics);
                }
                BodyDecl::Loop { collection, body: loop_body, .. } => {
                    check_expr_io(collection, &capabilities, &effects, &contract.name, is_pure, &mut diagnostics);
                    for inner in loop_body {
                ...
                BodyDecl::ServiceLoop { body: loop_body, .. } => {
                    for inner in loop_body {
                        if let BodyDecl::Compute { expr, .. } = inner {
                            check_expr_io(expr, &capabilities, &effects, &contract.name, is_pure, &mut diagnostics);
                        }
        ...
        for node in &contract.body {
            match node {
                BodyDecl::Capability { name, type_annotation } => {
                ...
                BodyDecl::Effect { name, capability_ref } => {
                ...
                BodyDecl::Stream { name, type_annotation, fragment_class, escape_capability } => {
                    declarations.push(ClassifiedDecl {
                ...
                BodyDecl::Read { name, type_annotation, from, lifecycle, scoped_by, cardinality, schema_version, tenant_free } => {
                    let is_temp = self.is_temporal_type(type_annotation);
                    let fragment = if is_temp { "temporal".to_string() } else { "escape".to_string() };
                    ...
                    if is_temp {
                        ...
                        decl.required_capability = Some(if t_name == "BiHistory" { "bihistory_read".to_string() } else { "history_read".to_string() });
                        decl.temporal_axis = Some(if t_name == "BiHistory" { "bitemporal".to_string() } else { "valid_time".to_string() });
                    }
                ...
                BodyDecl::FoldStream { name, expr, bound, type_annotation } => {
                    let deps = self.expr_refs(expr);
                    for dep in &deps {
                        if symbol_kinds.get(dep).map(|s| s.as_str()) == Some("stream") {
                            fold_stream_stream_refs.entry(dep.clone()).or_default().push(name.clone());
                        }
                    }

                    let is_bounded = bound.is_some();
                    let fragment = if is_bounded { "core".to_string() } else { "oof".to_string() };
                    ...
                    declarations.push(ClassifiedDecl {
                        ...
                        deps,
                        missing_refs: Vec::new(),
                        type_annotation: type_annotation.as_ref().map(|t| serde_json::to_value(t).unwrap()),
                ...
                BodyDecl::Invariant { name, predicate_ref, severity, label, message, overridable_with } => {
                    let mut missing = Vec::new();
                    ...
                    let fragment = if missing.is_empty() { "core".to_string() } else { "oof".to_string() };
                ...
                BodyDecl::Compute { name, expr, type_annotation } => {
                    let deps = self.expr_refs(expr);
                    let mut missing = Vec::new();
                    for dep in &deps {
                        if !symbol_fragments.contains_key(dep) && symbol_kinds.get(dep).map(|s| s.as_str()) != Some("compute") {
                    ...
                    for dep in &deps {
                        if symbol_kinds.get(dep).map(|s| s.as_str()) == Some("stream") {
                            diagnostics.push(ClassifierDiagnostic {
                                ...
                                message: format!("Direct use of stream '{}' is OOF - use fold_stream instead", dep),
                                node: name.clone(),
                                line: None,
                            });
                        }
                    ...
                    let upstream_oof = deps.iter().any(|dep| symbol_fragments.get(dep).map(|s| s.as_str()) == Some("oof"));
                    let mut fragment = if missing.is_empty() && !upstream_oof { "core".to_string() } else { "oof".to_string() };
                ...
                BodyDecl::Snapshot { name, expr, lifecycle } => {
                    let deps = self.expr_refs(expr);
                    let mut missing = Vec::new();
                    for dep in &deps {
                        if !symbol_fragments.contains_key(dep) && symbol_kinds.get(dep).map(|s| s.as_str()) != Some("compute") {
                ...
                BodyDecl::Output { name, type_annotation, lifecycle, evidence } => {
                    let mut missing = Vec::new();
                    if !symbol_fragments.contains_key(name) && symbol_kinds.get(name).map(|s| s.as_str()) != Some("compute") {
                        missing.push(name.clone());
                        diagnostics.push(ClassifierDiagnostic {
                            rule: "OOF-P1".to_string(),
                    ...
                    let src_fragment = symbol_fragments.get(name).map(|s| s.as_str()).unwrap_or("oof");
                    let mut fragment = if missing.is_empty() && src_fragment == "core" { "core".to_string() } else { "oof".to_string() };
                ...
                BodyDecl::Loop { name, item, collection, max_steps, body: loop_body } => {
                    ...
                    for body_node in loop_body {
                        if let BodyDecl::Compute { expr, name: inner_name, .. } = body_node {
                            ...
                            let fragment = if expr_has_io_call(expr) { "escape".to_string() } else { "core".to_string() };
                    ...
                    let mut fragment = if missing.is_empty() && !upstream_oof { "core".to_string() } else { "oof".to_string() };
                    if fragment != "oof" {
                        let mut has_escape = deps.iter().any(|dep| symbol_fragments.get(dep).map(|s| s.as_str()) == Some("escape"));
                        for body_node in loop_body {
                    ...
                    let expr_val = if let Some(ref_name) = collection.get_name() {
                        Some(Expr::Ref { name: ref_name.to_string() })
                    } else {
                        None
                    };
                ...
                BodyDecl::ServiceLoop { name, interval, body: loop_body } => {
                    ...
                    for inner_decl in loop_body {
                        if let BodyDecl::Compute { name: inner_name, .. } = inner_decl {
                            symbol_kinds.insert(inner_name.clone(), "compute".to_string());
                            symbol_fragments.insert(inner_name.clone(), "escape".to_string());
                        }
                    }
                    ...
                    options.insert("interval_value".to_string(), WindowValue::Int(interval.value as i64));
                    options.insert("interval_unit".to_string(), WindowValue::Str(interval.unit.clone()));
                    ...
                    for inner_decl in loop_body {
                        if let BodyDecl::Compute { name: inner_name, expr: inner_expr, type_annotation: inner_type_annotation } = inner_decl {
                            ...
                            let mut inner_missing = Vec::new();
                            for dep in &inner_deps {
                                if dep != name && !symbol_fragments.contains_key(dep) && symbol_kinds.get(dep).map(|s| s.as_str()) != Some("compute") {
                                    inner_missing.push(dep.clone());
                                    diagnostics.push(ClassifierDiagnostic {
                                        rule: "OOF-P1".to_string(),
                                        message: format!("Unresolved symbol: {}", dep),
                            ...
                            let upstream_oof = inner_deps.iter().any(|dep| dep != name && symbol_fragments.get(dep).map(|s| s.as_str()) == Some("oof"));
                            let inner_fragment = if inner_missing.is_empty() && !upstream_oof { "escape".to_string() } else { "oof".to_string() };
        ...
        if window_declarations.is_empty() {
            for stream_name in fold_stream_stream_refs.keys() {
                diagnostics.push(ClassifierDiagnostic {
                    ...
                    message: format!("stream '{}' has no window - every stream must declare a window", stream_name),
        ...
        if modifier == "irreversible" {
            for d in &declarations {
                if has_compensation_nodes(d) {
                    diagnostics.push(ClassifierDiagnostic {
                        rule: "OOF-M1".to_string(),
                        message: format!("irreversible contract '{}' cannot define compensations", contract.name),
                        node: contract.name.clone(),
                        line: None,
                    });
                    break;
        ...
        ClassifiedContract {
            ...
            contract_id: format!("{}.{}", parsed.module.clone().unwrap_or_default(), contract.name),
            ...
            Expr::IfExpr { cond, then, else_block } => {
                ...
                for s in &then.stmts {
                    match s {
                        ...
                        crate::parser::Stmt::ExprStmt { expr } => self.collect_expr_refs(expr, refs),
                    }
                }
            ...
            Expr::Lambda { params, body } => {
                ...
                match body.as_ref() {
                    ...
                    ExprOrBlock::Block(b) => {
                        for s in &b.stmts {
                            match s {
                                crate::parser::Stmt::Let { expr, .. } => self.collect_expr_refs(&expr, &mut temp_refs),
                                crate::parser::Stmt::ExprStmt { expr } => self.collect_expr_refs(&expr, &mut temp_refs),
                            }
                        }
    ...
    fn contract_fragment_for(&self, declarations: &[ClassifiedDecl], diagnostics: &[ClassifierDiagnostic], modifier: &str) -> String {
    ...
    fn symbol_table(&self, kinds: &HashMap<String, String>, fragments: &HashMap<String, String>) -> Vec<ClassifiedSymbol> {
        let mut keys: Vec<&String> = kinds.keys().collect();
        keys.sort();
        keys.iter().map(|name| ClassifiedSymbol {
            name: (*name).clone(),
...
fn check_expr_io(
    ...
    match expr {
        Expr::Call { fn_name, args } => {
            if fn_name.starts_with("stdlib.IO.") {
                if is_pure {
                    diagnostics.push(ClassifierDiagnostic {
                        ...
                        message: format!("I/O calls are blocked in pure contract '{}'", contract_name),
                    ...
                    let min_args = if fn_name == "stdlib.IO.write_text" || fn_name == "stdlib.IO.write_json" {
                        3
                    } else {
                        2
                    };
                    if args.len() < min_args {
                        diagnostics.push(ClassifierDiagnostic {
                            ...
                            message: format!("I/O call to '{}' is missing capability argument", fn_name),
                            node: contract_name.to_string(),
                            line: None,
                        });
                    } else {
                        ...
                        if let Expr::Ref { name: cap_ref } = cap_arg {
                            if !capabilities.contains_key(cap_ref) {
                                diagnostics.push(ClassifierDiagnostic {
                                    rule: "E-IO-CAP-UNKNOWN".to_string(),
                                    message: format!("Capability '{}' referenced in I/O call is undeclared", cap_ref),
                                    node: contract_name.to_string(),
                                    line: None,
                                });
                            } else {
                                ...
                                let is_write_fn = fn_name == "stdlib.IO.write_text" || fn_name == "stdlib.IO.write_json";
                                if is_write_fn {
            ...
            for arg in args {
                check_expr_io(arg, capabilities, effects, contract_name, is_pure, diagnostics);
            }
        }
        Expr::BinaryOp { left, right, .. } => {
            check_expr_io(left, capabilities, effects, contract_name, is_pure, diagnostics);
            check_expr_io(right, capabilities, effects, contract_name, is_pure, diagnostics);
        }
        Expr::UnaryOp { operand, .. } => {
            check_expr_io(operand, capabilities, effects, contract_name, is_pure, diagnostics);
        }
        Expr::FieldAccess { object, .. } => {
            check_expr_io(object, capabilities, effects, contract_name, is_pure, diagnostics);
        }
        Expr::IndexAccess { object, index } => {
            check_expr_io(object, capabilities, effects, contract_name, is_pure, diagnostics);
            check_expr_io(index, capabilities, effects, contract_name, is_pure, diagnostics);
        }
        Expr::SliceRecord { fields } => {
            for v in fields.values() {
                check_expr_io(v, capabilities, effects, contract_name, is_pure, diagnostics);
            }
        }
        Expr::IfExpr { cond, then, else_block } => {
            check_expr_io(cond, capabilities, effects, contract_name, is_pure, diagnostics);
            for s in &then.stmts {
                match s {
                    crate::parser::Stmt::Let { expr, .. } => {
                        check_expr_io(expr, capabilities, effects, contract_name, is_pure, diagnostics);
                    }
                    crate::parser::Stmt::ExprStmt { expr } => {
                        check_expr_io(expr, capabilities, effects, contract_name, is_pure, diagnostics);
                    }
            ...
            if let Some(re) = &then.return_expr {
                check_expr_io(re, capabilities, effects, contract_name, is_pure, diagnostics);
            }
            if let Some(eb) = else_block {
                for s in &eb.stmts {
                    match s {
        ...
        Expr::Lambda { body, .. } => {
            match &**body {
                crate::parser::ExprOrBlock::Expr(e) => {
                    check_expr_io(e, capabilities, effects, contract_name, is_pure, diagnostics);
                }
                crate::parser::ExprOrBlock::Block(b) => {
                    for s in &b.stmts {
                        match s {
        ...
        Expr::ArrayLiteral { items } => {
            for item in items {
                check_expr_io(item, capabilities, effects, contract_name, is_pure, diagnostics);
            }
        }
        Expr::RecordLiteral { fields } => {
            for v in fields.values() {
