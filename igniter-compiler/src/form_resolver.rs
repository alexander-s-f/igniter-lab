use crate::form_registry::{FormRegistry, FormEntry, FormDiagnostic};
...
impl FormResolver {
    pub fn resolve(typed: &TypedProgram, registry: &FormRegistry) -> ResolvedProgram {
        ...
        for contract in &typed.contracts {
            let symbol_types: HashMap<String, String> = contract.symbols.iter()
        ...
        ResolvedProgram { resolved_forms, trace, ambiguities, diagnostics }
    }
        ...
        match expr {
            Expr::BinaryOp { op, left, right } => {
                ...
                Self::resolve_trigger(
                    op, "binary_op", contract_name, decl_name,
                    registry, signatures, type_facts, resolved, trace, ambiguities, diagnostics,
                );
                Self::walk_expr(left,  registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
                Self::walk_expr(right, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
            }
            Expr::UnaryOp { op, operand } => {
                let operand_type = Self::expr_type(operand, symbol_types);
                Self::resolve_trigger(
                    op, "unary_op", contract_name, decl_name,
                    registry,
                    signatures,
                    TypeFacts { operands: vec![operand_type], result: None },
                    resolved, trace, ambiguities, diagnostics,
                );
                Self::walk_expr(operand, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
            }
            Expr::FieldAccess { object, field } => {
                let trigger = format!(".{}", field);
                let object_type = Self::expr_type(object, symbol_types);
                Self::resolve_trigger(
                    &trigger, "field_access", contract_name, decl_name,
                    registry,
                    signatures,
                    TypeFacts { operands: vec![object_type], result: None },
                    resolved, trace, ambiguities, diagnostics,
                );
                Self::walk_expr(object, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
            }
            Expr::Call { fn_name, args } => {
                ...
                for arg in args {
                    Self::walk_expr(arg, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
                }
                ...
                trace.push(TraceEvent {
                    ...
                    typed_operands: args.iter().map(|arg| Self::expr_type(arg, symbol_types)).collect(),
                    typed_result: None,
            ...
            Expr::IfExpr { cond, then, else_block } => {
                Self::walk_expr(cond, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
                for stmt in &then.stmts {
                    Self::walk_stmt_expr(stmt, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
                }
                if let Some(ret) = &then.return_expr {
                    Self::walk_expr(ret, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
                }
            ...
            Expr::IndexAccess { object, index } => {
                Self::walk_expr(object, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
                Self::walk_expr(index,  registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics);
            }
            Expr::Lambda { body, .. } => {
                match body.as_ref() {
                    crate::parser::ExprOrBlock::Expr(e) =>
                        Self::walk_expr(e, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics),
    ...
    fn walk_stmt_expr(
        ...
        match stmt {
            ...
            crate::parser::Stmt::ExprStmt { expr } =>
                Self::walk_expr(expr, registry, signatures, symbol_types, contract_name, decl_name, resolved, trace, ambiguities, diagnostics),
        }
    }
    ...
    fn resolve_trigger(
        ...
        let all_candidates: Vec<&FormEntry> = registry.trigger_index
            .get(trigger)
        ...
        let candidates: Vec<&FormEntry> = all_candidates.into_iter()
            .filter(|e| {
        ...
        let candidates: Vec<&FormEntry> = candidates.into_iter()
            .filter(|entry| {
        ...
        if candidates.is_empty() {
            ...
            trace.push(TraceEvent {
                ...
                trigger:     trigger.to_string(),
                expr_kind:   expr_kind.to_string(),
                candidates:  refused_candidates.iter().map(|c| c.contract.clone()).collect(),
                resolved_to: None,
                contract_ctx: contract_name.to_string(),
                decl_name:   decl_name.to_string(),
                typed_operands: type_facts.operands,
    ...
    fn candidate_refusal(
        ...
        let expected: Vec<String> = signature.input_types.iter()
            .take(actual_operands.len())
            .cloned()
            .collect();

        let mismatch = expected.iter().zip(actual_operands.iter()).any(|(expected, actual)| {
            expected != "Unknown" && actual != "Unknown" && expected != actual
        });

        if mismatch {
            Some(Self::refusal(entry, "operand_type_mismatch", expected, actual_operands.to_vec()))
        } else {
            None
        }
    }

    fn refusal(entry: &FormEntry, reason: &str, expected_operands: Vec<String>, actual_operands: Vec<String>) -> RefusedCandidate {
        RefusedCandidate {
    ...
    fn expr_type(expr: &Expr, symbol_types: &HashMap<String, String>) -> String {
        match expr {
            ...
            Expr::Ref { name } => symbol_types.get(name).cloned().unwrap_or_else(|| "Unknown".to_string()),
            Expr::BinaryOp { op, left, right } => {
                let left_type = Self::expr_type(left, symbol_types);
                let right_type = Self::expr_type(right, symbol_types);
                Self::binary_result_type(op, &left_type, &right_type).unwrap_or_else(|| "Unknown".to_string())
            }
    ...
    fn binary_result_type(op: &str, left_type: &str, right_type: &str) -> Option<String> {
        match op {
            ...
            "++" if left_type == "Collection" && right_type == "Collection" => Some("Collection".to_string()),
            ">" | "<" | ">=" | "<=" | "==" | "!=" | "&&" | "||" => Some("Bool".to_string()),
            _ => None,
        }
    }
