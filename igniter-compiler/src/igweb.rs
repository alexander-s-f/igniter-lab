...
//
// v0 convention (documented, not parsed from `.igweb`): the generated module is `AppRoutes` and it
// imports a `WebTypes` module (Request + Decision) and a `TodoHandlers` module (the handler contracts).
// Import plumbing is project wiring, not route authoring, so it is fixed here rather than in the grammar.
...
fn generate_ig(entry: &str, routes: &[Route]) -> String {
    let mut out = String::new();
    out.push_str("-- GENERATED by lower_igweb (LAB-IGNITER-WEB-ROUTING-LOWERING-P4). Do not edit by hand.\n");
    out.push_str("-- Routing/product meaning lives here (the app), never in igniter-server.\n");
    out.push_str("module AppRoutes\n");
    out.push_str("import WebTypes\n");
    out.push_str("import TodoHandlers\n\n");
