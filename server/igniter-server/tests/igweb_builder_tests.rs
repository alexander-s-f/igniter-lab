...
fn builder_reload_swaps_whole_loaded_app() {
    ...
    let b_paths = write_dir(
        "srv_reload_b",
        &[("handlers.ig", HANDLERS), ("routes_b.igweb", b_igweb)],
    );
    let app_b = build_igweb_app(IgWebBuildInput {
        sources: b_paths,
        entry: "Serve".into(),
    })
    .expect("build app B");
    ...
    assert_eq!(
        ...
        404,
        "reload swapped the whole loaded IgWeb app"
    );
...
fn builder_composes_with_middleware() {
    ...
    assert_eq!(
        ...
        401,
        "auth short-circuits before the IgWeb app"
    );
    assert_eq!(
        roundtrip(
            &stack,
        ...
        .0,
        200,
        "valid token reaches the IgWeb app"
    );
}
