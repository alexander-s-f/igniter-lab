...
| Typed rows in the shipped Todo JSON API routes | Partly adopted | The Todo JSON list route uses typed rows + `DatasetMeta` and returns `{items,next}` via `RespondJson` (P50). The single-todo show route still uses the legacy `rows_json` continuation; migrating show is a separate product slice, not a substrate blocker. |
| Stronger typed projection landings | Partly implemented / still bounded | v0 typed rows support String/Text/Integer/Bool plus limited Map/Collection[String]; P23 adds opt-in exact `Decimal[N]` crossing from host `Decimal{scale}`. Timestamp, nested records/Json, a generic `Dataset[T]` envelope, typed Bool adoption for the Todo `done` field, and broad decoder policy remain future slices. |
