...
Live-bind status refreshed for `P32`/`P34`: IgWeb calls `igniter-server::serving_gate::authorize_bind`
before both sync and machine-mode listener binds, and `P34` adds a parse-only `[host.live_bind]`
operator checklist with fail-closed diagnostics; no public bind is enabled in v0 (the checklist is
readiness, not wired into `authorize_bind`).
