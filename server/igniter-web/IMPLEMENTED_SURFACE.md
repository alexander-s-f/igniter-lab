...
Last refreshed against source: 2026-06-28 (route sugar `P16`-`P20`/`P22`/`P26`/`P27`,
...
DB-backed Decimal money report proof `P24`, host-config typed read field kinds `P33`, and compiler
package/admission pointers).
Live-bind status refreshed for `P32`/`P34`/`P36`/`P37`: IgWeb calls `igniter-server::serving_gate::authorize_bind`
before both sync and machine-mode listener binds, and `P34` adds a parse-only `[host.live_bind]`
operator checklist with fail-closed diagnostics. `P36` adds a report-only dry-run, and `P37`
host-verifies its signed-passport verifier material before setting `signed_passport_path_wired=true`
for non-loopback dry-run verdicts. No public bind is enabled in v0 (the real run path still passes
no checklist to `authorize_bind`).
...
| Read freshness / opt-in replay | `runner-integrated` | `StagedReadHost::execute` keys receipts on `correlation_id`+`plan_digest`; uncorrelated reads run fresh (`P12`/`P23`). Only a **client** correlation opts into replay — a trace-derived one (`x-correlation-source: trace`, set by `LoadedMiddleware`) runs fresh too (`P58`). `uncorrelated_same_plan_reads_run_fresh`, `trace_derived_correlation_runs_fresh`, `explicit_same_correlation_same_plan_replays`, `distinct_plans_never_collide`; middleware `trace_correlation_provenance_tests`. |
...
| `host.toml` read/write/effects | Implemented | `src/host_config.rs` (`parse_host_config`/`load_host_config`/`resolve_host_config`) | Keys: `[host] mode` (`"loopback"` only); `[effects.<t>]` `route`(req)+`passport_env`(opt); `[postgres.read]` `dsn_env`(req)+`source`+`fields`+`row_limit`(def 100)+`capability`; `[postgres.read.<name>]` extra allowlisted `(source, fields)` (`P38`); `[postgres.read.<source>.fields]` per-field typed read kinds (`text`, `integer`, `bool`, `decimal:<scale>`) (`P33`); `[postgres.write]` `dsn_env`(req)+`targets`+`ops`+`capability`+`key_column`(def `id`)+`columns`. Fail-closed on unknown section/key, inline secrets (`dsn`/`password`/`secret`/`token`/`passport`/`api_key`/`key`/`cert`), template `*_env`, route w/o `/`, missing `route`/`dsn_env`, bad `mode`, unsupported field kind, kind section without matching source, or kind for a non-allowlisted field. `resolve_host_config` resolves every `*_env` **before** any socket bind. |
| `[host.live_bind]` checklist (parse-only) | Implemented (`P34`) | `src/host_config.rs` (`LiveBindConfig`/`LiveBindTlsConfig`); `HostConfig.live_bind`; `examples/todo_postgres_app/host.live_bind.example.toml` | **Readiness only — does NOT enable public bind.** Parses + validates the live-bind checklist (`signed_passport_path` file ref, `body_cap_enabled`/`read_timeout_enabled`/`fail_closed_auth_enabled` must be `"true"`, `operator_signoff`, `[host.live_bind.inbound_tls]` `mode` = `terminated_upstream`+`upstream_header_policy` \| `native_tls`+`cert_file`/`key_file`; no `"none"`). An incomplete/invalid checklist fails closed as `CONFIG_PARSE` before any bind (`LiveBind*` error variants). The runner still calls `authorize_bind(addr, None)`, so a complete checklist does **not** open a non-loopback listener; `[host] mode = "public"` is still refused. Booleans are quoted (`"true"`/`"false"`) — parser is quoted-value uniform. P37 gives `signed_passport_path` host-verified backing only in the report-only dry-run path, not in real `Run`. Proof doc `lab-docs/lang/lab-igniter-web-host-live-bind-checklist-parse-p34-v0.md`. |
| `live-bind-check` dry-run verdict (report-only) | Implemented (`P36`/`P37`) | `src/live_bind_check.rs` (`load_inbound_passport_verifier`/`config_to_checklist`/`evaluate`/`LiveBindVerdict`); `src/bin/igweb-serve.rs` `LiveBindCheck` arm; `RunnerCliCommand::LiveBindCheck` in `src/lib.rs` | **Report-only — never binds, grants no bind authority.** `igweb-serve live-bind-check --host-config PATH [--addr HOST:PORT]` converts the parsed `[host.live_bind]` checklist into the server `LiveBindChecklist` and calls the **pure** `authorize_bind` to print a verdict: `[LIVE_BIND_DRY_RUN] … verdict=would_authorize\|would_refuse … socket_opened=false public_bind=closed`. Default `--addr` is `0.0.0.0:8080` (the public-bind question). Exit 0 authorize / 5 `BIND_REFUSED` refuse / 2 `CONFIG_PARSE` on bad config. P37 changes the non-loopback dry-run conversion so `signed_passport_path_wired=true` is set only after host I/O loads and validates a v0 durable verifier file: one 64-hex-char 32-byte trusted issuer key for the current symmetric `PassportVerifier`, validated with a synthetic signed-passport probe. Missing file/ref refuses with `signed_passport_verifier_unavailable`; malformed material refuses with `signed_passport_verifier_invalid`; verdicts never print key/passport material or file contents. Proof docs `lab-docs/lang/lab-igniter-web-live-bind-dry-run-verdict-p36-v0.md`, `lab-docs/lang/lab-igniter-web-inbound-signed-passport-durable-key-p37-v0.md`. |
| `terminated_upstream` TLS runbook | Documented (`TLS runbook P38`) | `lab-docs/lang/lab-igniter-web-tls-terminated-upstream-runbook-p38-v0.md` | Operator contract only. `terminated_upstream` means TLS terminates at a trusted upstream proxy/LB before IgWeb; IgWeb still has no TLS transport. Forwarding headers are hints only unless the request is already behind a trusted upstream boundary. P39 remains lab-only/human-gated; `native_tls` is blocked for actual non-loopback proof until a real TLS accept path exists. Public bind remains closed. |
| `live-bind-proof` human-gated lab proof | Implemented (`P39`) | `src/live_bind_proof.rs`; `src/bin/igweb-serve.rs` `LiveBindProof` arm; `RunnerCliCommand::LiveBindProof` in `src/lib.rs` | **Lab authorization proof only — never binds, grants no production bind authority.** Requires `IGNITER_LIVE_BIND_HUMAN_ACK=I_UNDERSTAND_IGNITER_LAB_LIVE_BIND_P39`, a non-loopback `--addr`, `[host.live_bind.inbound_tls] mode = "terminated_upstream"`, `upstream_header_policy = "trusted_proxy_only"`, and the P37 verifier-backed checklist. Calls the pure `authorize_bind(addr, Some(checklist))` and prints `[LIVE_BIND_PROOF] ... bind_attempted=false socket_opened=false public_bind=closed`. `native_tls` refuses with `native_tls_transport_not_implemented`. Proof doc `lab-docs/lang/lab-igniter-web-live-bind-human-gated-proof-p39-v0.md`. |
...
| Surface | Status | Note |
| --- | --- | --- |
| Public listener mode | Closed | IgWeb calls `igniter-server::serving_gate::authorize_bind(addr, None)` before sync and machine-mode binds. Loopback is allowed; non-loopback fails closed with `BIND_REFUSED` / `non_loopback_without_checklist` before `TcpListener::bind`. `ServingPolicy::loopback_only()` remains post-bind defense-in-depth. A parse-only `[host.live_bind]` checklist exists (`P34`), P37 backs its signed-passport bit in check/proof paths, and P39 can human-gate a lab authorization proof via pure `authorize_bind(addr, Some(checklist))`; none of these are wired into normal `Run`. A complete checklist still does not open a public listener. No TLS transport exists in IgWeb v0. |
...
| Stronger typed projection landings | Partly implemented / still bounded | v0 typed rows support String/Text/Integer/Bool plus limited Map/Collection[String]; P23 adds opt-in exact `Decimal[N]` crossing from host `Decimal{scale}`. P33 exposes host-config field kinds for `text`, `integer`, `bool`, and `decimal:<scale>` through `[postgres.read.<source>.fields]`, so Bool and Decimal adoption no longer requires Rust-only `allow_source_typed` fixture wiring. Timestamp, nested records/Json, native arrays, schema inference, and a generic `Dataset[T]` envelope remain future slices. |
