...
/// ASK2 precedent: the runtime output of the `.ig` generated by DESUGARING `web/list_view.form`
/// (terse `col { row { leaf } }`) — command-produced (desugar → igc → igniter-vm), not hand-authored.
const FORM_ENVELOPE: &str = include_str!("fixtures/list_view_form.runtime.json");
...
#[test]
fn desugared_form_view_renders_through_the_bridge() {
    // ASK2 precedent end-to-end: terse `col { row { leaf } }` --desugar--> .ig --igc--> .igapp
    // --igniter-vm--> THIS Element tree --render_ig_view--> SVG. Authored as a terse form; rendered live.
    let element = result_of(FORM_ENVELOPE);
    let svg = render_ig_view(&element.to_string(), 720, 440);
    assert!(svg.starts_with("<svg"));
    for needle in LABELS {
        assert!(svg.contains(needle), "form-desugared render missing {needle:?}");
    }
}
