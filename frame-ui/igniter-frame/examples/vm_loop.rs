...
fn run_vm(vm: &str, igapp: &str, entry: &str, input: &Value) -> Value {
    ...
    let line = stdout
        ...
        .unwrap_or_else(|| {
            eprintln!("no result envelope from {entry}:\n{stdout}\n{}", String::from_utf8_lossy(&out.stderr));
            exit(1);
        });
...
fn main() {
    ...
    let igapp = args.next().unwrap_or_else(|| { eprintln!("usage: vm_loop <igapp> <igniter-vm>"); exit(2); });
    let vm = args.next().unwrap_or_else(|| { eprintln!("usage: vm_loop <igapp> <igniter-vm>"); exit(2); });

    // ---- frame 0: render the initial state via the .ig VIEW on the VM --------------------------------
    let state0 = json!({ "sel": "" });
    let element0 = run_vm(&vm, &igapp, "View", &json!({ "state": state0 }));
    println!("frame 0  ·  View(sel=\"\")  ·  status leaf text = {:?}", status_text(&element0));
    ...
    let intent = derive_intent(&frame, &InputEvent { kind: "click".into(), x: cx, y: cy, payload: json!(null) })
        .expect("click derives an intent");
    let key = intent.params.get("key").and_then(|v| v.as_str()).unwrap_or("").to_string();
    println!("click ({cx},{cy})  ·  derive_intent -> action={:?} key={:?}", intent.action, key);
    ...
    let state1 = run_vm(&vm, &igapp, "Reduce", &json!({ "state": state0, "key": key }));
    ...
    println!("frame 1  ·  View(sel=\"lead:1\")  ·  status leaf text = {:?}", status_text(&element1));
    assert_eq!(status_text(&element0), "");
    assert_eq!(status_text(&element1), "lead:1", "the re-projected view reflects the new state");
