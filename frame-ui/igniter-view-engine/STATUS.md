...
- It is **also a live preview/runtime backend for `igniter-ide`** — the Tauri IDE reads from this exact
  directory at runtime (references in `ide/igniter-ide/src-tauri/{lib.rs,commands.rs}` +
  `ide/igniter-ide/src/lib/components/ViewInspector.svelte`, resolved via
  `resolve_workspace_path("frame-ui/igniter-view-engine/…")` which points at this copy).
