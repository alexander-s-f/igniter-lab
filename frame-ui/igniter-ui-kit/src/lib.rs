...
impl Form {
    ...
    pub fn initial_world(&self) -> Vec<(String, Value)> {
        ...
        for c in &self.body {
            match c {
                Component::Text { id, .. } => w.push((
                    id.clone(),
                    json!({ "kind": "text", "value": "", "focused": false }),
                )),
                Component::Select { id, .. } => {
                    w.push((id.clone(), json!({ "kind": "select", "selected": -1 })))
                }
                Component::Checkbox { id, .. } => {
                    w.push((id.clone(), json!({ "kind": "checkbox", "checked": false })))
                }
                _ => {}
            }
        }
        w.push((
            "__form__".to_string(),
            json!({ "submitted": false, "ok": false, "errors": {} }),
        ));
        w
    }
...
impl Projector for FormProjector {
    fn project(
        ...
        let mut y = MARGIN;
        let push = |nodes: &mut Vec<ProjectedNode>,
                    y: &mut i64,
                    id: String,
                    h: i64,
                    intent: Option<Value>,
            ...
            match c {
                ...
                Component::Select {
                    ...
                    let sel = st(id)
                        ...
                        .unwrap_or(-1);
                    let shown = if sel >= 0 {
                        options.get(sel as usize).cloned().unwrap_or_default()
                    } else {
                        "— select —".to_string()
                    };
                    push(
                ...
                .unwrap_or(false)
        {
            push(
                &mut nodes,
                &mut y,
                "__banner__".to_string(),
                32,
                None,
                json!({ "kind": "banner", "label": "\u{2713} lead submitted" }),
            );
        }
...
pub fn form_reducer(form: &Form) -> IntentReducer {
    ...
    let required_text: Vec<String> = form
        .body
        .iter()
        ...
        .collect();
    let selects: Vec<(String, usize, bool)> = form
        .body
        .iter()
        .filter_map(|c| match c {
            Component::Select {
                ...
                required,
                ..
            } => Some((id.clone(), options.len(), *required)),
            _ => None,
        })
        .collect();
    ...
    Box::new(move |intent, world| {
        ...
        match intent.action.as_str() {
            ...
            "type" => {
                ...
                if let Some((id, v)) = world.iter().find(|(_, v)| {
                    v.get("kind").and_then(|k| k.as_str()) == Some("text")
                        && v.get("focused").and_then(|f| f.as_bool()) == Some(true)
                }) {
            ...
            "cycle" => {
                ...
                let Some((_, len, _)) = selects.iter().find(|(id, _, _)| id == target) else {
                    return vec![];
                };
                let Some(mut v) = find(target) else {
                    return vec![];
            ...
            "submit" => {
                ...
                vec![(
                    "__form__".to_string(),
                    json!({ "submitted": true, "ok": ok, "errors": errors }),
                )]
            }
            _ => vec![],
        }
