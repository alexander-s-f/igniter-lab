...
fn step_zero_renders_no_overlay() {
    ...
    assert!(!c.render_svg().contains("diff-"), "shell shows no diff markers at step 0");
}
...
fn selecting_a_lead_overlays_added_removed_changed() {
    ...
    assert!(ov.contains("class=\"diff-added\""), "Grace's fields highlighted as added");
    assert!(ov.contains("class=\"diff-removed\""), "Ada's fields highlighted as removed");
    assert!(ov.contains("class=\"diff-changed\""), "the main panel marked changed");
...
fn overlay_is_mapped_into_the_viewer_coordinate_space() {
    ...
    assert!(ov.contains("x=\"204\""), "added field mapped into viewer x, not shell space");
    ...
    assert!(!ov.contains("y=\"22\""), "overlay is not drawn in the console chrome");
}
...
fn scrubbing_updates_the_overlay_deterministically() {
    ...
    c.select_step(1);
    assert_eq!(c.diff_overlay(), live, "re-selecting a step reproduces the same overlay (deterministic)");
}
...
fn box_node(id: &str, sx: i64, sy: i64, data: serde_json::Value) -> ProjectedNode {
    ProjectedNode { id: id.into(), x: 0.0, y: 0.0, z: 0.0, sx, sy, intent: None, sw: Some(50), sh: Some(20), data }
}
fn point_node(id: &str, data: serde_json::Value) -> ProjectedNode {
    ProjectedNode { id: id.into(), x: 0.0, y: 0.0, z: 0.0, sx: 10, sy: 10, intent: None, sw: None, sh: None, data }
}
...
fn all_four_change_kinds_are_distinguishable() {
    ...
    assert!(ov.contains("<line class=\"diff-moved\""), "moved shows a displacement line");
}
...
fn point_node_change_keeps_textual_diff_but_no_overlay_geometry() {
    ...
    assert_eq!(diff_overlay_svg(&prev, &cur), "", "point-node change emits no overlay rect");
}
