...
fn compile_app() -> PathBuf {
    ...
    let o = Command::new(igc_bin())
        .args(["compile", "--project-root", APP, "--entry", "App.Main", "--out", out.to_str().unwrap()])
        .output()
        .expect("run igc");
...
fn run(app: &Path, entry: &str, inputs: &str) -> String {
    ...
    let o = Command::new(vm_bin())
        .args(["run", "--contract", app.to_str().unwrap(), "--entry", entry, "--inputs", inf.to_str().unwrap()])
        .output()
...
fn vec3_ops_exact_values_through_package_and_vm() {
    ...
    assert!(
        r.contains(r#""x": Float(5.0)"#) && r.contains(r#""y": Float(7.0)"#) && r.contains(r#""z": Float(9.0)"#),
        "add = (5,7,9): {r}"
    );
    // sub = (-3,-3,-3)
    let r = run(&app, "SubProof", ab);
    assert!(r.contains(r#""x": Float(-3.0)"#) && r.contains(r#""z": Float(-3.0)"#), "sub = (-3,-3,-3): {r}");
    // scale (1,2,3)*2 = (2,4,6)
    let r = run(&app, "ScaleProof", r#"{"v":{"x":1.0,"y":2.0,"z":3.0},"k":2.0}"#);
    assert!(
        r.contains(r#""x": Float(2.0)"#) && r.contains(r#""y": Float(4.0)"#) && r.contains(r#""z": Float(6.0)"#),
        "scale = (2,4,6): {r}"
    );
    // dot (1,2,3)·(4,5,6) = 32
    assert!(run(&app, "DotProof", ab).contains("Float(32.0)"), "dot = 32");
    // cross (1,0,0)×(0,1,0) = (0,0,1) — right-handed
    let c = run(&app, "CrossProof", r#"{"a":{"x":1.0,"y":0.0,"z":0.0},"b":{"x":0.0,"y":1.0,"z":0.0}}"#);
    assert!(
        c.contains(r#""x": Float(0.0)"#) && c.contains(r#""y": Float(0.0)"#) && c.contains(r#""z": Float(1.0)"#),
        "cross = (0,0,1): {c}"
    );
    // norm((3,4,0)) = 5.0 via fast sqrt
    assert!(run(&app, "NormProof", r#"{"v":{"x":3.0,"y":4.0,"z":0.0}}"#).contains("Float(5.0)"), "norm = 5.0");
    // det_norm((3,4,0)) = 5.0 via det_sqrt
    assert!(run(&app, "DetNormProof", r#"{"v":{"x":3.0,"y":4.0,"z":0.0}}"#).contains("Float(5.0)"), "det_norm = 5.0");
    // distance((0,0,0),(3,4,0)) = 5.0
    assert!(
        run(&app, "DistanceProof", r#"{"a":{"x":0.0,"y":0.0,"z":0.0},"b":{"x":3.0,"y":4.0,"z":0.0}}"#).contains("Float(5.0)"),
        "distance = 5.0"
    );
}
