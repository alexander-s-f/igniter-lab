...
async fn matches_anchored_and_unanchored() {
    assert_eq!(
        run(call(
            "stdlib.regexp.matches",
            vec![lit_s("/todos/42"), lit_s("^/todos/([0-9]+)$")]
        ))
        .await,
        Ok(Value::Bool(true))
    );
    assert_eq!(
        run(call(
            "stdlib.regexp.matches",
            vec![lit_s("/todos/x"), lit_s("^/todos/([0-9]+)$")]
        ))
        .await,
        Ok(Value::Bool(false))
    );
...
async fn capture_index_semantics() {
    ...
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/todos/42"), lit_s(p), lit_i(0)]
        ))
        .await,
        Ok(s("/todos/42"))
    );
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/todos/42"), lit_s(p), lit_i(1)]
        ))
        .await,
        Ok(s("42"))
    );
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/todos/42"), lit_s(p), lit_i(2)]
        ))
        .await,
        Ok(Value::Nil)
    ); // out-of-range → None
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/todos/x"), lit_s(p), lit_i(1)]
        ))
        .await,
        Ok(Value::Nil)
    ); // no match → None
    ...
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("ab"), lit_s("^a(x)?b$"), lit_i(1)]
        ))
        .await,
        Ok(Value::Nil)
    );
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("axb"), lit_s("^a(x)?b$"), lit_i(1)]
        ))
        .await,
        Ok(s("x"))
    );
...
async fn route_pressure() {
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/todos/42"), lit_s("^/todos/([0-9]+)$"), lit_i(1)]
        ))
        .await,
        Ok(s("42"))
    );
    ...
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/accounts/7/todos/42"), lit_s(nested), lit_i(1)]
        ))
        .await,
        Ok(s("7"))
    );
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/accounts/7/todos/42"), lit_s(nested), lit_i(2)]
        ))
        .await,
        Ok(s("42"))
    );
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![
                lit_s("/webhooks/callrail"),
                lit_s("^/webhooks/([a-z0-9_-]+)$"),
                lit_i(1)
            ]
        ))
        .await,
    ...
    assert_eq!(
        run(call(
            "stdlib.regexp.matches",
            vec![lit_s("/nope"), lit_s("^/webhooks/([a-z0-9_-]+)$")]
        ))
        .await,
        Ok(Value::Bool(false))
    );
...
async fn bare_names_also_dispatch() {
    assert_eq!(
        run(call("matches", vec![lit_s("/x/1"), lit_s("^/x/([0-9]+)$")])).await,
        Ok(Value::Bool(true))
    );
    assert_eq!(
        run(call(
            "capture",
            vec![lit_s("/x/1"), lit_s("^/x/([0-9]+)$"), lit_i(1)]
        ))
        .await,
        Ok(s("1"))
    );
...
async fn unicode_capture_substring() {
    assert_eq!(
        run(call(
            "stdlib.regexp.capture",
            vec![lit_s("/todos/київ"), lit_s("^/todos/(.+)$"), lit_i(1)]
        ))
        .await,
        Ok(s("київ"))
    );
...
async fn invalid_pattern_is_runtime_error() {
    assert!(
        run(call("stdlib.regexp.matches", vec![lit_s("x"), lit_s("(")]))
            .await
            .is_err()
    );
    assert!(run(call(
        "stdlib.regexp.capture",
        vec![lit_s("x"), lit_s("["), lit_i(1)]
    ))
    .await
    .is_err());
    ...
    assert!(run(call(
        "stdlib.regexp.matches",
        vec![lit_s("foobar"), lit_s("foo(?=bar)")]
    ))
    .await
    .is_err());
    assert!(run(call(
        "stdlib.regexp.matches",
        vec![lit_s("aa"), lit_s(r"(a)\1")]
    ))
    .await
    .is_err());
}
