...
async fn sin_cos_sqrt_known_values() {
    ...
    assert!(
        (call1("sin", std::f64::consts::FRAC_PI_2).await - 1.0).abs() < TOL,
        "sin(pi/2)=1"
    );
    assert!((call1("cos", 0.0).await - 1.0).abs() < TOL, "cos(0)=1");
    assert!((call1("sqrt", 4.0).await - 2.0).abs() < TOL, "sqrt(4)=2");
    assert!(
        (call1("sqrt", 2.0).await - std::f64::consts::SQRT_2).abs() < TOL,
        "sqrt(2)"
    );
}
...
async fn pi_is_a_zero_arg_constant() {
    let v = run(vec![call("pi", 0), Instruction::new(OP_RET, vec![])])
        .await
        .unwrap();
    match v {
...
async fn non_float_argument_errors() {
    ...
    assert!(
        int_arg.is_err(),
        "sqrt(Integer) must error (no implicit coercion)"
    );
}
