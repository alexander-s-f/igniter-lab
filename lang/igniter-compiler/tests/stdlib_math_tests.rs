...
fn valid_transcendental_calls_compile_clean() {
    ...
    assert_eq!(
        status, "ok",
        "valid math program must compile; errors: {errors:?}"
    );
...
fn deterministic_surface_compiles_and_typechecks() {
    ...
    assert!(
        has_rule(&e_arity, "OOF-MATH1"),
        "det_sin/2 → OOF-MATH1: {e_arity:?}"
    );
    ...
    assert!(
        has_rule(&e_type, "OOF-MATH2"),
        "det_sqrt(Integer) → OOF-MATH2: {e_type:?}"
    );
}
...
fn kuramoto_order_parameter_slice_compiles_clean() {
    ...
    assert_eq!(
        status, "ok",
        "Kuramoto slice must compile on Tier-1 math; errors: {errors:?}"
    );
    assert!(errors.is_empty(), "no error diagnostics: {errors:?}");
    assert!(
        !src.contains("5040"),
        "fixture must NOT contain a hand-rolled Taylor series"
    );
}
...
fn numeric_basics_typecheck() {
    ...
    assert!(
        has_rule(&e_num, "OOF-MATH2"),
        "abs(String) → OOF-MATH2: {e_num:?}"
    );
    ...
    assert!(
        has_rule(&e_mixed, "OOF-MATH3"),
        "min(Float, Integer) → OOF-MATH3: {e_mixed:?}"
    );
}
