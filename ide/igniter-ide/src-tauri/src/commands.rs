...
pub fn checkpoint_machine(state: State<MachineState>, path: String) -> Result<(), String> {
    let machine = state.0.lock();
    futures::executor::block_on(machine.checkpoint(std::path::Path::new(&path)))
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn resume_machine(state: State<MachineState>, path: String) -> Result<String, String> {
    let resumed = futures::executor::block_on(IgniterMachine::resume(
...
pub fn resolve_workspace_path(sub_path: &str) -> std::path::PathBuf {
    ...
    path.pop(); // Go up from igniter-ide to ide
    path.pop(); // Go up from ide to igniter-lab
    path.join(sub_path)
}
...
fn write_telemetry_receipt(receipt: &CommandReceipt) -> std::io::Result<()> {
    let out_path = resolve_workspace_path("frame-ui/igniter-view-engine/out");
...
fn write_vm_trace_fixture(receipt: &VmTraceReceipt) -> std::io::Result<()> {
    let out_path = resolve_workspace_path("frame-ui/igniter-view-engine/fixtures");
    let _ = std::fs::create_dir_all(&out_path);
...
pub fn run_session_telemetry_dispatch_inner<R: tauri::Runtime>(
    ...
    let output = std::process::Command::new("ruby")
        ...
        .current_dir(resolve_workspace_path("frame-ui/igniter-view-engine"))
        .output()
