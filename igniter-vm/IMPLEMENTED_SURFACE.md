...
| 4 | **closures — lambdas don't capture enclosing scope.** `map(xs, e -> call_contract(..., new_tick, ...))` where `new_tick`/`closes` are enclosing `compute` bindings → `Symbol not found in env`. VM stores compute results in `registers: HashMap<i64,Value>` (by index, no name map); HOF handlers pass `{param: item}` only. Needs a real **closures feature** (runtime name→value capture OR compile-time closure conversion). Card: `LAB-VM-HOF-CLOSURE-CAPTURE-P1` (design pending). | sim_framework, trade_robot (+likely others) | feature, not a patch |
| ~~4b~~ | ✅ **NOT A BUG** — trade_robot `c1` was a harness artifact (auto-entry ran inner `RunBacktest` with empty inputs; `c1` is its input). Correct entry `RunTradingBot` → hits the closure gap (#4). Spike: `LAB-VM-TRADE-ROBOT-LOAD-REF-P1`. | trade_robot | folds into #4 |
