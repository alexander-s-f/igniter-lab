...
|---|---|---|---|---|---|
| unary op | ✅ implemented | `compiler.rs:590-605` | `OP_NOT 0x1C` / `OP_NEG 0x21` | `igniter-compiler/proofs/verify/verify_unary_operators_p4.rb` | `!`, `-` |
| array literal | ✅ implemented | `compiler.rs:608-613` | `OP_PUSH_ARRAY 0x1E` | `igniter-compiler/proofs/verify/verify_compiler.rb` (suite) | element_count arg |
| record literal | ✅ implemented | `compiler.rs:616-628` | `OP_PUSH_RECORD 0x1F` | `igniter-compiler/proofs/verify/verify_compiler.rb` (suite) | deterministic key sort |
| concat | ✅ implemented | `compiler.rs:631-636` (+ `:302`) | `OP_CONCAT 0x1D` | `igniter-compiler/proofs/verify/verify_str_core.rb` | also via binary `++` |
| let binding | ✅ implemented | `compiler.rs:639-661` | `OP_STORE_REG 0x03` / `OP_LOAD_REG 0x04` | `igniter-compiler/proofs/verify/verify_compiler.rb` (suite) | register alloc + body |
| lambda / fn | ✅ implemented | `compiler.rs:664-667` | `OP_PUSH_LIT 0x01` | `igniter-compiler/proofs/verify/verify_hof_lambda_error_propagation_p2.rb` | serialized; consumed by HOF ops |
...
|---|---|---|
| Collection `zip` signature | Implemented in stdlib declarations | `igniter-stdlib/stdlib/collections.ig` declares `zip(a,b) -> Collection[Pair[A,B]]` with shorter-length truncation semantics; proof marker `LAB-STDLIB-COLLECTION-ZIP-PROOF-P2`. |
| HOF runtime parity for math calls | Implemented for covered shapes | `igniter-vm/tests/stdlib_math_hof_tests.rs` covers `sin`, `cos`, `sqrt`, `pi`, `det_sin`, `det_sqrt` inside `fold`/`map` lambda bodies through `eval_ast`, not only bytecode `OP_CALL`. |
...
| Decimal money arithmetic/comparison | Implemented in stdlib + VM call paths | `igniter-stdlib/src/decimal.rs` uses bounded scale, checked i128 intermediates, exact-only division, and scale-normalized comparison; VM equality/order routes through Decimal comparison rather than f64. Proof docs: `lab-stdlib-decimal-money-contract-readiness-p1.md`, `lab-stdlib-decimal-money-safe-p2.md`. |
| Exact text conversion `to_text` | Implemented for `Integer` and `Decimal`; `Float` is explicit-only via `float_to_text` | `igniter-compiler/src/typechecker/stdlib_calls.rs` accepts `to_text` / `stdlib.string.to_text` for `Integer | Decimal -> String` and rejects `Float`; `igniter-vm/src/vm.rs` routes both bytecode `OP_CALL` and `eval_ast` through the same VM arm. `Integer` renders exact base-10; `Decimal { value, scale }` renders canonical fixed decimal text with exactly `scale` fractional digits, no locale/grouping/currency/rounding. Proof docs: `lab-lang-number-to-text-p1-v0.md`, `lab-lang-decimal-to-text-p2-v0.md`; tests: `stdlib_to_text_tests` in compiler + VM. |
...
| Float `Vec3` local package | Implemented as package proof | `igniter-compiler/tests/fixtures/project_mode/linalg_vec3` plus `igniter-vm/tests/linalg_vec3_tests.rs`; pure `.ig`, package resolver path, no VM builtins. |
| Float `Mat3` local package | Implemented as package proof | `igniter-compiler/tests/fixtures/project_mode/linalg_mat3` plus `igniter-vm/tests/linalg_mat3_tests.rs`; imports Vec3 and Mat3 through project mode and runs exact VM value checks. |
...
surface truth, start with `igniter-compiler/src/main.rs` (`igc package graph|pack|verify|admit`),
`igniter-compiler/src/project.rs` (`admit_archive`, archive verification, local package graph),
`igniter-compiler/tests/package_lockfile_cli_tests.rs`, and
`igniter-compiler/tests/package_workspace_tests.rs`.
