...
async fn prod(n: usize, dp: DuplicatePolicy) -> (CoordinationHub, IngressRouter) {
    ...
    h.accept_recipe(&cpass("dev", "coordination", &["accept_recipe"]), "svc", recipe(&digest, n as u32, dp))
        .await
        .unwrap();
    h.grant(&cpass("dev", "coordination", &["grant_access"]), "svc", "vendor:acme", PoolRight::ActivateCapsule)
        .await
        .unwrap();
...
async fn server_post(addr: std::net::SocketAddr, path: &str, key: &str, corr: &str, base: i64) -> (u16, Value) {
...
fn server_invoke_effect_replay_no_second_effect() {
    rt().block_on(async {
        ...
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let (_s1, (st1, _)) = tokio::join!(serve_once_effect(&listener, &app, &eh), server_post(addr, "/effect/record", "E1", "c1", 1000));
        let (_s2, (st2, _)) = tokio::join!(serve_once_effect(&listener, &app, &eh), server_post(addr, "/effect/record", "E1", "c2", 1000));

        assert_eq!(st1, 200);
        assert_eq!(st2, 200);
        assert_eq!(st.exec.attempts(), 1, "server replay performs no second effect");
    });
}
...
fn server_invoke_effect_bounded_fresh_attempts_match_ingress() {
    rt().block_on(async {
        ...
        for i in 0..3 {
            let corr = format!("c{i}");
            let (_s, _b) = tokio::join!(serve_once_effect(&listener, &app, &eh), server_post(addr, "/effect/record", "E1", &corr, 1000));
        }

        assert_eq!(st.exec.applied_count(), 3, "three fresh attempts → three distinct effects");
        // distinct effect idempotency keys = CAP:duplicate_key:attempt (matches direct ingress).
        assert!(st.receipts.read_as_of("__receipts__", "IO.Demo:E1:0", f64::MAX).await.unwrap().is_some());
        assert!(st.receipts.read_as_of("__receipts__", "IO.Demo:E1:2", f64::MAX).await.unwrap().is_some());
    });
}
...
fn server_path_matches_direct_ingress_normalized() {
    rt().block_on(async {
        ...
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let (_s, (sstatus, sbody)) = tokio::join!(serve_once_effect(&listener, &app, &eh), server_post(addr, "/effect/record", "E1", "c1", 1000));
        ...
        let ingress = IngressRequest { method: "POST".into(), path: "/w".into(), headers, body: json!({ "base": 1000 }) };
        let direct = r2.handle_effect(&h2, &ingress, &c2).await;

        assert_eq!(sstatus, direct.status, "status equal across server-mediated and direct ingress");
        assert_eq!(sbody, direct.body, "body equal across server-mediated and direct ingress");
...
fn server_routing_still_lives_in_app() {
    rt().block_on(async {
        ...
        impl ServerApp for OtherApp {
            fn call(&self, req: ServerRequest) -> ServerDecision {
                match (req.method.as_str(), req.path.as_str()) {
                    ...
                    _ => ServerDecision::Respond { response: ServerResponse::json(404, json!({ "error": "other-app: no route" })) },
                }
            }
        }
        ...
        let (_s, (status1, body1)) = tokio::join!(serve_once_effect(&l1, &app, &eh), server_post(a1, "/effect/record", "E1", "c1", 1000));
        assert_eq!(status1, 404);
        assert_eq!(body1["error"], json!("other-app: no route"));
        assert_eq!(st.exec.attempts(), 0, "no effect: this app does not route there");

        // the path THIS app routes to an effect commits through the same host/adapter.
        let l2 = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let a2 = l2.local_addr().unwrap();
        let (_s2, (status2, body2)) = tokio::join!(serve_once_effect(&l2, &app, &eh), server_post(a2, "/different-effect", "E2", "c2", 1000));
...
fn server_concurrent_same_key_exactly_one_effect() {
    rt().block_on(async {
        ...
        req.headers.insert("authorization".into(), "Bearer vtok".into());
        req.headers.insert("x-vendor-event-id".into(), "SAME".into());
        ...
        for resp in [&r0, &r1, &r2, &r3] {
            assert_eq!(resp.status, 200, "all four collapse to the same committed result");
        }
        assert_eq!(st.exec.attempts(), 1, "four concurrent same-key requests perform exactly one effect");
        assert!(st.receipts.read_as_of("__receipts__", "IO.Demo:SAME:0", f64::MAX).await.unwrap().is_some());
    });
}
