# VM RUN-OK Recheck P1 — 2026-06-15

**Card:** `LAB-VM-RUN-OK-RECHECK-P1`  
**Authority:** evidence-only runtime recheck; no source files modified  
**Proof runner:** `igniter-view-engine/proofs/verify_lab_vm_run_ok_recheck_p1.rb`

## Result

The active registry-backed runtime fleet is **25 apps**. Fresh compile + VM run
evidence gives **RUN-OK 23/25**, up from checkpoint **RUN-OK 18/25**.

The five new runtime-green apps since the checkpoint are:
...
| `spreadsheet` | `RunWorkbookDemo` | RUN-NOT-OK | `sha256:412ec0f347efc63f726dedefc95ae492b4b75e0ae3a3c4966277c050d0533644` |
...
| app | owner class | evidence | next route |
|---|---|---|---|
| `spreadsheet` | real runtime bug | Rust compile ok, `RunWorkbookDemo` selected, VM traps on `Unsupported operator: eval_expr` | VM app-local function-call / `eval_expr` support |
| `rule_engine` | governance-gated | compile remains fail-closed on `Unknown.action` + `expected RuleDecision, got Unknown` | `LAB-DYNAMIC-CONTRACT-DISPATCH` / ledger D-001 |

No app remains in the old needs-inputs/demo-entry bucket after this recheck. `rule_engine`
remains deliberately governance-gated; this card does not relax dynamic dispatch.
...
- Compile-clean and runtime-run-clean are tracked separately.
