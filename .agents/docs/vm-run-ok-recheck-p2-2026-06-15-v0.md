# VM RUN-OK Recheck P2 — 2026-06-15

**Card:** `LAB-VM-RUN-OK-RECHECK-P2`  
**Authority:** evidence-only runtime recheck; no source files modified  
**Proof runner:** `igniter-view-engine/proofs/verify_lab_vm_run_ok_recheck_p2.rb`

## Result

The active registry-backed runtime fleet remains **25 apps**. Fresh compile + VM run
evidence gives **RUN-OK 23/25**.

Delta vs P1: **0**. `LAB-VM-EVALAST-EVAL-EXPR-P1` closed as a routed spike, not an
implementation, so no runtime count movement is expected.
...
|---|---|---|---|---|
| `spreadsheet` | RUN-NOT-OK | function SIR/runtime substrate | compile ok; VM still traps on `Unsupported operator: eval_expr`; SIR has no `functions` table | `LAB-FUNCTION-SIR-RUNTIME-P1` |
| `rule_engine` | COMPILE-NOT-OK | governance-gated | compile remains fail-closed on `Unknown.action` + `expected RuleDecision, got Unknown` | `LAB-DYNAMIC-CONTRACT-DISPATCH` / ledger D-001 |
...
- Compile status and runtime status are separate columns.
