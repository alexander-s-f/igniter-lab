...
**Last updated:** 2026-06-10 (PROP-047-P2: Failure Outcome Naming Convention partial_success amendment — PROPOSAL AMENDMENT / GOVERNANCE UPDATE / NO IMPLEMENTATION; CLOSED; stable cross-domain terms now 6: denied/unknown_external_state/timed_out/system_error/query_error/partial_success; LAB-FAILURE-TAXONOMY-P4 closed evidence gap with batch/job processing + multi-upstream network proof, 54/54 PASS; partial_success definition: bounded operation produced both confirmed successes and confirmed failures/non-completions with item/sub-operation evidence; recovery: continue/reconcile/record mixed receipt and retry or repair failed subset; forbidden collapses updated: partial_success != unknown_external_state/system_error/failed-or-total-failure/ok-or-total-success; partial_success requires observed item/sub-operation evidence, not ambiguous timeout or missing ack; DEFERRED remains: compensation stable term, retryable/non-retryable stable term; CLOSED remains: parser/compiler/VM/runtime changes, OOF codes, global FailureKind enum, Outcome[T,E], serialization ABI, public/stable API; artifacts: igniter-lang/.agents/work/proposals/PROP-047-failure-outcome-naming-convention-and-recovery-axis-guidance-v0.md + card PROP-047-P2.md) | (LAB-IGV-TAILMIX-P1: Tailmix-on-Igniter view-runtime DESIGN BOUNDARY — RESEARCH / DESIGN / NO IMPLEMENTATION; fixes the view+interaction architecture for a Tauri IDE-for-Igniter-written-in-Igniter (fractal dogfooding, not a bootstrap paradox), CRUD/forms + bounded interactivity, STATIC build-time component set; 10 locked decisions: D1 NO client-side VM (Tauri backend runs canonical native Rust igniter-vm, webview↔VM over IPC; JS-VM/WASM/SIR→JS-codegen rejected for this target — zero new parity surface), D2 NO Ruby runtime (reimplement the IDEA of Tailmix on Igniter, not the gem), D3 4-parts-only (.igv DSL → definition-JSON compiler → ONE tiny generic JS instruction-interpreter → dispatch escalation seam; interpreter must NOT become a VM), D4 three tiers by lifecycle (:local→Tailmix definitions/client-JS, :session/:durable→Igniter contracts/Rust-VM-IPC, raw-text→host widget) with DISJOINT ownership to avoid tri-parity, D5 single seam dispatch(event)→host→contract, D6 type-vs-instance = G1 in UI form (definition per-TYPE content-addressed by hash, render emits per-INSTANCE binding only), D7 static set → ONE build-time definition bundle loaded once + API render→{html,def_refs} (definitions never inlined per render → kills many-component redundancy: N instances = 1 definition + N tiny bindings), D8 CLOSED/frozen :local instruction vocabulary (toggle/set/class/attr/aria/show/hide/match/dispatch) fail-closed beyond→dispatch, D9 definitions = inert content-addressed inspectable artifacts (node_id/srcmap-debuggable, not authority, no capability), D10 bounded parity Igniter-render↔JS-interp via diff-oracle; security note: client :local is honesty/structure NOT enforcement, real authority backend-side; preserves view-engine 'no contract execution in view runtime' boundary; CLOSED client-VM/Ruby/adopted-grammar/contract-exec-in-view/vocab→computation/compiler-parser-VM-change/client-capability-authority/canon-stable-public-framework-API; ZERO impl files touched; next = LAB-IGV-TAILMIX-P2 proof-local (FileTreeRow .igv→content-addressed definition JSON; render→{html,def_refs} instance-binding-only + N→1 dedup; reference-applier oracle diff-tested vs igniter_view_runtime.js over (definition,state,event) triples; fail-closed unknown-op; dispatch→host-event; ~40-60 checks, no Tauri, no toolchain change), then IDE drives app-state G1/G4/G3 → LAB-APP-STATE-P3 / LAB-APP-ASSEMBLY-P1; artifacts: lab-docs/view/lab-igv-tailmix-on-igniter-view-runtime-design-boundary-v0.md + .agents/work/cards/view/LAB-IGV-TAILMIX-P1.md) | (LAB-FAILURE-TAXONOMY-P4: Partial Success Cross-Domain Pressure Proof — LAB PROOF / GOVERNANCE EVIDENCE; 54/54 PASS; 9 sections; closes partial_success evidence gap (axis 6 of 10) from LAB-FAILURE-TAXONOMY-P1 HOLD; domain: batch job processing (non-reconciliation); secondary cross-domain confirmation: multi-upstream HTTP network; core claim: partial_success independently meaningful when K items succeed and N-K fail, all outcomes OBSERVED; 8 required proof questions answered affirmatively; all 6 outcome kinds mutually distinct: ok/partial_success/failed = per-item evidence; denied/system_error/unknown_external_state = no per-item evidence; system_error vs partial: system_error has total_count=0 (infra failure, nothing ran), partial has typed counts for every item; unknown_external_state vs partial: unknown = batch dispatched, no ack (Covenant P15); partial = all outcomes observed; EvidenceInspector proves succeeded_count+failed_count==total_count for partial_success; distinct recovery action: retry_failed_items (vs retry_batch/consume/fix_policy/investigate/reconcile); 11 contracts (7 scenarios + BatchOutcomeClassifier + BatchActionRouter + MultiUpstreamClassifier + EvidenceInspector); TAXP4-COMPILE 5/5|TAXP4-SCENARIO 7/7|TAXP4-CLASSIFY 6/6|TAXP4-PARTIAL 6/6|TAXP4-BOUNDARY 8/8|TAXP4-MULTIUP 6/6|TAXP4-ACTION 6/6|TAXP4-EVIDENCE 5/5|TAXP4-CLOSED 5/5; GOVERNANCE: PROMOTE partial_success into PROP-047 stable terms; conditions: (1) requires observed per-item evidence — if outcomes unknown use unknown_external_state; (2) at least one success AND one failure (0 success → failed; N success → ok); (3) domain vocabularies may rename arms — taxonomy-level kind stays partial_success; CLOSED: no global Outcome[T,E]/FailureKind enum/OOF codes/compiler+parser+VM/canon change; artifacts: fixtures/failure_taxonomy/batch_partial_success.ig + proofs/verify_lab_failure_taxonomy_p4.rb + lab-docs/governance/lab-failure-taxonomy-partial-success-cross-domain-proof-v0.md + card LAB-FAILURE-TAXONOMY-P4.md) | (LAB-FAILURE-TAXONOMY-P3: Taxonomy Proposal Readiness Decision — GOVERNANCE / READINESS DECISION / NO IMPLEMENTATION; DECISION = A — OPEN NARROW NAMING-CONVENTION PROP NOW; P1 HOLD condition satisfied by P2; 9/10 axes cross-domain; partial_success (axis 6) deferred explicitly; PROP SCOPE AUTHORIZED: title "Failure Outcome Naming Convention and Recovery-Axis Guidance"; 5 stable cross-domain terms (denied/unknown_external_state/timed_out/system_error/query_error); 2 patterns (denial-as-data/timeout-is-not-failure); 7 forbidden-collapse rules; KDR/variant usage boundary rule; observation type guidance (real/model/human; No-Upward-Coercion); all 10 axes documented (axis 6 deferred); recovery guidance per axis; DEFERRED IN PROP: partial stable term (single-domain; defer to P4); compensation stable term (pattern only); retryable/non-retryable term (pattern only; arm names domain-local); PERMANENTLY CLOSED IN PROP: global FailureKind enum / Outcome[T,E] / OOF codes / compiler+parser+VM / runtime behavior / serialization ABI; WHY NOT B: P1 HOLD condition ("one non-reconciliation proof for timeout + unknown_external_state") was met by P2; partial_success can be deferred explicitly — stronger than silence; further waiting delays value from 9 proven axes; WHY NOT C: evidence not fragmented; naming-convention PROP has no runtime consequences; risk low; EXPLICIT ANSWERS: 9/10 enough=YES / partial blocks=NO (deferred) / exclude partial=YES / Outcome[T,E] closed=YES / FailureKind enum closed=YES / Path B relevant=YES (KDR/variant boundary rule) / next card=PROP authoring card (next available PROP number); NEXT: (1) proposal authoring card for naming-convention PROP (next available PROP number); (2) LAB-FAILURE-TAXONOMY-P4 (partial_success cross-domain proof, parallel, not blocking); deliverables: lab-docs/governance/lab-failure-taxonomy-proposal-readiness-decision-v0.md + card LAB-FAILURE-TAXONOMY-P3.md + portfolio update) | (LAB-EXECUTE-QUERY-P3: Unified mocked query execution receipt — LAB PROOF / INTEGRATED QUERY PIPELINE / NO DB; 68/68 PASS; 10 sections; proves complete v0 pipeline: G1→G2→G3→G4→G5→filter→multi-order→limit→projection→receipt; QueryPlanUnified with Collection[FilterPredicate]+Collection[OrderBy]+Projection (new type; no mutation of existing QueryPlan/QueryPlanMultiOrder/QueryPlanProjection); projection final step after filter+multi-order+limit; row count invariant; G4 clamp NON-denial (cap_granted:true after clamp); G5 include_all→query_error NOT denied; query_error≠denied throughout pipeline; receipt mirrors result_kind+rows_returned after full pipeline (after projection); 8 pure contracts (all CORE; no effect; no capability); all 8 contracts VM-executed; LAB-TC-ARRAY-P2 8th confirmation (BuildUnifiedPlan.filters:Collection[FilterPredicate] in Rust SIR); B9 TypeChecker nested-record-literal boundary documented (projection passed as input; not fixed here); G1/G2/G3 short-circuit before filter/order/limit/projection; UnifiedQuerySim PROOF-LOCAL ONLY; no SQL/DB/ORM/optimizer/joins/writes/StorageCapability authority; EXECQ3-COMPILE 5/5|EXECQ3-SHAPE 8/8|EXECQ3-GATES 6/6|EXECQ3-PIPELINE 7/7|EXECQ3-PROJECTION 7/7|EXECQ3-RECEIPT 6/6|EXECQ3-ERROR 8/8|EXECQ3-VM 8/8|EXECQ3-CLOSED 8/8|EXECQ3-GAP 5/5; fixtures: execute_query_unified.ig; proof runner: verify_lab_execute_query_p3.rb; lab doc: lab-execute-query-unified-filter-multiorder-projection-receipt-v0.md; card: LAB-EXECUTE-QUERY-P3.md; CLOSED) | (LAB-IDE-TRACE-VIEW-P1: Static source trace viewer / execution narrative — LAB PROOF / STATIC TRACE VIEW / NO STEPPER; 50/50 PASS; 10 sections; proof-runner-only renderer (zero VM source changes); render_source_trace_view() reads source_trace.json + original .ig source lines → source_trace_view.md; emits "# Trace: <contract>" header + Status/Inputs digest/Result digest metadata + "## Execution Timeline"; node entries: [node] marker + node_id + "**Source:** line N, col M" + "**Snippet:** `<stripped source line>`" + "**Instructions:** count (first 6 mnemonics, ...)" + "**Offsets:** csv list"; non-contiguous offset note appended when offsets != (min..max).to_a: "> **Note:** Non-contiguous offsets — control flow skipped bytecode (branch jump; unexecuted match arm not recorded)"; fires for f3 CheckStatus.label [0,1,2,3,4,5,6,7,8,20], appears after node_id heading (label_pos < note_pos — VIEW-BRANCH); f1 (no branches) has no note; infrastructure entries: [infrastructure] marker + Instructions + Offsets + "*(No source location — VM infrastructure instructions)*" (no **Source:** claim); digests embedded from source_trace (read-through, not recomputed); deterministic: same source_trace → identical markdown (re-render f1 and f3 identical — VIEW-DETERMINISM); vm_trace.json unchanged after render (renderer never invokes VM — VIEW-NONSEMANTIC); VIEW-INPUT 5/5|VIEW-RENDER 6/6|VIEW-SCHEMA 5/5|VIEW-READABLE 7/7|VIEW-BRANCH 5/5|VIEW-INFRA 5/5|VIEW-DRILLDOWN 5/5|VIEW-DETERMINISM 4/4|VIEW-NONSEMANTIC 4/4|VIEW-CLOSED 4/4; P2 60/60 regression green; CLOSED: no stepper/breakpoints/pause/resume/watch/IDE UI/Tauri; VM zero edits; OP_UNSUPPORTED still 0x99; schema_version stays "source-trace-v0" (no debugger-* upgrade); does NOT authorize LAB-IDE-STEP-P1; route: LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 ✅ → LAB-VMTRACE-P1 ✅ → LAB-VMTRACE-P2 ✅ → LAB-IDE-TRACE-VIEW-P1 ✅ → LAB-VMTRACE-P3 (not authorized); next: LAB-VMTRACE-P3 (loop/nested-branch/error coverage) or LAB-IDE-TRACE-VIEW-P2 (static HTML viewer); do NOT authorize LAB-IDE-STEP-P1 until loop/nested-branch traces covered; artifacts = proofs/verify_lab_ide_trace_view_p1.rb + lab-docs/ide/lab-static-source-trace-view-readable-execution-narrative-v0.md + card LAB-IDE-TRACE-VIEW-P1.md) | (LAB-FAILURE-TAXONOMY-P2: Network Timeout / Unknown External State Proof — LAB PROOF / FAILURE TAXONOMY EVIDENCE / NO TAXONOMY PROP; 51/51 PASS; 8 sections; provides second-domain evidence for LAB-FAILURE-TAXONOMY-P1 HOLD recommendation; proves in HTTP client / upstream-call domain that post-dispatch timeout routes to unknown_external_state (not system_error); fixture: Lab.FailureTaxonomy.NetworkTimeout (network_timeout_unknown_state.ig); 10 contracts (CapabilityDenied/UpstreamServerError/UpstreamUnavailablePreDispatch/TimeoutBeforeDispatch/DispatchedNoAck/DispatchedLostResponseBody/ConfirmedSuccess/NetworkOutcomeClassifier/ReconciliationDataCheck/MetadataPassthrough); CORE RESULT: dispatch_started=true + ack_received=false → kind:"unknown_external_state" (Covenant P15 structurally enforced via nested if/else); transport_kind="timeout" alone NOT sufficient — pre-dispatch timeout (dispatch_started=false) → "upstream_unavailable" (NOT unknown_external_state); FTAX2-COMPILE 6/6|FTAX2-SHAPE 7/7|FTAX2-CLASSIFY 8/8|FTAX2-NOT-UNKNOWN 6/6|FTAX2-METADATA 7/7|FTAX2-RECONCILE 5/5|FTAX2-CROSSDOMAIN 6/6|FTAX2-CLOSED 7/7; SIR TYPE STRUCTURE: types proved via contract I/O type refs (no type_declarations in SIR) + SIR node analysis (is_dispatched/has_ack node type=Bool; is_blocked binary_op left=transport_kind right=String literal; DispatchedNoAck record literal type_tags for Bool/String fields); CROSSDOMAIN: epistemic domain (storage/lost_confirmation_kdr.ig) compiles + routes to same "unknown_external_state"; no ReconciliationOutcome arm names imported; both domains use KDR, no variant/match; idempotency_key + request_id + Map[String,String] metadata preserved; AXES: 9/10 cross-domain after this proof (timeout + unknown_external_state now cross-domain; partial_success still single-domain); HOLD recommendation extended; CLOSED: no taxonomy PROP/FailureKind enum/Outcome[T,E]/OOF diagnostics/VM change/real network IO (all contracts pure effects:[])/retry scheduler/variant values/canon; artifacts = igniter-view-engine/fixtures/failure_taxonomy/network_timeout_unknown_state.ig + proofs/verify_lab_failure_taxonomy_p2.rb + lab-docs/governance/lab-failure-taxonomy-network-timeout-unknown-state-proof-v0.md + card LAB-FAILURE-TAXONOMY-P2.md) | (LAB-TC-NESTED-RECORD-CONTEXT-P1: Nested record literal context propagation — LAB FIX + PROOF / RUST TYPECHECKER HARDENING / NO QUERY SEMANTICS CHANGE; 42/42 PASS; 7 sections; closes B9 gap from LAB-QUERY-PROJECTION-P1; extends check_record_literal_shape with type_shapes param + RecordLiteral arm for recursive contextual validation; bounded: one call per nesting level, no global inference, no Hindley-Milner, no retroactive mutation; natural projection syntax now compiles: compute plan = { ..., projection: { fields: "...", include_all: false }, ... }; two-level nesting works; fail-closed: missing/extra/wrong-type fields → OOF-TY0; array P1/P2 unaffected; projection workaround still valid; Ruby TC B9 divergence documented (not fixed); no VM/parser/grammar change; CLOSED) | (LAB-OUTCOME-VARIANT-P3: Match Binding / Compute Register Collision Hardening — LAB FIX + PROOF / VM COMPILER HARDENING / NO SEMANTIC CHANGE; 43/43 PASS; 7 sections; fixes VM compiler panic when match arm binding name == enclosing compute node name; ROOT CAUSE: arm cleanup `remove(name)` deleted the outer compute node register allocated in Step 1, causing `unwrap()` panic at compiler.rs:145 on `OP_STORE_REG` emission; FIX: single-block change in compiler.rs match arm section — save outer register before `insert(binding, arm_reg)`, restore outer (or remove) after arm body compiles; lexical scoping: arm binding lifetime = arm body only; NATURAL PATTERN NOW WORKS: `compute attempt = match { ConfirmedFailed { attempt } => attempt }` (previously required rename to `n_attempt`); P3-COMPILE 5/5|P3-COLLISION 8/8|P3-SHADOW 5/5|P3-NESTED 5/5|P3-MULTIARM 6/6|P3-REG 8/8|P3-CLOSED 6/6; ENABLES: LAB-OUTCOME-VARIANT-P2's `ts`/`rid`/`n_attempt` workaround renames were collision workarounds — no longer required; REGRESSIONS: P2 56/56 + P1 11-arm + LAB-VARIANT-VM-P1 fixture 12+13 all green; CLOSED: no new opcodes, no Value::Variant, no grammar/TC/canon/Outcome[T,E]/taxonomy change; only compiler.rs changed; artifacts = igniter-vm/src/compiler.rs + fixtures/outcome_variant/outcome_variant_binding_collision.ig + proofs/verify_lab_outcome_variant_p3.rb + lab-docs/lang/lab-outcome-variant-match-binding-register-collision-hardening-v0.md + card LAB-OUTCOME-VARIANT-P3.md) | (LAB-FAILURE-TAXONOMY-P1: Failure Taxonomy Proposal-Planning — GOVERNANCE / TAXONOMY RESEARCH / NO IMPLEMENTATION; RECOMMENDATION = HOLD — 7/10 axes proven cross-domain; 3 axes (timeout/unknown_state/partial) need one cross-domain proof beyond reconciliation; 9 proof sources surveyed: Ch12 7-kind canon + Covenant P11/P13/P15/P16/P17 + LAB-EPISTEMIC-OUTCOME-P1..P4 + LAB-OUTCOME-VARIANT-P1 + LAB-EXECUTE-QUERY-P1/P2 + LAB-RESULT-ENVELOPE-P1/P2 + network proofs; STABLE NAMES (safe to use now): denied/denial-as-data (7 proofs — strongest invariant, zero contradictions), unknown_external_state (Ch12 + P2 + P4 + P1-variant), timed_out (Ch12 + Covenant P15), system_error (query + validation), query_error (4 proofs, proven distinct from denied); DOMAIN-LOCAL (must stay local): all 11 ReconciliationOutcome arms, rows/empty, found/created/not_found/upstream_unavailable, non_retryable, valid/invalid, E-HTTP-* codes; DO NOT COLLAPSE: denied vs query_error / timed_out vs unknown_external_state / unknown_external_state vs system_error / partial vs unknown / validation_invalid vs capability_denied / retry_budget_exhausted vs upstream_failure / observation types real/model/human; 10 orthogonal axes identified (capability_denial / malformed_plan / external_unavailable / timeout / unknown_external_state / partial_success / validation_invalid / compensation / retryable_vs_not / type_error_vs_domain_outcome); WHY NOT ONE GLOBAL ENUM: flattening 10 axes into one enum erases recovery distinctions (denied ≠ query_error, timeout ≠ unknown, partial ≠ unknown); Outcome[T,E] = CLOSED (3 unsatisfied preconditions: generic type params, sealed variant across domains, cross-domain vocabulary consensus); Path B effect = enables domain-specific variant adoption, does not change HOLD; EXACT NEXT ROUTE: LAB-FAILURE-TAXONOMY-P2 — one targeted cross-domain proof: network domain timeout → unknown_external_state (not system_error); deliverables: lab-docs/governance/lab-failure-taxonomy-proposal-planning-v0.md + .agents/work/cards/governance/LAB-FAILURE-TAXONOMY-P1.md + portfolio update) | (LAB-VMTRACE-P2: Node-level trace aggregation — source trace receipt — LAB PROOF / TRACE RECEIPT / NO STEPPER; 60/60 PASS; 11 sections; Option A: proof-runner-only aggregation (zero VM source changes); aggregate_source_trace() in Ruby: group consecutive vm_trace events with same non-null node_id → one node entry; group consecutive null node_id events → one infrastructure entry; "consecutive" = execution-order adjacent (not IP-space adjacent); key finding: f3 CheckStatus.label node has non-consecutive ip offsets [0,1,2,3,4,5,6,7,8,20] — Active arm JMP at ip=8 jumps over unexecuted Inactive arm (ip=9-19); STORE_REG at ip=20 immediately follows JMP in execution order; this is correct and expected — proved in VMT2-GROUP; consecutive-run grouping == per-node grouping in all 3 fixtures (no same-node non-consecutive events); source_trace.json schema_version="source-trace-v0"; node entry fields: seq/node_id/sir_path/source_span/instruction_count/instruction_offsets/mnemonics/stack_depth_entry/stack_depth_exit; infra entry fields: seq/instruction_count/instruction_offsets/mnemonics (node_id key ABSENT, not null — absence identifies infra); seq globally ordered across nodes+infrastructure (sort all entries by seq to reconstruct timeline); lossless: union of all offsets == vm_trace ip_before set; total count == events_total; deterministic: same vm_trace → identical source_trace; result_digest+inputs_digest passthrough from vm_trace (derived, not re-executed); error vm_trace → nodes=[]+infrastructure=[]; manifest updated with source_trace_ref; P1 66/66 green; SRCMAP-P2 61/61 green; VMT2-INPUT 5/5|VMT2-SCHEMA 8/8|VMT2-GROUP 9/9|VMT2-ORDER 5/5|VMT2-SOURCE 6/6|VMT2-INFRA 5/5|VMT2-DRILLDOWN 5/5|VMT2-DETERMINISM 4/4|VMT2-NONSEMANTIC 4/4|VMT2-ERROR 4/4|VMT2-CLOSED 5/5; CLOSED: no stepper/breakpoints/pause/resume/watch/IDE UI/Tauri/Ruby canon; VM vm.rs+instructions.rs+value.rs zero edits; OP_UNSUPPORTED still 0x99; schema_version≠debugger-*; does NOT authorize LAB-IDE-STEP-P1; route: LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 ✅ → LAB-VMTRACE-P1 ✅ → LAB-VMTRACE-P2 ✅ → LAB-IDE-TRACE-VIEW-P1 ✅ → LAB-VMTRACE-P3 (not authorized); next: LAB-IDE-TRACE-VIEW-P1 (static source trace viewer) or LAB-VMTRACE-P3 (loop/error coverage) before IDE-STEP-P1; artifacts = proofs/verify_lab_vmtrace_p2.rb + lab-docs/ide/lab-vm-node-level-trace-aggregation-source-receipt-v0.md + card LAB-VMTRACE-P2.md) | (LAB-QUERY-PROJECTION-P1: Projection and include_all row-shaping semantics over mocked rows — LAB PROOF / QUERY SEMANTICS / NO DB; 62/62 PASS; 10 sections; proves Projection{fields:String,include_all:Bool} as final pipeline step after filter+multi-order+limit; include_all=true→full row passthrough (identity projection); include_all=false→comma-split field list (split+strip+reject_empty); empty fields→query_error (malformed plan); missing field in row→query_error (fail-closed); duplicate fields→de-duplicate preserving first occurrence (not query_error); projection does not change row count; G5 include_all policy (allow_include_all=false)→query_error (NOT denied); query_error≠denied invariant confirmed; 7 pure contracts (all CORE): BuildIncludeAllPlan+BuildFieldsProjectionPlan+BuildSingleFieldPlan+BuildProjectionRowsResult+BuildProjectionEmptyResult+BuildProjectionQueryErrorResult+ProjectionMetadataReader; LAB-TC-ARRAY-P2 7th confirmation (BuildFieldsProjectionPlan.order_list:Collection[OrderBy]); B9 TypeChecker nested-record-literal boundary documented (workaround: projection as input; gap deferred); ProjectionSim PROOF-LOCAL ONLY; no SQL/DB/ORM/optimizer/joins/writes/StorageCapability; PROJ-COMPILE 5/5|PROJ-SHAPE 7/7|PROJ-INCLUDE-ALL 5/5|PROJ-FIELDS 8/8|PROJ-PIPELINE 6/6|PROJ-POLICY 5/5|PROJ-ERROR 6/6|PROJ-VM 7/7|PROJ-CLOSED 8/8|PROJ-GAP 5/5; fixtures: projection_query.ig; proof runner: verify_lab_query_projection_p1.rb; lab doc: lab-query-projection-and-include-all-over-mocked-rows-v0.md; card: LAB-QUERY-PROJECTION-P1.md; CLOSED) | (LAB-OUTCOME-VARIANT-P2: Rich ReconciliationOutcome Payloads + routing — LAB PROOF / DOMAIN VARIANT PRESSURE / NO TAXONOMY AUTHORITY; 56/56 PASS; 9 sections; extends LAB-OUTCOME-VARIANT-P1 to prove payload binding flow; 5-arm ReconciliationOutcomeRich: ConfirmedSucceededReal/Model(evidence_kind+observed_at:String), ConfirmedFailed(attempt:Integer), StillUnknown(budget_remaining:Integer), ReconciliationError(metadata:Map[String,String]); 12 contracts: 5 build + RouteRich + ExtractEvidenceKind + ExtractObservedAt + ExtractRequestId + ExtractAttempt + ExtractBudget + ExtractTraceId; OUTVAR2-COMPILE 6/6|OUTVAR2-SHAPE 8/8|OUTVAR2-BIND 8/8|OUTVAR2-ROUTE 6/6|OUTVAR2-MAP 5/5|OUTVAR2-BUDGET 5/5|OUTVAR2-NOUC 5/5|OUTVAR2-REG 6/6|OUTVAR2-CLOSED 7/7; KEY FINDINGS: (1) String/Integer payload bindings flow from match arm to output end-to-end; (2) Map[String,String] binding + map_get + or_else executes on bound map in arm body; (3) No-Upward-Coercion holds when arm field sets identical — arm name is discriminant, not payload; (4) Integer 0 preserved (not coerced to nil); VM COMPILER CONSTRAINT: compute node name must ≠ binding name in same arm — cleanup removes shared register → panic at compiler.rs:145; affected: ExtractObservedAt(ts≠observed_at), ExtractRequestId(rid≠request_id), ExtractAttempt(n_attempt≠attempt); NOUC: ConfirmedSucceededModel{evidence_kind:"real"} → "needs_human_review" (never "accept"); CLOSED: no OP_MATCH/OP_PUSH_VARIANT/Value::Variant, no Outcome[T,E], no taxonomy in code, no Ruby canon change, no new VM types; REGRESSIONS: P1 11-arm routing PASS + PROP-044-P9 reserved_fields_valid PASS + variant_match fixture 12 PASS; artifacts = igniter-view-engine/fixtures/outcome_variant/outcome_variant_rich.ig + proofs/verify_lab_outcome_variant_p2.rb + lab-docs/lang/lab-outcome-variant-rich-payload-routing-proof-v0.md + card LAB-OUTCOME-VARIANT-P2.md) | (LAB-VMTRACE-P1: Record-only VM execution trace with source links — LAB PROOF / RUST VM / DEBUGGER SUBSTRATE / NO VM SEMANTICS; 66/66 PASS; 10 sections; trace_collector: Option<Arc<Mutex<Vec<serde_json::Value>>>> on VM struct — None = zero overhead (branch-not-taken, zero allocation, no JSON serialization); Some = per-instruction push after match block; take_trace_events() = std::mem::take idiom (drain without return-type change); OP_RET inline recording before return Ok(val) (only early-return instruction; all Err paths skip recording — correct for error traces); post-match recording for all other instructions; trace_seq monotonic counter incremented in post-match block only; enrichment post-execution in handle_vm_trace (vm.rs has zero source-map knowledge); vm_trace.json schema_version="vm-trace-v0"; event fields: seq/ip_before/opcode/mnemonic/stack_depth_before/stack_depth_after/node_id(nullable)/sir_path(nullable)/source_span(nullable); inputs_digest + result_digest = sha256 truncated 16-char hex; handle_vm_trace pipeline: bytecode-map → offset_lookup → compile → load-inputs → set trace_collector → execute_with_grants → take_trace_events → enrich → write vm_trace.json → update manifest; two stdout lines (bytecode-map summary + trace summary; parse last non-empty line); determinism: re-run → identical events/opcode sequence/result_digest/inputs_digest; non-semantic: result_digest == sha256(JSON.generate(untraced_result))[0,16] verified for all 3 fixtures; coverage: f1 has RET/STORE_REG/MUL/SUB/GET_FIELD; f2/f3 has PUSH_LIT; error case: empty inputs → status=error, events=[] (deterministic); manifest updated with vm_trace_ref; VMTRACE-COMPILE 7/7|VMTRACE-RUN 8/8|VMTRACE-SCHEMA 9/9|VMTRACE-OFFSETS 6/6|VMTRACE-SOURCE 7/7|VMTRACE-DETERMINISM 5/5|VMTRACE-NONSEMANTIC 7/7|VMTRACE-COVERAGE 6/6|VMTRACE-ERROR 5/5|VMTRACE-CLOSED 6/6; new fixtures: inputs_nested.json+inputs_variant.json; CLOSED: no breakpoint/step/pause/resume/watch; Value enum/Instruction struct/opcodes unchanged (OP_UNSUPPORTED still 0x99); no IDE UI/Tauri/public debugger API/Ruby canon/grammar/runtime semantics; route: LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 ✅ → LAB-VMTRACE-P1 ✅ → LAB-VMTRACE-P2 ✅ → LAB-IDE-TRACE-VIEW-P1 ✅ → LAB-VMTRACE-P3 (not authorized); artifacts = igniter-lab/igniter-vm/src/{vm.rs,main.rs} + igniter-view-engine/fixtures/source_map/{inputs_nested.json,inputs_variant.json} + proofs/verify_lab_vmtrace_p1.rb + lab-docs/ide/lab-vm-record-only-execution-trace-with-source-links-v0.md + card LAB-VMTRACE-P1.md) | (LAB-QUERY-MULTI-ORDER-P1: Multi-column order semantics over mocked rows — LAB PROOF / QUERY SEMANTICS / NO DB; 64/64 PASS; 11 sections; extends single-key ordering (LAB-QUERY-ORDER-LIMIT-P1) to QueryPlanMultiOrder.order: Collection[OrderBy]; new type (no mutation of existing QueryPlan); 7 pure contracts (all CORE; no effect); empty Collection[OrderBy]→preserve input order (no-op); empty direction in entry→query_error (multi-order entries are explicit steps, must have direction; differs from P1 where empty=no sort); unknown direction→query_error (NOT denied); missing field→query_error (NOT denied); stable sort: equal keys preserve input order (integer index tiebreaker); priority order: first=primary, second=secondary, third=tertiary; per-column asc/desc via ReverseComparable (all positions uniform type → Array#<=> safe); limit applied AFTER all ordering; gates+filter+multi-order+limit compose correctly in integrated pipeline; Collection[OrderBy] from record-field context (LAB-TC-ARRAY-P2 mechanism, 6th confirmation); all 7 contracts VM-executed; B1: empty list no-op; B2: empty direction→query_error; B3: ReverseComparable per-column desc; B4: index tiebreaker=stable sort; B5: query_error≠denied throughout; B6: 6th P2 confirmation; B7: QueryPlanMultiOrder new type; B8: order-then-limit invariant; MultiOrderSim PROOF-LOCAL ONLY; MORDER-COMPILE 5/5|MORDER-SHAPE 6/6|MORDER-SINGLE 5/5|MORDER-MULTI 8/8|MORDER-STABLE 5/5|MORDER-LIMIT 4/4|MORDER-ERROR 5/5|MORDER-INTEGRATED 6/6|MORDER-VM 7/7|MORDER-CLOSED 8/8|MORDER-GAP 5/5; fixture: igniter-view-engine/fixtures/query_execution/multi_order_query.ig; proof runner: igniter-view-engine/proofs/verify_lab_query_multi_order_p1.rb; design doc: igniter-lab/lab-docs/lang/lab-query-multi-column-order-over-mocked-rows-v0.md; card: igniter-lab/.agents/work/cards/lang/LAB-QUERY-MULTI-ORDER-P1.md) | (LAB-SRCMAP-P2: Bytecode instruction span bridge — parallel node_id_map in Compiler; every VM instruction offset cross-referenced to SIR node_id + source_span; durable bytecode_map.json sidecar — LAB PROOF / RUST VM COMPILER / DEBUGGER SUBSTRATE / NO VM SEMANTICS; 61/61 PASS; 8 sections; additive parallel Vec<Option<String>> (Instruction struct unchanged); current_node_id threaded through compute node loop: OP_STORE_REG tagged with compute node_id, output OP_LOAD_REG + OP_RET get null (infrastructure gap); opcode_mnemonic() maps all 34 opcodes; bytecode-map CLI subcommand placed before banner (clean JSON stdout); sidecar schema_version="bytecode-map-v0"; many-to-1 (N instructions per SIR node) vs sourcemap.json 1:1; cross-ref: node_id→sir_path+source_span from sourcemap.json; take_node_id_map() = std::mem::take idiom; P2-COMPILE 8/8|P2-MAP-SCHEMA 9/9|P2-COVERAGE 8/8|P2-OFFSETS 8/8|P2-SOURCE 8/8|P2-STABILITY 5/5|P2-NONSEMANTIC 8/8|P2-CLOSED 7/7; smoke test: srcmap_basic_contract.ig → 26 instructions, offsets 0-23 tagged, offsets 24+25 null (LOAD_REG+RET); VM result=25 correct; manifest.json updated with bytecode_map_ref; P1 proof 61/61 still green (SRCMAP-CLOSED-2 scope narrowed: excludes compiler.rs/instructions.rs/main.rs which are P2 extension scope, targets vm.rs only); CLOSED: vm.rs execution loop/Instruction struct/Value enum/opcodes/IDE UI/Ruby canon/language grammar/runtime semantics; route: LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 ✅ → LAB-VMTRACE-P1 ✅ → LAB-VMTRACE-P2 ✅ → LAB-IDE-TRACE-VIEW-P1 ✅ → LAB-VMTRACE-P3 (not authorized); artifacts = igniter-lab/igniter-vm/src/{compiler.rs,instructions.rs,main.rs} + igniter-view-engine/fixtures/source_map/inputs_basic.json + proofs/verify_lab_srcmap_p2.rb + proofs/verify_lab_srcmap_p1.rb(scope-fix) + lab-docs/ide/lab-source-map-bytecode-instruction-span-bridge-v0.md + card LAB-SRCMAP-P2.md) | (PROP-044-P9: Reserved Variant Runtime Field Policy — GOVERNANCE + PROOF / TYPECHECKER GUARDRAIL / NO VM CHANGE; 36/36 PASS; closes PROP-044-P8 RISK-006 (reserved-field-collision); DECISION: BROAD RESERVATION — all __* prefixes reserved (not just __arm/__variant); no existing fixture uses __* as field identifier; OOF-KIND6 fires in Rust TC at 3 sites (type decls + variant arm fields + record literals) and Ruby TC at 2 sites (module-level only; record-literal gap documented); "__absent__" string literal value unaffected; Path B lowering not broken (outcome_variant.ig 11-arm regression PASS); RESERVE-SCAN 5/5|RESERVE-TYPE 5/5|RESERVE-RECORD 5/5|RESERVE-VARIANT 5/5|RESERVE-ALLOW 6/6|RESERVE-PATHB 5/5|RESERVE-CLOSED 5/5; OOF-KIND6 surfaces in result["diagnostics"] NOT result["type_errors"]; 5 fixtures: reserved_type_field.ig + reserved_variant_field.ig + reserved_record_literal.ig + reserved_variant_name_field.ig + reserved_fields_valid.ig; proof runner: igniter-view-engine/proofs/verify_prop044_p9_reserved_fields.rb; proposal: igniter-lang/.agents/work/proposals/PROP-044-reserved-runtime-fields-policy-v0.md; card: igniter-lang/.agents/work/cards/lang/PROP-044-P9.md; CLOSED SURFACES: VM instructions.rs/vm.rs/value.rs unchanged, no OP_MATCH, no Value::Variant, __arm/__variant NOT public API; Ruby canon: module-level OOF-KIND6 only; RECOMMENDED NEXT: LAB-OUTCOME-VARIANT-P2 or failure-taxonomy proposal-planning) | (LAB-APP-STATE-P2: Proof-local code-editor app-state model — LAB PROOF / APP-STATE MODEL / NO KEYWORD; tests the LAB-APP-STATE-P1 B⊕E recommendation using EXISTING Igniter concepts ONLY, zero parser/compiler/VM/keyword change; 70/70 PASS, 9 sections; state-VALUES = 11 typed records, TRANSITIONS = 8 pure CORE (snapshot+event)→next, VM-verified incl composite ApplyEdit preserving nested records, outputs are fresh values (no hidden holder); E PATH IN-LANGUAGE = lifecycle :local/:session/:window/:durable/:audit rides `output … lifecycle :x` into SIR output_ports[].lifecycle; DURABLE save/load = effect + IO.StorageCapability / observed read-from-store, NO storage execution, 2nd fixture split (VM rejects unbound-capability igapp load; LAB-STORAGE-CAPABILITY-P2 pattern); holder host-owned, hot/session need NO capability; six P1 terms separate, DocumentState reused for two facts so instance≠type-name; FINDINGS: lifecycle E-path works w/ zero surface; `intent` NOT parseable in lab toolchain (PROP-045 convention-only) → app vocabulary in proof-local sidecar editor_app_state.registry.json (inert metadata); `modifier` only PARTIAL visibility signal (effecting vs pure, NOT public-pure vs helper-pure); no hidden mutation; GAP PACKET — 4 P1 gaps all NON-LANGUAGE but inert-sidecar-expressible, none blocking, each SIR-ABSENT + sidecar-PRESENT: G1 instance-identity, G2 fact↔holder-binding, G3 app-assembly(event→op→fact), G4 public/internal-visibility=SMALLEST; agent answers "what state does this app own?" via (type_tag,lifecycle) from SIR + instance/holder/visibility/wiring from sidecar; DECISION = A metadata-is-enough-for-now (hold proposals, document convention); held smallest-first: G4 → LAB-MODULE-SURFACE-P1, G2 → LAB-APP-STATE-P3, G3 → LAB-APP-ASSEMBLY-P1; CLOSED — no keyword/state{}/visibility-kw/module-instance/service-actor/app-manifest/storage-exec/parser-compiler-VM change (ZERO impl files touched by this card), no canon/stable/framework API; regressions clean LAB-TC-ARRAY-P1 27/27 + LAB-QUERY-P3 44/44; artifacts = fixtures/app_state/{editor_app_state.ig,editor_app_state_durable.ig,editor_app_state.registry.json} + proofs/verify_lab_app_state_p2.rb + lab-docs/lang/lab-code-editor-app-state-model-proof-local-v0.md + card LAB-APP-STATE-P2.md) | (LAB-EXECUTE-QUERY-P2: First complete mocked ExecuteQuery pipeline — LAB PROOF / INTEGRATED MOCKED QUERY EXECUTION / NO DB; 73/73 PASS; 10 sections; integrates StorageCapability gates + filter evaluation + order/limit semantics + QueryExecutionReceipt in one IntegratedQuerySim; 8 pure contracts (all CORE; no effect); G1/G2/G3→denied; G4 clamp ≠ denial (effective_limit=min(plan.limit,cap.row_limit); cap_granted:true after clamp); G5→query_error (NOT denied); gate failures short-circuit before filter/order/limit; filter: eq/neq/contains/prefix; AND-only; unknown op→query_error; missing field→empty; order: asc/desc lexicographic stable sort; unknown direction→query_error; limit applied AFTER filter+order; limit==0→empty; limit<0→query_error; query_error ≠ denied invariant holds throughout (all 73 checks); QueryExecutionReceipt 15-field invariants (cap_checked/cap_granted/denial_gate/effective_limit/row_limit_clamped/rows_returned/result_kind); BuildIntegratedPlan.filters=Collection[FilterPredicate] in Rust SIR (LAB-TC-ARRAY-P2 record-field-context mechanism, 5th confirmation); all 8 contracts VM-executed; B1: gate short-circuit correct model; B2: G4 clamp ≠ denial; B3: G5→query_error not denied; B4: query_error ≠ denied throughout; B5: QueryPlan.limit ≠ StorageCapability row_limit orthogonal; B6: 5th P2 confirmation; B7: message keyword (use deny_reason/reason); KDR 5-kind: rows/empty/denied/query_error/system_error; IntegratedQuerySim PROOF-LOCAL ONLY; EXECQ2-COMPILE 5/5|EXECQ2-SHAPE 8/8|EXECQ2-GATES 6/6|EXECQ2-FILTER 8/8|EXECQ2-ORDER-LIMIT 8/8|EXECQ2-INTEGRATED 7/7|EXECQ2-RECEIPT 7/7|EXECQ2-VM 8/8|EXECQ2-CLOSED 9/9|EXECQ2-GAP 7/7; fixture: igniter-view-engine/fixtures/query_execution/execute_query_integrated.ig; proof runner: igniter-view-engine/proofs/verify_lab_execute_query_p2.rb; design doc: igniter-lab/lab-docs/lang/lab-execute-query-integrated-gates-filter-order-limit-receipt-v0.md; card: igniter-lab/.agents/work/cards/lang/LAB-EXECUTE-QUERY-P2.md) | (PROP-044-P8: PROP-044 Path B runtime lowering semantics governance lock — GOVERNANCE / DESIGN LOCK / NO IMPLEMENTATION; 335-check evidence chain (PROP-044-P3 50/50 + P5 75/75 + P6 50/50 + P7-READINESS 15/15 + LAB-VARIANT-RUST-P1 39/39 + LAB-VARIANT-VM-P1 42/42 + LAB-OUTCOME-VARIANT-P1 58/58 + LAB-EPISTEMIC-OUTCOME-P4 46/46); PATH B ACCEPTED as current lab VM runtime semantics: variant_construct→OP_PUSH_RECORD{__arm(discriminant)+__variant(diagnostic)+payload(user-fields)}; match_node→OP_GET_FIELD("__arm")+OP_EQ+OP_JMP_UNLESS chain+arm-body+OP_JMP(NO NEW OPCODE); wildcard=final unconditional body; payload-bindings=OP_GET_FIELD into scoped temp regs; exhaustiveness=TypeChecker-owned (OOF-KIND1..5); VM trusts SIR+appends OP_UNSUPPORTED as fail-closed safety net (NOT semantic exhaustiveness check); runtime mismatch must NEVER silently return Nil; KDR NOT OBSOLETED (remains valid for proof-local/boundary/serialization/interop; variant supersedes only where vocabulary is known+should-be-sealed); CLOSED: Value::Variant/OP_MATCH/OP_PUSH_VARIANT/native-variant-identity-reflection/generic-Outcome[T,E]/failure-taxonomy/production-runtime-stability/Ruby-canon/compiler.rs+vm.rs+value.rs+instructions.rs/public-stable-__arm-__variant-API; GUARDRAILS: users must not write __arm/__variant as public contract fields; external serialization of variant values is not stable; SIR variant_construct/match_node is portable artifact not lowered Record; public docs must teach source variant/match not __arm records; future compiler.rs match changes must re-verify sorted-key-invariant+"match_node"|"match_expr"-alias+has_wildcard-unwrap_or(false)-fail-closed; RISKS LOGGED (7): reserved-field-collision/hand-authored-records/cross-lang-serialization(HIGH)/future-native-identity/parity-drift/debugger-gap/agents-treating-Path-B-as-ABI; NEXT: LAB-OUTCOME-VARIANT-P2(if-needed)/LAB-FAILURE-TAXONOMY-P1(proposal-planning-only)/PROP-044-P9(requires-auth)/LAB-SRCMAP-P1(independent)/reserved-field-policy-card(recommended); proposal: igniter-lang/.agents/work/proposals/PROP-044-path-b-runtime-lowering-semantics-v0.md) | (LAB-APP-STATE-P1: Application state / module / instance-composition design-boundary RESEARCH report — RESEARCH / DESIGN BOUNDARY / NO IMPLEMENTATION; derive-from-pain analysis, deliberately does NOT assume a `state` keyword / service / actor / module-visibility / host-state is the answer; central finding = Igniter already has a state-LIFETIME vocabulary (:local/:session/:window/:durable/:audit on read/snapshot/output + `read … from "<store>"` + `window`) but NO state-HOLDER — it pushes holding outside the language and treats contracts as pure transforms over snapshots; the "flat application" pain is NOT missing state, it is that the COMPOSITION of {stateful facts + their lifetimes + their holders + the public operations that transition them} is invisible in source; three precisely-missing pieces = (1) state-INSTANCE identity in source, (2) named app-FACT↔HOLDER binding, (3) an app-ASSEMBLY artifact — everything else (typed-value records, lifecycle vocab, effect-surface/capability external boundary, contract modifiers, intent queryable purpose) already exists and should be REUSED not reinvented; six terms held distinct throughout = state-value / state-instance / state-holder / transition / module-boundary / external-capability; FIVE routes compared with full evaluation matrix — A Host-Owned+Pure-Reducers (honest floor, app invisible in source), B Descriptive State Vocabulary (inert metadata naming facts↔lifetime↔holder-class), C Capability-Carried State Handles (reuse PROP-035/046; correct only at durable boundary, fatal for hot editor state), D Composition Manifest `.igapp` (expose/internal + event→op→fact wiring; second-language + framework-drift risk), E Lifecycle-Scope Promotion (lean entirely on existing lifecycle annotations; conflation risk retention-vs-ownership); RECOMMENDATION = STAGED, status research-only→proof-candidate, explicitly NOT design-ready and NO keyword adopted: Stage 0 = Route A discipline as canonical app shape (already true, no authority needed), Stage 1 = B⊕E hybrid prototyped PROOF-LOCALLY with ZERO compiler/parser/VM/keyword/canon change, DEFER C (use only at durable edge) and D (manifest) until the Stage-1 proof shows metadata genuinely insufficient; non-recommendations (tempting, do-not-adopt-yet) = `state{}` declaration keyword (premature lock — Ch2 refuses to reserve entrypoint/section before a proven AST shape), service-object/actor with mutable fields (imports hidden mutable identity → breaks honesty/no-hidden-mutation/debuggability/proofability triad), module-as-instance/module-level mutable state (PROP-015 explicitly rejects), host-owned-opaque-state-with-no-vocabulary (leaves flat-surface unsolved), capability-handle-for-ALL-state (fatal for keystroke-rate cursor), capability-that-accumulates-operations (OOP back-door drift); pressure model = code-editor primary (document/cursor/selection/clipboard/diagnostics/undo-redo/buffers/prefs/bg-analysis/persistence/UI-routing mapped to the 6 terms — holders cluster into hot/:local, :session/:window, :durable matching existing lifecycle vocab; UI routing + hot-state pumping are HOST responsibilities not language) + Query/Storage non-editor case (LAB-QUERY-P3/LAB-STORAGE-CAPABILITY-P2: pure QueryPlan value in source + ExecuteQuery effect at capability boundary + QueryResult KDR denial-as-data — already-validated app shape; editor `save` ≅ `ExecuteQuery`) + Epistemic-Outcome (LAB-EPISTEMIC-OUTCOME-P2 `unknown_external_state` must be preserved by value-transition-with-receipt, lost by mutable-object model); §1.3 argues solving the symptom fast (a holder construct) erodes honesty+debuggability+proofability at once; reviewed PROP-015 (modules = scope+fragment-class authority, NOT holder/visibility/instance/composition) + PROP-031/Ch10 (pure|observed|effect|privileged|irreversible modifiers) + PROP-035/Ch12 (7-field effect surface + 7-outcome taxonomy incl unknown_external_state) + PROP-045 (intent = inert queryable metadata precedent) + Ch2 (lifecycle grammar + keyword-reservation caution) + LAB-DEBUGGER-FEASIBILITY-P1 + LAB-SRCMAP-P1 (node-anchored record-only value-trace model; no-hidden-mutation constraint); opens StorageCapability execution=NO, touches Ruby canon=NO, opens DB/SQL/ORM/runtime/storage=NO, adopts keyword=NO; authorized writes were exactly THREE — lab-docs/lang/lab-application-state-modules-and-instance-composition-boundary-v0.md + .agents/work/cards/lang/LAB-APP-STATE-P1.md + this portfolio; EXACT next route = LAB-APP-STATE-P2 proof-local code-editor app-state model [host-owned state values + pure/effect transition contracts + descriptive state vocabulary over existing lifecycle classes; validate inspectability + 6-term separation; no compiler/parser/VM/keyword/canon change] whose GAP PACKET naming which of {instance-identity, fact-holder-binding, app-assembly, public/internal-visibility} metadata cannot express GATES any future proposal-authoring card) | (LAB-SRCMAP-P1: Source-map substrate — stable node_id + source span metadata from Rust parser through SemanticIR, with durable sourcemap.json artifact — LAB PROOF / RUST COMPILER / DEBUGGER SUBSTRATE / NO VM; 61/61 PASS; 8 sections; SpanEntry/span_table additive to parser (no AST perturbation); 12 node kinds covered (contract/type/variant/input/output/compute/record_literal/array_literal/field_access/call/match_expr/variant_construct); node_id field additive to SIR compute nodes/ports/variant_declarations; JSONPath sir_path per entry; schema_version="srcmap-v0"; de-duplication: first occurrence wins; SRCMAP-COMPILE 6/6|SRCMAP-SCHEMA 11/11|SRCMAP-COVERAGE 12/12|SRCMAP-SIR-LINK 11/11|SRCMAP-STABILITY 4/4|SRCMAP-NONSEMANTIC 7/7|SRCMAP-ERROR 4/4|SRCMAP-CLOSED 7/7; 4 fixtures: srcmap_basic_contract.ig+srcmap_nested_record.ig+srcmap_variant_match.ig+srcmap_error_fixture.ig; proof runner: igniter-view-engine/proofs/verify_lab_srcmap_p1.rb; design doc: igniter-lab/lab-docs/ide/lab-rust-source-map-node-span-to-semanticir-v0.md; card: igniter-lab/.agents/work/cards/ide/LAB-SRCMAP-P1.md; VM/bytecode/opcodes/Value-enum/IDE-UI all CLOSED; dotted-stdlib-call constraint documented; keystone for LAB-SRCMAP-P2→LAB-VMTRACE-P1→LAB-IDE-STEP-P1→LAB-TEXTBOOK-P1 debugger track) | (LAB-QUERY-ORDER-LIMIT-P1: Order and limit semantics over mocked in-memory rows — LAB PROOF / QUERY SEMANTICS / IN-MEMORY MOCKED ROWS / NO DB; 54/54 PASS; 9 sections; proves QueryPlan.order (OrderBy) and QueryPlan.limit (Integer) have v0 semantic meaning over mocked rows; 7 pure contracts (all CORE; no effect; no capability); asc/desc lexicographic sort correct over 5-row deterministic dataset; stable sort (equal keys preserve input order); empty direction→preserve input order; unknown direction→kind:"query_error" (NOT denied); missing order field in row→kind:"query_error" (fail-closed); limit>0→first N rows after ordering; limit==0→kind:"empty"; limit<0→kind:"query_error" (NOT denied); order-then-limit invariant; filter→order→limit pipeline composes; QueryPlan.limit ≠ StorageCapability row_limit gate (orthogonal); BuildQueryPlanOrderLimit.filters=Collection[FilterPredicate] in Rust SIR (LAB-TC-ARRAY-P2 record-field-context mechanism, 4th confirmation); count==returned_rows.length invariant; B1: VM has no sort/iteration opcodes (Layer C correct boundary); B2: 4th confirmation of P2 mechanism; B3: unknown dir/neg limit/missing field all query_error not denied; B4: QueryPlan.limit ≠ StorageCapability row_limit; B5: message Ruby keyword confirmed (use reason); all comparisons lexicographic String in v0; OLIMIT-COMPILE 5/5|OLIMIT-SHAPE 7/7|OLIMIT-SEMANTICS 8/8|OLIMIT-LIMIT 7/7|OLIMIT-RESULT 6/6|OLIMIT-VM 8/8|OLIMIT-COMPOSE 4/4|OLIMIT-CLOSED 5/5|OLIMIT-GAP 4/4; proof runner: igniter-view-engine/proofs/verify_lab_query_order_limit_p1.rb; fixture: igniter-view-engine/fixtures/query_execution/order_limit.ig; design doc: igniter-lab/lab-docs/lang/lab-query-order-and-limit-semantics-over-mocked-rows-v0.md) | (LAB-OUTCOME-VARIANT-P1: Epistemic outcome as ReconciliationOutcome variant + exhaustive match — LAB PROOF / VARIANT OUTCOME / VM EXECUTION / NO FAILURE TAXONOMY; 58/58 PASS; 7 sections; re-expresses KDR routing (LAB-EPISTEMIC-OUTCOME-P4) as real Igniter variant+match; ReconciliationOutcome 11 arms (ConfirmedSucceededReal/Human/Model, ConfirmedFailedRetryable/Compensatable/Terminal, StillUnknownWithBudget/NoBudget, PartiallyConfirmed, ReconciliationDenied, ReconciliationError); StillUnknown split: budget-present→reconcile_again/budget-absent→hold, arm name encodes routing, no numeric guard in arm body; 11 contracts (RouteOutcome+9 build contracts+RouteBuiltOutcome); RouteBuiltOutcome: construct+route in one contract, proves full pipeline; OOF-KIND1..5 all fire (non-exhaustive/unknown-arm/duplicate-arm/non-variant-subject/divergent-types); payload fields String+Integer survive full compile→VM path; No-Upward-Coercion enforced by distinct arm names (ConfirmedSucceededModel≠Real; StillUnknown*→hold/reconcile_again never→accept/retry/compensate; Denied/Error→hold); OUTVAR-COMPILE 6/6|OUTVAR-SIR 6/6|OUTVAR-VM 16/16|OUTVAR-OOF 10/10|OUTVAR-KDR-EQUIV 8/8|OUTVAR-NO-UPWARD 5/5|OUTVAR-CLOSED 7/7; CLOSED: no Value::Variant, no OP_MATCH/OP_PUSH_VARIANT, no native variant identity, no Ruby canon change, no failure taxonomy, no generic Outcome[T,E], no production runtime, no real storage/IO, no retry/compensation execution; fixtures: epistemic_outcome/outcome_variant.ig+oof_kind1..5.ig; proof runner: igniter-view-engine/proofs/verify_lab_outcome_variant_p1.rb; design doc: igniter-lab/lab-docs/lang/lab-epistemic-outcome-as-variant-and-vm-match-proof-v0.md) | (LAB-FILTER-EVAL-P1: Filter predicate evaluation over mocked in-memory rows — LAB PROOF / QUERY SEMANTICS / IN-MEMORY MOCKED ROWS / NO DB; 50/50 PASS; 9 pure contracts (all CORE; no effect; no capability); v0 operators: eq/neq/contains/prefix; AND-only composition; Layer C FilterEvalSim proof-local Ruby evaluator (NOT production runtime); 5-row deterministic dataset; empty filter list → all rows; unknown field → no match (kind:"empty"); unknown op → kind:"query_error" (NOT "denied"); count==matched_rows.length invariant; BuildQueryPlanWithFilters.filters=Collection[FilterPredicate] in Rust SIR (LAB-TC-ARRAY-P2 record-field-context mechanism, 3rd confirmation); inline empty array → Collection[FilterPredicate] from record-field context; B1: VM has no iteration opcodes (Layer C correct boundary, not a workaround); B2: empty array field-context confirmed; B3: unknown field ≠ unknown op (must not collapse); B4: G1–G6 gate sequence orthogonal to filter evaluation; no DB/SQL/ORM/StorageCapability/production runtime; FEVAL-COMPILE 5/5|FEVAL-SHAPE 7/7|FEVAL-ARRAY 4/4|FEVAL-SEMANTICS 7/7|FEVAL-RESULT 6/6|FEVAL-VM 8/8|FEVAL-CLOSED 5/5|FEVAL-GAP 8/8; proof runner: igniter-view-engine/proofs/verify_lab_filter_eval_p1.rb; design doc: igniter-lab/lab-docs/lang/lab-query-filter-predicate-evaluation-over-mocked-rows-v0.md) | (LAB-VARIANT-VM-P1: Rust Lab VM variant/match lowering Path B — LAB VM LOWERING / PATH B / NO NEW OPCODES; 42/42 PASS; 10 sections; lowers variant_construct+match_node to existing VM opcodes in igniter-vm/src/compiler.rs; VVM-COMPILE: 7 fixtures (11–17) compile; VVM-CONSTRUCT: variant_construct→Record{__arm,__variant}+payload via OP_PUSH_RECORD sorted-keys; VVM-MATCH: unit arm selection via GET_FIELD("__arm")+EQ+JMP_UNLESS chain; VVM-BIND: payload field bindings into scoped temp regs, body refs via OP_LOAD_REG; VVM-WILDCARD: wildcard arm catches unlisted; VVM-FAILCLOSED: unknown arm→OP_UNSUPPORTED (error, not nil), malformed __arm→GET_FIELD error; VVM-TWONODES: two match nodes, independent subject registers, no collision; VVM-NESTED: nested match_expr (raw AST kind from annotate_expr_with_type) handled via "match_node"|"match_expr" Rust match union; VVM-EQUIV: ReconciliationOutcome 5 arms route to P4 KDR actions (accept/needs_human_review/retry/hold/hold); VVM-CLOSED: instructions.rs/vm.rs/value.rs closed—no OP_MATCH, no OP_PUSH_VARIANT, no Value::Variant; design: variant=Record(sorted keys); __arm=discriminant; compiler.rs-only change; match_expr alias covers nested arm body raw-AST form; has_wildcard=unwrap_or(false)→fail-closed default; 7 new fixtures: 11_vm_variant_construct_basic/12_vm_match_unit_arms/13_vm_match_payload_bindings/14_vm_match_wildcard/15_vm_match_two_nodes/16_vm_match_kdr_equivalence/17_vm_nested_match; proof runner igniter-view-engine/proofs/verify_lab_variant_vm_p1.rb; design doc igniter-lab/lab-docs/lang/lab-rust-variant-match-vm-lowering-path-b-v0.md; satisfies PROP-044-P7b gate; Outcome[T,E]/failure-taxonomy/canon all CLOSED) | (LAB-DEBUGGER-FEASIBILITY-P1: Igniter debugger & source-mapping feasibility report (out-of-track) — REPORT / FEASIBILITY / LAB-ONLY / NO IMPLEMENTATION; goal = interactive debugger as the instrument under a TEXTBOOK that teaches by showing execution across multiple abstraction levels (source→AST→fragment-classified→typed→SemanticIR→bytecode→live state→observations), the anti-black-box alternative to a REPL; VERDICT = FEASIBLE (high confidence, instrument + textbook); THESIS: Igniter is an honest layered IR BY DESIGN (Covenant: observation is typed, evidence chained, uncertainty non-discardable) → "dimensional learning" is architectural not aspirational; EXISTS TODAY: igniter-ide Tauri2+Svelte5 (monaco/d3/vis-network) with DebuggerPanel/ExecutionTracer(frame stepper)/TemporalTimeline(D3 playback)/ContractDAG/ObservationStream/ContractInspector + Rust bridge (load_contract/dispatch_traced/play_trace_playback/read_introspection_receipt/read_facts) per LAB-IDE-DEBUGGER-P1/P2 + LAB-TAURI-IVF-P3..P20 + LAB-IDE-VIEWER-P1; VM single dispatch loop + OP_EMIT_OBS observation sink + latency_us + temporal OP_LOAD_AS_OF audit trail; learning-by-contract curriculum lab-docs/tutorial (LAB-TUTORIAL-P1..P5); all 8 abstraction layers are REAL inspectable artifacts incl. fragment_class; TWO FOUNDATIONAL GAPS: G-SRCMAP (lexer captures line/col but PARSER DROPS it; Instruction={opcode,args} no provenance; NO bytecode→source map today; fix=additive node_id+span thread parse→SIR→bytecode + .sourcemap artifact) and G-TRACE (VM runs to completion, no step/snapshot/breakpoint; the *_trace_receipt.json files are RESULT receipts NOT execution traces; fix=record-only execute_traced → .trace.json, prove equivalence to untraced); both additive, single clean insertion points, low-medium risk; CAVEAT: dual toolchain — build source-map on the Rust VM SIR path first, parity to Ruby (same asymmetry theme as LAB-VARIANT-RUST-P1/PROP-044-P7-READINESS); synchronized multi-pane debugger joins everything on node_id (source span↔SIR node↔bytecode offset↔trace frame↔observations); proposed route (NONE authorized): keystone LAB-SRCMAP-P1 (node_id+span parse→SIR Rust-first + .sourcemap) → LAB-SRCMAP-P2 (bytecode spans) → LAB-VMTRACE-P1 (record-only trace, equivalence-proved) → LAB-IDE-STEP-P1 (synchronized panes) → LAB-TEXTBOOK-P1 (watchable lessons) → optional LAB-DEBUG-REVERSE-P1 (reverse/temporal scrubbing + line breakpoints); canon touched=NO, implementation authorized=NO; report lab-docs/ide/igniter-debugger-and-source-mapping-feasibility-report-v0.md) | (LAB-VARIANT-RUST-P1: Rust lab compiler variant/match front-end + SIR parity — LAB RUST PARITY / FRONT-END + SEMANTICIR / NO VM; 39/39 PASS; 8 sections; implements variant/match in igniter-compiler from lexer through SemanticIR emission producing structural SIR parity with Ruby PROP-044-P6; VRUST-LEX: FatArrow+variant+match keywords lexed (no OOF-G1); VRUST-PARSE: VariantDecl/VariantConstruct/MatchExpr in AST; VRUST-TYPE: VariantShapes registry, exhaustive match accepted, scope isolation; VRUST-OOF: OOF-KIND1..5 all fire correctly (non-exhaustive/unknown-arm/duplicate-arm/non-variant-subject/divergent-arm-types) + error fixtures blocked; VRUST-SIR: variant_declarations at top level + match_node with exhaustive/has_wildcard/subject_type/arm resolved_types; VRUST-PARITY: 8 structural keys match Ruby P6 SIR shape (variant_decl/match_node/pattern.arm+bindings+wildcard/arm resolved_type); VRUST-REG: 3 conformance fixtures unaffected; VRUST-CLOSED: igniter-vm/src/* untouched, no Value::Variant, no OP_MATCH, no OP_PUSH_VARIANT, no match_node lowering; design: annotated_expr flow (Option<Value> on TypedExpression/TypedDecl carries enriched SIR-ready JSON from TC to emitter, no new IR pass); VariantShapes=HashMap<variant→arm→field→type_ir>; match_expr→match_node rename in lower_annotated_expr; 10 fixtures (basic/unit-arm/wildcard/OOF-KIND1..5/scope-isolation/sir-parity); proof runner igniter-view-engine/proofs/verify_lab_variant_rust_p1.rb; design doc igniter-lab/lab-docs/lang/lab-rust-variant-match-front-end-and-sir-parity-v0.md; satisfies PROP-044-P7-READINESS precursor condition for P7b VM dispatch (Path B); VM/opcodes/bytecode/Outcome[T,E]/failure-taxonomy/Ruby-canon all CLOSED) | (LAB-EXECUTE-QUERY-P1: ExecuteQuery effect contract and StorageCapability injection proof — LAB PROOF / STAGE 2+ / MOCKED STORAGE EXECUTION / NO REAL DB; 57/57 PASS; 10 sections; proves first executable Stage 2+ query path: ExecuteQuery effect contract (Layer A+B compile; ESCAPE class; VM requires capability injection — correct enforcement boundary); 6-gate denial sequence (G1–G6) via Layer C ExecuteQuerySim with full QueryPlan + StorageCapability hashes; G4=row-limit clamp (not denial); G5→query_error (not denied); QueryExecutionReceipt 15-field invariants (cap_granted:false iff {denied,query_error}; rows_returned:0 when denied); BuildQueryPlanInline.filters typed Collection[FilterPredicate] in Rust SIR (LAB-TC-ARRAY-P2 record-field-context confirmed in EXECQ fixture); denial-as-data 10th proof (StorageCapability 5th domain); 5-kind KDR vocabulary proved; TBackend/TEMPORAL absent (orthogonal confirmed); write ops CLOSED in v0; two-fixture architecture (execute_query_capability.ig compile-only + execute_query_receipts.ig VM-executable 12 pure contracts); B3: deny_reason (message keyword); B4: read_file (read keyword); B5: TBackend absent; no DB/SQL/ORM/raise/persistence) | (PROP-044-P7-READINESS: VM variant/match dispatch readiness & risk map — GOVERNANCE / VM DISPATCH READINESS / NO IMPLEMENTATION; DECISION = HOLD P7 (VM dispatch); CORE FINDING: variant/match exists ONLY in the Ruby canon pipeline (PROP-044-P3 parser/P5 TypeChecker+OOF-KIND/P6 SIR emitter) — the ENTIRE Rust lab toolchain (igniter-compiler + igniter-vm, which is what every VM proof runs) has ZERO variant/match support at every layer: lexer KEYWORDS lacks variant/match (→OOF-G1), parser Expr enum has no Match node, typechecker has no variant/exhaustive/OOF-KIND, emitter emits no variant_declarations/variant_construct/match_node, VM Value enum has no Value::Variant (8 kinds), instructions.rs has no OP_MATCH (~34 ops), VM compiler.rs has no match_node lowering (fails closed on unknown nodes); so the Rust compiler REJECTS variant/match source before any node reaches the VM — "VM dispatch" is blocked behind a full Rust front-end re-implementation, NOT "add an opcode"; survey survey_variant_match_vm_readiness.rb 15/15 PASS grounds this (Rust→OOF-G1/no-SIR; Ruby parses variants; Rust source greps confirm absent surfaces; KDR P4 regression anchor green); PATH COMPARISON: Path A (native Value::Variant+opcodes — high VM surface/risk, true runtime identity) vs Path B RECOMMENDED (lower match_node→Record+if/else over OP_GET_FIELD/OP_EQ/OP_JMP_UNLESS/OP_PUSH_RECORD — NO new opcode, NO Value::Variant, compiler.rs-only, P4 ALREADY proved the lowered shape executes 46/46, low risk; removes string-dispatch fragility at SOURCE/typecheck layer where exhaustiveness+narrowing live, runtime stays tag== which is fine); design answers: vm_dispatch_ready=NO, recommended=Path B, new_opcodes=NO, Value::Variant=NO, match→ifelse=YES(proven), avoids ==/||-divergence for outcome routing (post-typecheck lowering, user writes match not ==) but does NOT fix it (STAB-P4 owns), requires Rust changes BOTH front-end+VM, Ruby changes=NO, authorizes failure-taxonomy PROP=NO, authorizes sealed Outcome[T,E]=NO, exhaustiveness=typecheck-only (VM trusts TC + lowers to FAIL-CLOSED default never silent Nil), arm-identity=compiler-owned; PRECURSOR ROUTE: PROP-044-P7a (Rust front-end variant/match + SIR PARITY with Ruby emitter, no VM — gate=P7a-PARITY Rust SIR≡Ruby SIR + OOF-KIND parity) → then PROP-044-P7b (VM dispatch Path B — gate=P7b-EQUIV variant Outcome routing yields SAME terminal actions as P4 KDR RouteReceipt); failure-taxonomy proposal-planning WAITS until ≥P7a lands; regression KDR P2 54/54 + P4 46/46 + P3 43/43 green, git only-new-files; 15/15 PASS) | (LAB-TC-ARRAY-P2: Rust TypeChecker array-literal-in-record-field-context proof — LAB PROOF / RUST TYPECHECKER / COLLECTION CONTEXT PROPAGATION; closes the non-blocking gap left by LAB-TC-ARRAY-P1; an intermediate `compute filters = [...]` that feeds a typed record field — `compute plan = {kind:"select",...,filters:filters,...}` / `output plan : QueryPlan` where QueryPlan.filters : Collection[FilterPredicate] — now types `filters` as Collection[FilterPredicate] (was Unknown in P1, data-preserved-but-type-lost); impl in typechecker.rs = an order-independent prescan contributing record-field hints to the SAME collection_output_hints map P1 introduced: for each RecordLiteral compute whose declared output type is a named record (output_type_hints), each field bound to a bare Ref whose record-type-declared field type is Collection[T] feeds element hint T to the referenced compute node (entry().or_insert — P1 output-context hints win); the compute-phase upgrade block is UNCHANGED (consults collection_output_hints); LOCAL single-hop syntactic Ref-field lookup — NO global/Hindley-Milner inference, NO unification, NO retroactive symbol mutation (the referenced `filters` compute is processed before the enclosing `plan` record literal in dependency order, so the array-literal node is upgraded in place); empty intermediate array typed from field context IFF expected field type known; bad/mixed record elements still fail closed (OOF-TY0) via check_array_literal_shape; P1 output-context typing + free-standing-Unknown preserved; Collection[FilterPredicate] survives into SIR type_tag (filters compute node); VM round-trips plan.filters (2 records / empty []); no new grammar; touches Ruby canon=NO (parity anchor); opens StorageCapability execution=NO; opens DB/SQL/ORM/runtime/storage=NO; PROP-046 unchanged; remaining edges deferred to optional v1 collection-inference (inline-in-field literal not via Ref, multi-hop Ref chains, conflicting hints first-wins); 19/19 PASS; regressions clean — P1 27/27, P3 44/44, VM-MAP 48/48, P13 47/47, record-vm construction/field-access/nested 43/42/49; next route = LAB-EXECUTE-QUERY-P1 (Stage 2+ capability-injection) — P1+P2 give sufficient inline-filter expressivity) | (LAB-EPISTEMIC-OUTCOME-P4: VM KDR ReconciliationReceipt flow proof — LAB PROOF / VM KDR RECEIPT FLOW / NO OUTCOME VARIANT; proves a KDR ReconciliationReceipt (11 fields: kind/request_id/resource/idempotency_key/observed_at/evidence_kind/compensation/attempt:Integer/budget_remaining:Integer/detail/metadata:Map) is PRODUCED, CARRIED, INSPECTED, ROUTED through the lab Rust VM as record data — P3 transition guards executed IN-VM as nested if/else; attempt typed Integer (numeric w/ budget guard, no String→Int coercion, Sidekiq precedent); 5 contracts (ReconcileFromLostAck producer pulls request_id/resource from env.metadata + preserves idempotency_key; MakeReceipt; RouteReceipt heart-router; RouteEnvelope; ReceiptInspector map-chain); VM-PROVED all P3 transitions: confirmed_succeeded+real|human→accept, +model→needs_human_review (evidence_kind LOAD-BEARING at runtime — No-Upward-Coercion executable), confirmed_failed+idem→retry(P16), +named-comp→compensate(P17), +neither→fail, still_unknown/reconciliation_error+budget→reconcile_again|+no-budget→hold, partially_confirmed→reconcile_remainder, reconciliation_denied/unknown-kind→hold(fail-closed), raw unknown/timed_out/partial→reconcile_required ONLY (NO VM branch to terminal success/failure); REAL Ruby/Rust DIVERGENCE surfaced+flagged for STAB-P4: production Ruby TypeChecker rejects String `==`/`||` (routers BLOCKED in Layer A) while Rust compiler accepts `==` (fixture avoids `||` by nesting) and Rust VM EXECUTES routing → routing is Rust-VM-only proof, not dual-impl (NOT resolved here); Layer A=Ruby TC proves receipt type-shape+producers accepted, Layer B=Rust compiler+VM proves routing execution; uses variant/match runtime=NO; implements sealed Outcome[T,E]=NO; opens storage/DB/network/runtime I/O=NO; authorizes failure-taxonomy PROP=NO; regression P2 54/54 + P3 43/43 green, git only-new-files; next=PROP-044-P7-READINESS (VM variant/match dispatch sequencing + risk map — true gate for sealed Outcome[T,E], also closes the ==-divergence), then only after: failure-taxonomy proposal-planning card; 46/46 PASS) | (LAB-TC-ARRAY-P1: Rust TypeChecker array-literal-in-Collection-context proof — LAB PROOF / RUST TYPECHECKER / NO STORAGE RUNTIME; closes LAB-QUERY-P3 finding B1 (Rust array_literal catch-all gap); `compute filters = [{field,op,value},{...}]` and `[f1,f2]` now type as Collection[FilterPredicate] in a declared `output x : Collection[T]` position; behavior is CONTEXTUAL — mirrors the RecordLiteral nominal upgrade (LAB-RACK-P13): an array literal is checked element-by-element against T and upgraded to Collection[T]; impl in typechecker.rs = collection_output_hints prescan (output decls with Collection annotation → element type IR) + explicit Expr::ArrayLiteral arm in infer_expr (types items for deps, resolves Unknown free-standing — removes OOF-TY0 "Unsupported expression kind: array_literal") + contextual upgrade block in compute phase + check_array_literal_shape helper (RecordLiteral element → check_record_literal_shape; Ref/Literal element → element type-name match; record-literal-vs-scalar → fail closed); empty array accepted ONLY with contextual type (0 elements → upgrade); free-standing array literal (no Collection output hint) stays Unknown (no fabricated type); missing/extra/wrong-typed record fields + mixed element shapes fail closed (OOF-TY0); Collection[FilterPredicate] survives into SIR type_tag (compute node + output port); VM round-trips inline-constructed collection (InlineFilterCollection→2 records, EmptyFilterCollection→[], BuildInlineSelectPlan→plan.filters 2-elem); Layer B (Rust compiler+VM) primary + Layer A (Ruby TC) parity anchor; no new grammar; touches Ruby canon=NO; opens DB/SQL/ORM/runtime/storage=NO; opens StorageCapability execution=NO; PROP-046 semantics unchanged; P3 workaround CLOSED; open follow-up (non-blocking) = record-field-position contextual typing for nested array literals (intermediate filters feeding a record field stays Unknown, data preserved); 27/27 PASS; regressions clean — P3 44/44, P13 47/47, VM-MAP 48/48, record-vm construction/field-access/nested 43/42/49; next route = LAB-EXECUTE-QUERY-P1 (Stage 2+ capability-injection) now unblocked OR record-field collection-typing follow-up if broader inference wanted first) | (LAB-EPISTEMIC-OUTCOME-P3: reconciliation-consumer boundary for unknown state — DESIGN NOTE / LAB-ONLY / NO IMPLEMENTATION AUTHORITY; defines how a downstream DAG node may consume unknown_external_state without coercing it into success/failure/retry/compensation; spine: reconciliation IS the explicit typed conversion the Covenant No-Upward-Coercion rule requires (unknown=low certainty → confirmed=observed); three state bands (effect kinds | reconciliation lifecycle reconcile_required/confirmed_succeeded/confirmed_failed/still_unknown/partially_confirmed/reconciliation_denied/reconciliation_error | terminal actions accept/deny/retry/compensate/fail/cancel/record/hold); allowed: unknown/timed_out/partial→reconcile_required→6 results; confirmed_succeeded→accept⇐real|human(P13); confirmed_failed→retry⇐idempotency(P16)|compensate⇐named(P17)|fail; still_unknown→reconcile⇐budget|hold; forbidden: unknown→succeeded/failed/accept/retry/compensate(direct), timed_out→failed(P15), reconcile_required→accept, model→real-without-conversion(P13); min ReconciliationReceipt KDR = kind+request_id+resource(required)+idempotency_key/observed_at/evidence_kind/compensation/attempt/metadata(typed,never-dropped); evidence_kind load-bearing (blocks model→real upgrade → needs_human_review); KDR-now/Outcome-later bridge (forbidden transitions rejected-by-convention now → unrepresentable-type-error later once VM variant dispatch proved); requires variant/match runtime=NO; authorizes Outcome[T,E] impl=NO; opens storage/IO=NO; proof-local pure-Ruby state machine verify_reconciliation_state_machine.rb 43/43 PASS (design evidence, not runtime); existing proofs unchanged (git: only new files); next=LAB-EPISTEMIC-OUTCOME-P4 executable reconciliation state-machine + parallel governance probe on VM variant/match dispatch sequencing (the real gate for Outcome[T,E]); failure-taxonomy PROP deferred until that gate understood) | (LAB-STORAGE-CAPABILITY-P2: IO.StorageCapability mocked execution boundary proof — 51/51 PASS; 6-gate denial sequence (G1–G6); G4=row-limit clamp (not denial); G5→query_error (not denied); QueryExecutionReceipt 15-field evidence record; denial-as-data 9th proof/5th domain; ESCAPE class enforcement confirmed (passport gap = correct behavior); two-fixture architecture (effect+pure contract lab separation); KDR 5th domain (5 kinds: rows/empty/denied/query_error/system_error); B2: Rust classifier effect name closed vocab {read_file,read_json,read,write_file,write_json,write}; B3/B4: read/message are parser keywords; no DB/SQL/ORM/raise/persistence) | (PROP-046-P1: IO.StorageCapability boundary proposal — PROPOSAL AUTHORING ONLY; 14 sections; 15 decisions locked (D1..D15); core formula: QueryPlan=pure CORE/StorageCapability=ESCAPE authority gate/QueryResult=typed KDR; StorageCapability≠DB/ORM/SQL/TBackend; 6-gate sequence locked; QueryExecutionReceipt 15-field schema locked; ExecuteQuery ESCAPE→STORAGE Stage 2+ (ch4 amendment required); write ops deferred; LAB-STORAGE-CAPABILITY-P2 authorized) | (LAB-EPISTEMIC-OUTCOME-P2: unknown-state KDR convention proof — LAB PROOF / KDR CONVENTION / NO IMPLEMENTATION AUTHORITY; domain: storage write commit-ack loss; OutcomeEnvelope KDR (kind:String + idempotency_key:String + metadata:Map[String,String]); 7 kinds (succeeded/denied/timed_out/unknown_external_state/partial/cancelled/compensated); 9 contracts; Layer A Ruby TC 9/9 accepted + Layer B Rust VM (7 kinds executed) + Layer C consumer sim ReconciliationRouter; PRIMARY: CommitWriteLostAck → unknown_external_state (NOT failed/system_error/upstream_unavailable), idempotency_key + reconcile metadata preserved through VM, StorageOutcomeMapper lost-ack → unknown not system_error; invariants: timeout≠failure (P15), retry gated on explicit idempotency (P16 — no key⇒no retry, denied never retried), denied≠unknown (not-sent vs sent-unconfirmed), partial≠unknown, reconciliation is data not raise; KDR sufficient as v0 = YES; proves enforced Outcome[T,E] = NO (no variant/match); authorizes PROP impl = NO; storage/network/runtime I/O opens = NO; existing Query/Rack/Sidekiq proofs not changed (git: only new files; P14 58/60 = pre-existing map_get VM-gap); next = LAB-EPISTEMIC-OUTCOME-P3 reconciliation-consumer design note; 54/54 PASS) | (LAB-EPISTEMIC-OUTCOME-P1: epistemic outcome model + unknown-state boundary — GOVERNANCE/DESIGN/LAB-ONLY; finding: canon already declares the model (Ch12 7 outcomes incl. unknown_external_state ch12:131 + Covenant P11/P13/P15/P16/P17 + Epistemic State Machine/No-Upward-Coercion) but it is unimplemented (UnknownExternalOutcome/ObservedFailure named-not-spec'd; P15=planned PROP covenant:680) AND lab proofs flatten it (QueryResult system_error folds timeout/lost-conn; ContractResult upstream_unavailable=budget-exhaustion is unknown→failure coercion; storage commit-ack unmodeled); three orthogonal axes (Outcome=Ch12/UnknownExternalOutcome | Observation=Obs[kind,T] P13 | Estimation=~T/Uncertain P11/PROP-026 — probabilistic is SEPARATE track); Result/Option insufficient (closed-world only); KDR kind:String = sufficient v0 convention; variant Outcome[T,E] = enforced model, typecheck-expressible via PROP-044-P3/P5 (grammar blocker LIFTED at TC) but runtime-blocked (match VM unproved); blocks storage WRITE + effectful network exec (P15+P16); does NOT retroactively change Query/Rack/Sidekiq proofs (reclassified epistemically-incomplete); no impl/types/parser/runtime/PROP; next = LAB-EPISTEMIC-OUTCOME-P2 unknown-state KDR convention proof → reconciliation note → failure-taxonomy PROP on PROP-044 substrate; STAB-P4 flag: P15→PROP-035 number collision) | (PROP-044-P6: variant+match SemanticIR emitter — semantic_variant_declarations (variant_env→variant_decl array); semantic_variant_construct (typed_fields→fields rename; arm/variant/resolved_type); semantic_match_node (match_expr→match_node rename; subject/subject_type/arms/exhaustive/has_wildcard/resolved_type); semantic_match_arm (pattern/body/resolved_type); dispatch wired into semantic_expr; variant_declarations emitted at top level of semantic_ir_program; OOF-KIND1..5 programs → nil semantic_ir (emit_typed guard); 50/50 PASS; P5 75/75 + P3 50/50 + OOF-R3 33/33 regressions clean) | (LAB-QUERY-P3: QueryPlan v1 nested records + Collection[FilterPredicate] proof — 8 contracts; 7 types (QuerySource/Projection/FilterPredicate/OrderBy/QueryPlan/QueryResult/StorageDenied); Layer A+B (Ruby TC + Rust VM); chained field access plan.source.table (two-hop OP_GET_FIELD); C1 chain (map_get+or_else) on richer QueryPlan; denial-as-data kind:"denied" 8th proof; Rust typechecker array_literal gap documented; 44/44 PASS) | (PROP-044-P5: variant+match TypeChecker implementation — classifier bridge (variant_declarations() method + variant_declarations key in classified_program); @variant_shapes 3-level store (variant→arm→field→type_ir); variant_shapes() builder; find_variant_for_arm(); infer_variant_construct; infer_match_expr (full + degraded mode); unify_match_arm_types; OOF-KIND1..5 ACTIVE; variant_env propagated to typed_program; 75/75 PASS; all regressions clean 55+33+100+50) | (PROP-045-P2: intent descriptor parser + metadata propagation proof — ParsedProgram#to_h intent_text fix; keyword added to KEYWORDS; module-level + contract-level parse; classifier OOF-INTENT3; typechecker pass-through; SemanticIR intent_text emission; orthogonal to profile_binding; 53/53 PASS; 15 decisions locked) | (PROP-044-P4: variant+match TypeChecker design — @variant_shapes 3-level store; variant_shapes() builder; classifier bridge (variant_declarations() + classified_program key); infer_variant_construct pseudocode; infer_match_expr pseudocode; unify_match_arm_types; OOF-KIND1..5 formal trigger+message definitions; per-arm narrowed scopes; exhaustiveness algorithm; degraded mode; 16 design decisions locked; proof requirements 15 groups ~75-80 checks; P5 implementation requires explicit auth) | (PROP-044-P3: variant+match parser implementation — fat_arrow lexer; variant/match keywords; parse_variant_decl/parse_variant_arm/parse_variant_construct/parse_match_expr/parse_match_arm/parse_match_pattern; ParsedProgram variants field + grammar_version=variant-v0; 50/50 PASS; all prior proofs clean) | (LAB-STORAGE-CAPABILITY-P1: IO.StorageCapability boundary design — allowed_sources (fail-closed); allowed_ops (["read"] v0); row_limit clamp; allow_include_all gate; read_allowed/write_allowed split; 6-gate denial-as-data sequence; QueryExecutionReceipt shape; ExecuteQuery ESCAPE→STORAGE fragment; OOF-STORE1..5 candidates; 10 decisions locked; DB/SQL/ORM/migrations/transactions permanently closed) | (PROP-045-P1: Source-level intent descriptor — keyword `intent`; bounded plain string; module+contract placement; preserved in contract_ir as intent_text; not in behavior digest; not capability/policy/runtime; OOF-INTENT1..4 reserved; CR-003 orthogonal; P2 parser impl requires explicit auth; 20 decisions locked) | (LAB-QUERY-P2: QueryPlan pure builder proof — 6 contracts (BuildQuerySource/BuildSelectQuery/BuildFilteredQuery/QueryResultDenied/QueryMetadataReader/QueryMapper); 7 types (QuerySource/Projection/FilterPredicate/OrderBy/QueryPlan/QueryResult/StorageDenied); denial-as-data QueryResult{kind:"denied"}; C1 chain in 4th domain; all CORE fragment; 42/42 PASS) | (PROP-044-P2: variant+match grammar design — VariantDecl EBNF; MatchExpr EBNF; VariantConstruct expression; type narrowing rules; OOF-KIND1..5 formal definitions; SemanticIR shapes (variant_decl/variant_construct/match_node); parser+typechecker extension points; 15 design decisions locked; P3 parser impl requires explicit auth) | (LAB-QUERY-P1: Query/Arel-like data access pressure boundary — QueryPlan/QueryResult/FilterPredicate/OrderBy/QuerySource typed Records; ORM permanently closed; joins/aggregates deferred; StorageCapability boundary defined; denial-as-data 5-kind QueryResult; CORE fragment class for plan-building; LAB-QUERY-P2 next) | (LAB-COMPILER-LIVENESS-P6: Body-decl recovery generalised — 11 .ok() arms → parse_body_decl_with_recovery; window/loop/for deferred to P7; decreases proved always-Ok; 54/54 PASS) | (PROP-044-P1: Kind-discriminated outcome convention and sum type requirements — convention doc authored; KDR pattern defined; denial-as-data invariant stated; grammar gap enumerated (variant+match+narrowing); OOF-KIND1..4 namespace reserved; production implementation blocked; grammar proposal P2 authorized) | (LAB-COMPILER-LIVENESS-P5: Parser hang class closed — peek_type EOF fix; parse_body_decl_with_recovery; parse_type_decl field recovery; BoundedCommand timeout kill; 46/46 PASS) | (LAB-RESULT-ENVELOPE-P2: Third-domain kind-discriminant pressure — validation/form-processing domain; ValidationResult 4-kind (valid/invalid/unauthorized/system_error); no HTTP status, no job fields; denial-as-data 7th proof; Map[String,String] metadata 3rd context; kind-discriminant generalised across 3 domains; PROP-044 unblocked for proposal-authoring; 50/50 PASS) | (LAB-CONCURRENCY-P4: Minimal scheduler substrate contract design-locked; five-phase model; 9 invariants SI-1..SI-9) | (LAB-VM-MAP-P1: VM runtime map_get/map_has_key OP_CALL handlers; or_else pre-existing; Value::Record = Map runtime; compiler input field access fix; Rack P14 10/10 gap closed; 48/48 PASS) | (LAB-RESULT-ENVELOPE-P1: Governance taxonomy — 5 reusable patterns confirmed; next route = LAB-VM-MAP-P1 + LAB-RESULT-ENVELOPE-P2)
...
**Boundary:** `sourcemap.json`, `bytecode_map.json`, `vm_trace.json`, `source_trace.json`, and proof-rendered `source_trace_view.md` are lab-only sidecar/derived artifacts — not stable public APIs, not canon claims, not runtime authority. The P3 loop source-node annotation is proof-local in `/tmp` compiled artifacts only; it does not change compiler/VM semantics or canon source authority. VM execution loop, opcodes, Value enum, debugger/stepper/breakpoints/watch expressions, IDE UI/Tauri/Svelte, and public trace APIs remain closed.

**Route:** LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 ✅ → LAB-VMTRACE-P1 ✅ → LAB-VMTRACE-P2 ✅ → LAB-IDE-TRACE-VIEW-P1 ✅ → LAB-VMTRACE-P3 ✅. Next exact route: **A. LAB-IDE-TRACE-VIEW-P2 static HTML viewer may open**. LAB-IDE-STEP-P1 remains closed.
