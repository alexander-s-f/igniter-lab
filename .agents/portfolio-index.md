...
56. ✅ LAB-QUERY-PROJECTION-P1: Projection and include_all row-shaping semantics over mocked rows (2026-06-10)
    ...
    Next authorized: TypeChecker nested-record-literal context propagation (B9 — separate card); Typed Row[T]/schema-aware projection (separate card); Collection[String] field list grammar (grammar change — separate card); LAB-EXECUTE-QUERY-P3: integrate projection+multi-order into unified receipt (separate card); production projection runtime (ProjectionSim is PROOF-LOCAL ONLY — separate card)
