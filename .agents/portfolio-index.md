...
**Boundary:** `sourcemap.json` and `bytecode_map.json` are lab-only sidecar artifacts — not stable public APIs, not canon claims, not runtime concerns. The `node_id` field is additive to existing SIR nodes (does not replace or rename any existing field). The `node_id_map` in the VM compiler is a parallel structure — `Instruction { opcode, args }` is unchanged. VM execution loop, bytecode offsets beyond the map, opcodes, and Value enum are all closed. IDE UI/Tauri/Svelte are closed (separate LAB-TAURI-IVF track). LAB-VMTRACE-P1 (record-only VM trace) is the natural next step but requires separate authorization.

**Route:** LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 ✅ → LAB-VMTRACE-P1 (record-only trace; not authorized) → LAB-IDE-STEP-P1 (synchronized debugger panes; not authorized) → LAB-TEXTBOOK-P1 (watchable lessons; not authorized)
