...
**Boundary:** `sourcemap.json` is a lab-only sidecar artifact — not a stable public API, not a canon claim, not a runtime concern. The `node_id` field is additive to existing SIR nodes (does not replace or rename any existing field). VM execution, bytecode offsets, opcodes, and Value enum are all closed. IDE UI/Tauri/Svelte are closed (separate LAB-TAURI-IVF track). LAB-SRCMAP-P2 (bytecode span threading) and LAB-VMTRACE-P1 (record-only VM trace) are the natural next steps but require separate authorization.

**Route:** LAB-SRCMAP-P1 ✅ → LAB-SRCMAP-P2 (bytecode spans; not authorized) → LAB-VMTRACE-P1 (record-only trace; not authorized) → LAB-IDE-STEP-P1 (synchronized debugger panes; not authorized) → LAB-TEXTBOOK-P1 (watchable lessons; not authorized)
...
53. ✅ LAB-QUERY-ORDER-LIMIT-P1: Order and limit semantics over mocked in-memory rows (2026-06-10)
    ...
    Next authorized: LAB-EXECUTE-QUERY-P2 (gate sequence + filter + order + limit + receipt in one mocked execution simulator — requires explicit card); multi-column ordering (order: Collection[OrderBy] — separate card); numeric/date ordering (type promotion in row values — deferred v0); production order/limit runtime (OrderLimitSim is PROOF-LOCAL only — separate card)
