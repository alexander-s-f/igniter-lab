...
LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P4 CLOSED (IMPLEMENTATION PROOF — PROVED 67/67 PASS; RUST PARITY FOR stdlib.collection.{map,filter,count}; THREE GAPS: canonical SIR names via COLLECTION_HOF_OPS in emitter.rs + delegation fix in semantic_expr_for_compute; map lambda param Integer→element type T via get_param; filter OOF-COL3 for non-Bool predicates; TWO FILES: typechecker.rs + emitter.rs; 9 sections A–I; no Ruby/fold/sum/VM/inventory changes; next: LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P5 inventory integration + OOF-COL1/COL2 Rust parity)
LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P5 CLOSED (INVENTORY + OOF PARITY — PROVED 86/86 PASS; TWO NEW INVENTORY ENTRIES: stdlib.collection.map + stdlib.collection.filter (dual-toolchain/lab-implemented/OOF-COL1+COL2 in diagnostics); UPDATED count proof_lineage; digest recomputed; Rust OOF-COL1/COL2 active for count/filter/map in typechecker.rs; no Ruby changes; map/filter/count STABILIZATION TRACK CLOSED)
LANG-STDLIB-COLLECTION-APPEND-P1 AUTHORED (PROPOSAL — stdlib.collection.append; Collection[T] × T → Collection[T]; pure/core/no-authority; bare alias 'append'; order-preserving/item-at-end; OOF-COL1+COL2+COL6-NEW; bootstrap pattern (T×T via call_contract) outside canonical type; 6+ app fixture evidence surfaces; next: LANG-STDLIB-COLLECTION-APPEND-PROP-P2 Ruby TC implementation)
**Last updated:** LANG-STDLIB-SUM-PROP-P3 CLOSED (IMPLEMENTATION PROOF — PROVED 51/51 PASS; Ruby TypeChecker implementation of stdlib.collection.sum two-arg field-projection form; ONE FILE: typechecker.rb only; TWO INSERTIONS: (1) when "sum" arm in infer_call after COLLECTION_HOF_FNS arm; (2) infer_sum_call private method ~55 lines; SEPARATE DISPATCH: Symbol not lambda — no has_symbol flag in COLLECTION_HOF_FNS; direct when "sum" + infer_sum_call (matches or_else/infer_outcome_call precedent); FIELD LOOKUP: element_type_from_collection → elem_type_name → @type_shapes[elem_type_name][field_name] → declared type_ir verbatim; Decimal[2] params format: [2] integer array (not hash); scale preserved automatically; OOF-COL1 arity != 2; OOF-COL2 non-Collection first arg; OOF-COL5 non-Symbol second arg + missing field (Unknown elem exempt); no numeric constraint; SIR fn = "stdlib.collection.sum" (qualified — closes Rust bare-"sum" parity gap); fold dispatches through COLLECTION_HOF_FNS unaffected (A-04); SECTIONS: A regression 6/6 / B dispatch 6/6 / C arity 6/6 / D collection 6/6 / E symbol 6/6 / F field lookup 8/8 / G OOF codes 7/7 / H authority 6/6; proof: experiments/stdlib_sum_proof/verify_stdlib_sum_p3.rb; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-SUM-PROP-P3.md; authority closed: no emitter / no parser / no Rust / no VM / no one-arg sum / no sumBy / no numeric constraint / no fold; next: LAB-STDLIB-NUMERIC-P1 (gates one-arg sum Split B) / Rust SIR parity card (qualified name)) | LANG-STDLIB-SUM-PROP-P2 CLOSED (IMPLEMENTATION PLANNING / READY FOR P3 — stdlib.collection.sum two-arg field-projection form; ONE FILE: typechecker.rb only; TWO INSERTIONS: (1) when "sum" arm in infer_call after COLLECTION_HOF_FNS arm (~line 893); (2) infer_sum_call private method after infer_collection_hof_call (~line 2366) ~45 lines; SEPARATE DISPATCH: Symbol not lambda — no has_symbol flag in COLLECTION_HOF_FNS; direct when "sum" + infer_sum_call pattern (matches or_else/infer_outcome_call precedent); FIELD LOOKUP: element_type_from_collection(collection_arg.resolved_type) → elem_type_name → @type_shapes[elem_type_name][field_name] → declared type_ir; Decimal[N] scale preserved automatically (params stored in type_ir hash by type_ir(annotation)); OOF-COL1 arity != 2; OOF-COL2 non-Collection first arg; OOF-COL5 non-Symbol second arg + missing field in type_shapes (Unknown elem type → permissive, no error); no numeric constraint in v0 (non-numeric field type accepted); SIR fn = "stdlib.collection.sum" (qualified — closes Rust bare-"sum" parity gap); zero emitter changes; REGRESSIONS: string_core + typed_ref_p5 71/71 + collection HOF 61/61 + outcome 60/60; PROOF MATRIX: 8 sections / 51 checks (A regression 6 / B dispatch 6 / C arity 6 / D collection 6 / E symbol 6 / F field lookup 8 / G OOF codes 7 / H authority 6); proof runner: experiments/stdlib_sum_proof/verify_stdlib_sum_p3.rb; planning doc: igniter-lang/.agents/work/proposals/LANG-STDLIB-SUM-P2-implementation-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-SUM-PROP-P2.md; authority closed: no impl / no one-arg sum / no sumBy / no Decimal operator / no fold / no Rust parity / no VM / no inventory / no app fixtures; next: LANG-STDLIB-SUM-PROP-P3 (implementation proof) / LAB-STDLIB-NUMERIC-P1 (gates Split B)) | LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P3 CLOSED (IMPLEMENTATION PROOF — PROVED 61/61 PASS; bounded Ruby TypeChecker implementation of stdlib.collection.map / stdlib.collection.filter / stdlib.collection.count; THREE INSERTIONS: (1) COLLECTION_HOF_FNS constant after OUTCOME_STDLIB_FNS.freeze ({map: arity 2/has_lambda true, filter: arity 2/has_lambda true, count: arity 1/has_lambda false}); (2) when *COLLECTION_HOF_FNS.keys dispatch arm in infer_call after OUTCOME arm; (3) infer_collection_hof_call + infer_lambda_body private methods; LAMBDA BINDING: element_type_from_collection extracts T → symbol_types.dup merge(param => elem_type) → infer_lambda_body; infer_lambda_body handles single-expr and block-form (kind=="block", stmts+return_expr); RETURN TYPES: map=collection_type_ir_from(body_type) / filter=collection_type_ir_from(elem_type) passthrough / count=Integer; SIR NAMING: zero emitter changes — generic semantic_expr preserves fn field verbatim; semantic_ir_name==canonical_name for all three; OOF-COL1 arity/non-lambda / OOF-COL2 non-Collection first arg / OOF-COL3 filter non-Bool predicate (Unknown passes permissively); PREEXISTING GAP: == operator returns Unknown (OOF-TY0 from operator_type); Unknown passes OOF-COL3 — filter with == dispatches correctly but emits OOF-TY0 from operator; P3 fixtures use Bool field predicates; COUNT T3 COEXISTENCE: T3 path in handle_t3_variant, never reaches infer_call; IGAPP ARCHITECTURE: igapp is a directory; SIR at igapp/semantic_ir_program.json; only written on status=ok; OOF negative checks use diagnostic inspection not SIR; APP FIXTURE RESULT: bookkeeping/ledger.ig filter+map no longer Unknown function; erp_logistics/optimizer.ig filter no longer Unknown function; REGRESSIONS: string_core_proof PASS + stdlib_outcome_p3 60/60 + typed_contract_ref_p5 71/71; SECTIONS: A regression 7 / B count 8 / C filter 9 / D map 9 / E SIR names 7 / F type inference 8 / G app fixtures 6 / H authority 7; authority closed: no Rust / no VM / no fold / no sum / no predicate-count / no named fn refs / no inventory edits / no app fixtures; proof: igniter-lang/experiments/stdlib_collection_proof/verify_stdlib_collection_map_filter_p3.rb; proof packet: igniter-lang/.agents/work/proposals/LANG-STDLIB-COLLECTION-MAP-FILTER-P3-implementation-proof-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P3.md; next: LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P4 Rust parity + canonical SIR names) | LANG-STDLIB-SUM-PROP-P1 CLOSED (PROPOSAL AUTHORED / P2 IMPLEMENTATION PLANNING REQUIRED — stdlib.collection.sum two-arg field-projection form; ENTRY CONTRACT: canonical_name=stdlib.collection.sum; semantic_ir_name==canonical_name; legacy_sir=null; alias=sum (source_alias); type_params=[T,F]; input=Collection[T]+Symbol; output=F (declared field type); CORE/pure/authority_surface:none/deterministic:true/totality:partial; CORE INSIGHT: return type F is declared field type from @type_shapes lookup — scale-preserving for Decimal[N] without arithmetic type inference; sum(leads,:bid_decimal) returns Decimal[2] because Lead.bid_decimal:Decimal[2] in type_shapes (no numeric operator rules at TC time); SPLIT A ONLY: two-arg field-projection accepted; Split B one-arg sum(Collection[T])->T BLOCKED (absent from stdlib spec; Rust scale-stripping bug; requires LAB-STDLIB-NUMERIC-P1/STAB-P4); DESIGN DECISIONS: bare sum alias accepted; non-Symbol second arg → OOF-COL5; missing field → OOF-COL5 (no silent Decimal fallback unlike Rust TC); Unknown first arg → return Unknown (permissive); no numeric constraint on field type in v0; empty-collection type is F (not Option[F]); runtime value deferred to P2; sumBy explicitly deferred; fold-derivation noted (implementation independent); TOOLCHAIN: Ruby TC OOF-TY0 for both forms — P2 adds infer_sum_call using element_type_from_collection + @type_shapes lookup; second arg is Symbol not lambda — different dispatch pattern from COLLECTION_HOF_FNS (P2 decides: extend HOF_FNS with has_symbol flag vs separate COLLECTION_SUM_FNS); SIR: Ruby will emit qualified "stdlib.collection.sum"; Rust TC two-arg working/scale-preserving but SIR fn name bare "sum" (parity gap); OOF-COL1 arity / OOF-COL2 non-Collection first arg / OOF-COL5 sum-specific; SPEC NOTE: stdlib/collections.ig declares Decimal[S] return — proposal generalizes to F (declared field type) to accommodate Integer fields; open design point for P2; AUTHORITY CLOSED: no impl / no Rust / no VM / no inventory / no one-arg sum / no sumBy / no Decimal operator / no fold / no numeric constraint / no identity element / no app fixtures; proposal: igniter-lang/.agents/work/proposals/LANG-STDLIB-SUM-field-projection-sum-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-SUM-PROP-P1.md; next: LANG-STDLIB-SUM-PROP-P2 (implementation planning) / LAB-STDLIB-NUMERIC-P1 (gates Split B)) | LANG-STDLIB-FOLD-PROP-P3 CLOSED (IMPLEMENTATION PROOF — PROVED 52/52 PASS; bounded Ruby canon implementation of stdlib.collection.fold; TWO INSERTIONS: (1) when "fold" dispatch arm in infer_call after when "sum"; (2) infer_fold_call private method after infer_lambda_body before infer_or_else (~55 lines); FOLD NOT IN COLLECTION_HOF_FNS: structural incompatibility — 3 args / 2-param lambda / Acc scalar result; SEED TYPE: infer_expr(args[1]) handles literals/refs/calls; 2-PARAM BINDING: symbol_types.merge(params[0] => acc_type, params[1] => elem_type); LAMBDA BODY: infer_lambda_body unchanged — block-body (Bool condition if-else) proven working; OOF-COL4: arity/non-Collection/non-lambda/param-count (early-return) + return-mismatch (non-early, acc_type returned); Unknown permissive; SIR: "stdlib.collection.fold" inline — zero emitter changes; fold_stream T3 path unaffected (G-01..G-05 all PASS); TC ARITHMETIC GAP: TC + always returns Integer — bookkeeping body acc+0.00 gives OOF-COL4 (expected, proves dispatch reached body check); ERP fold (block-body) gets zero OOF-COL4; MAP/FILTER RUNNER UPDATED: H-01/H-02 adapted for fold+sum dispatch; G-03 global OOF-COL1 check narrowed; 61/61 still PASS; REGRESSIONS: collection_map_filter_p3 61/61 + stdlib_outcome_p3 60/60 + string_core PASS; SECTIONS: A regression 7 / B basics 8 / C lambda inference 7 / D OOF-COL4 8 / E SIR names 5 / F app fixtures 6 / G fold_stream coexistence 5 / H authority 6; authority closed: no inventory / no Rust / no VM / no fold_stream / no OOF beyond OOF-COL4; proof runner: verify_stdlib_fold_p3.rb; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-FOLD-PROP-P3.md; next: LANG-STDLIB-FOLD-PROP-P4 inventory amendment + Rust parity) | LANG-STDLIB-FOLD-PROP-P2 CLOSED (IMPLEMENTATION PLANNING — stdlib.collection.fold Ruby TC; PLANNING DECISION: READY FOR P3; ONE FILE: typechecker.rb only; TWO INSERTIONS: (1) when "fold" dispatch arm after COLLECTION_HOF_FNS arm in infer_call; (2) infer_fold_call private method after infer_lambda_body before infer_or_else (~55 lines); FOLD NOT IN COLLECTION_HOF_FNS: structural incompatibility — 3 args / 2-param lambda / Acc scalar result; SEED TYPE: infer_expr(args[1]) — handles literals/refs/calls; better than fold_stream literal-only pattern; 2-PARAM BINDING: symbol_types.merge(params[0] => acc_type, params[1] => elem_type); LAMBDA BODY: infer_lambda_body unchanged — block-body ERP fixture already works; OOF-COL4: arity/non-Collection/non-lambda/param-count (early-return) + return-mismatch (non-early, acc_type returned); Unknown permissive; SIR: "stdlib.collection.fold" inline — zero emitter changes; fold_stream T3 path unaffected; PROOF MATRIX: 52 checks 8 sections (A regression / B basics / C lambda inference / D OOF-COL4 / E SIR names / F app fixtures / G fold_stream coexistence / H authority); proof runner: verify_stdlib_fold_p3.rb; planning doc: igniter-lang/.agents/work/proposals/LANG-STDLIB-FOLD-P2-implementation-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-FOLD-PROP-P2.md; next: LANG-STDLIB-FOLD-PROP-P3 Ruby implementation proof) | LANG-STDLIB-FOLD-PROP-P1 CLOSED (PROPOSAL AUTHORED / P2 PLANNING REQUIRED — stdlib.collection.fold v0; 1 entry contract; CANONICAL: stdlib.collection.fold / source_alias: fold (bare, unqualified) / TYPE PARAMS: [T, Acc] / SIGNATURE: Collection[T]×Acc×((Acc,T)→Acc)→Acc / RESULT: Acc (scalar, NOT Collection[Acc]); ACC TYPING: seed literal type_tag bootstrap — fold_stream_result_type pattern already in Ruby TC (line 1622): args[1].fetch("type_tag","Unknown") / non-literal seed → Unknown (permissive); LAMBDA: inline only / exactly 2 params (acc,elem) / acc→Acc from seed / elem→T from element_type_from_collection / both single-expr and block-body supported / return type must equal Acc (OOF-COL4 strict; Unknown propagates permissively); OOF-COL4: arity≠3 / non-lambda 3rd arg / non-Collection 1st arg / lambda params≠2 / lambda return≠Acc; OOF-COL5 stays reserved for sum; FOLD_STREAM SEPARATION: handle_t3_variant (fold_stream) completely separate from infer_call; adding "fold" arm to infer_call does not touch T3 path; RUBY TC: OOF-TY0 "Unknown function: fold" (no dispatch); RUST TC: dispatched (line 3233) with seed-type result; lambda not evaluated; SIR bare "fold" (parity gap, not blocker); ENTRY CONTRACT: lifecycle_status proof-local / fragment_class core / purity pure / authority_surface none / deterministic true / semantic_ir_name==canonical_name / no legacy_sir / no emitter changes; INVENTORY EDITS: deferred to P2/P3 (after implementation proof); SUM STAYS SEPARATE (LAB-STDLIB-SUM-P1 independent); authority closed: no impl / no fold_stream changes / no sum / no Rust / no VM / no inventory edits / no Fn type / no app fixtures; proposal: LANG-STDLIB-FOLD-collection-fold-v0.md; card: LANG-STDLIB-FOLD-PROP-P1.md; next: LANG-STDLIB-FOLD-PROP-P2 / LAB-STDLIB-SUM-P1 / LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P3) | LAB-STDLIB-SUM-P1 CLOSED (READINESS PROOF — PROVED 46/46 PASS; VERDICT: SPLIT-NUMERIC — Split A (accepted): two-arg sum(Collection[T], Symbol)->DeclaredFieldType ready for proposal; Split B (blocked): one-arg sum(Collection[T])->T deferred; CALL FORM SURVEY: two forms in use — ledger.ig lines 9/13 sum(debit_amounts)/sum(credit_amounts) one-arg on Collection[Decimal[2]]; stdlib_extension.ig lines 26-37 sum(leads,:bid_decimal) two-arg on Collection[Lead] where bid_decimal:Decimal[2]; NO app uses sum(Collection[Integer]) — ACCEPT-INTEGER-ONLY ungrounded; STDLIB SPEC: collections.ig defines ONLY two-arg form sum(Collection[T], field: Symbol)->Decimal[S]; one-arg form absent from spec; no sumBy; RUBY TC: zero dispatch for either form — OOF-TY0 for sum(Collection[Integer])/sum(Collection[Decimal[2]])/sum(Collection[Lead],:bid_decimal)/sumBy; no NUMERIC_MEASURE_BUILTINS sum entry; no when "sum" arm; RUST TC (line 2667): two-arg form extracts field type from type_shapes via get_param → scale-preserving (sum(leads,:bid_decimal) where bid_decimal:Decimal[2] → Decimal[2]); one-arg form returns bare Decimal — SCALE STRIPPING GAP (sum(Collection[Decimal[2]]) → Decimal not Decimal[2]); SIR fn name bare "sum" not qualified (parity gap); SPLIT A BASIS: two-arg spec'd + Rust-tested + scale-preserving + type-lookup-only (no arithmetic inference needed) + Ruby dispatch solvable (COLLECTION_HOF_FNS pattern has_lambda:false arity:2); SPLIT B BLOCKS: (1) absent from stdlib spec (2) Rust scale-stripping bug (3) numeric type constraint undefiled (4) scale propagation rule unsettled (5) identity element unspecified; FOLD RELATIONSHIP: sum is fold-derivable (sum(coll)=fold(coll,0,(acc,x)->acc+x); sum(coll,:field)=fold(coll,0,(acc,x)->acc+x.field)); fold ACCEPT (LAB-STDLIB-FOLD-P1); independent implementation justified — Ruby TC has no fold yet; Rust TC dispatches both independently; SUMBY: explicitly deferred — no sumBy in any fixture or spec; two-arg sum IS sum-by-field but uses sum name; EMPTY COLLECTION: no Option wrapper in spec; no identity element defined; non-empty by bookkeeping domain invariant; deferred to LANG-STDLIB-SUM-PROP-P1; SECTIONS: A spec-check 5 / B fixture-survey 8 / C ruby-diagnostics 8 / D rust-dispatch 6 / E integer-evidence 4 / F decimal-evidence 4 / G empty-collection 3 / H fold-relationship 3 / I sumBy+authority 5; authority closed: no Ruby impl / no Rust impl / no VM / no inventory / no app fixture / no fold / no Decimal operator / no sumBy / no public API; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_stdlib_sum_p1.rb; doc: igniter-lab/lab-docs/governance/lab-stdlib-sum-readiness-v0.md; card: igniter-lab/.agents/work/cards/governance/LAB-STDLIB-SUM-P1.md; next: LANG-STDLIB-SUM-PROP-P1 (Split A two-arg form) / LAB-STDLIB-NUMERIC-P1 (gates Split B)) | LAB-STDLIB-FOLD-P1 CLOSED (READINESS PROOF — PROVED 50/50 PASS; VERDICT: ACCEPT — stdlib.collection.fold ready for proposal authoring; APP PRESSURE: bookkeeping/ledger.ig fold(txs, 0.00, (acc,tx)->acc+0.00) DUMMY Decimal seed / erp_logistics/optimizer.ig fold(matching_routes, 999999.0, (acc,r)->if r.cost_per_kg<acc{...}) Float seed; BOTH: 3-arg bare fold / 2-param inline lambda / no named fn refs; RUBY TC: fold → OOF-TY0 "Unknown function: fold" (else arm infer_call); fold_stream (T3 path) separate and unaffected; no fold arm in TEXT_STDLIB_FNS/MAP_STDLIB_FNS/OUTCOME_STDLIB_FNS; RUST TC: fold dispatched at typechecker.rs:3233; is_resolved=true; result=typed_args[1].resolved_type (seed type); no lambda type check / no arity check / bare SIR name "fold" (not stdlib.collection.fold); ACCUMULATOR TYPING: seed literal type_tag bootstrap — fold_stream_result_type (line 1622) already uses this pattern: args[1].fetch("type_tag","Unknown"); element_type_from_collection exists (~line 1825) for T extraction; collection_type_ir_from exists (~line 2301); CANONICAL SIGNATURE: Collection[T] × Acc × ((Acc,T)→Acc) → Acc; RESULT TYPE: Acc (scalar, not Collection[Acc]); LAMBDA: 2-param extension of infer_lambda_body (P3); bind acc_param→acc_type + elem_param→elem_type via symbol_types.merge; fold_stream coexistence: no conflict — T3 path in handle_t3_variant separate from infer_call; OOF-COL4 reserved (fold-family errors) / OOF-COL5 reserved (sum, stays separate); fold is CORE-only/pure/deterministic/authority_surface:none; sum stays separate (not derivable from fold in v0 — numeric constraints + ergonomics); SECTIONS: A inventory 6 / B app-scan 6 / C ruby-diags 8 / D rust-diags 6 / E acc-typing 6 / F lambda-shape 6 / G sig+OOF 6 / H closed-surfaces 6; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_stdlib_fold_p1.rb; doc: igniter-lab/lab-docs/governance/lab-stdlib-fold-readiness-v0.md; card: igniter-lab/.agents/work/cards/governance/LAB-STDLIB-FOLD-P1.md; authority closed: no impl / no sum / no map/filter/count changes / no recursion / no app fixtures / no VM / no inventory; next: LANG-STDLIB-FOLD-PROP-P1 / LAB-STDLIB-SUM-P1 / LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P3) | LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P2 CLOSED (IMPLEMENTATION PLANNING — READY FOR P3; all three helpers (map/filter/count) in one slice; no SPLIT required; ONE FILE: typechecker.rb only; THREE INSERTIONS: (1) COLLECTION_HOF_FNS constant after OUTCOME_STDLIB_FNS {map: arity 2/has_lambda true, filter: arity 2/has_lambda true, count: arity 1/has_lambda false}; (2) when *COLLECTION_HOF_FNS.keys arm in infer_call after OUTCOME arm; (3) infer_collection_hof_call+infer_lambda_body private methods ~80-90 lines; LAMBDA BINDING: element_type_from_collection → symbol_types.merge(param => elem_type) → infer_expr(body, local_symbols); RETURN TYPES: map=collection_type_ir_from(body_type) / filter=collection_type_ir_from(elem_type) passthrough / count=Integer; SIR names: qualified "stdlib.collection.{fn}" automatic via generic semantic_expr; zero emitter changes; COUNT T3: no conflict — T3 in handle_t3_variant separate from infer_call; OOF-COL1 arity+non-lambda / OOF-COL2 non-Collection / OOF-COL3 filter-non-Bool (P3 decision); REGRESSION: string_core+typed_ref_p5+stdlib_outcome_p3; PROOF: ≥56 checks 8 sections; proof runner: experiments/stdlib_collection_proof/verify_stdlib_collection_map_filter_p3.rb; no inventory edits in P3; authority closed: no impl/Rust/VM/fold/sum/predicate-count/named-fn-refs/app-fixtures; planning doc: LANG-STDLIB-COLLECTION-MAP-FILTER-P2-implementation-planning-v0.md; card: LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P2.md; next: LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P3) | LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P1 CLOSED (PROPOSAL AUTHORED / P2 PLANNING REQUIRED — stdlib.collection.map + stdlib.collection.filter + stdlib.collection.count v0; 3 entry contracts defined; all pure/core/authority_surface:none/semantic_ir_name==canonical_name; ACCEPTED aliases: map/filter/count (bare, unqualified, source_alias kind); SIGNATURES: map=Collection[T]×(T→U)→Collection[U] / filter=Collection[T]×(T→Bool)→Collection[T] / count=Collection[T]→Integer; INLINE LAMBDA ONLY: no Fn[T,U] type; element_type_from_collection reused (~line 1825); COUNT SEMANTICS: count-all-elements only; predicate-count deferred; count amended not replaced (lifecycle_status=production-implemented preserved); RUBY TC: zero dispatch for map/filter (OOF-TY0); count regular-call OOF-TY0 (T3-only gap clarified); P2 adds COLLECTION_HOF_FNS+infer_collection_hof_call following OUTCOME_STDLIB_FNS pattern; RUST TC: all three dispatched; map lambda param Integer gap + bare SIR name gap documented; OOF-COL namespace reserved: COL1=arity / COL2=non-Collection-first-arg active in P2; COL3=filter-non-Bool P2 decision; COL4/COL5 deferred; authority closed: no Ruby impl / no Rust impl / no VM / no inventory edits / no fold / no sum / no app fixtures; proposal: igniter-lang/.agents/work/proposals/LANG-STDLIB-COLLECTION-MAP-FILTER-collection-map-filter-count-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P1.md; next: LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P2 / LAB-STDLIB-FOLD-P1 / LAB-STDLIB-SUM-P1) | LAB-STDLIB-COLLECTION-P1 CLOSED (READINESS + PROOF — PROVED 64/64 PASS; VERDICT: SPLIT — Group A map+filter+count-parity (LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P1) / Group B fold (LAB-STDLIB-FOLD-P1) / Group C sum (LAB-STDLIB-SUM-P1); APP PRESSURE: bookkeeping filter×2+map×2+sum×2+fold / spreadsheet map / ERP filter+fold — all bare unqualified names, no qualified or domain aliases; RUBY TC: zero HOF dispatch for map/filter/fold/sum — all OOF-TY0 Unknown function; NEW FINDING: count T3-only gap — NUMERIC_MEASURE_BUILTINS dispatched only in handle_t3_variant (decreases count context), NOT infer_call — regular count(items) also OOF-TY0 in Ruby; READINESS PRECONDITION: element_type_from_collection(collection_type) exists in Ruby TC at ~line 1825 — exact primitive needed for lambda param binding in map/filter/fold; no general Fn[T,U] type required; RUST: dispatches count/sum/filter|take/map/fold as regular calls; RUST GAP: map lambda params hardcoded as Integer (not element type T) — field accesses on lambda param silently Unknown; INVENTORY: count present (production-implemented dual-toolchain T3-only) + concat (orphaned single-toolchain); map/filter/fold/sum absent; TYPE SIGNATURES: map=Collection[T]×(T→U)→Collection[U] / filter=Collection[T]×(T→Bool)→Collection[T] / fold=Collection[T]×U×((U,T)→U)→U / sum=Collection[T]→T where T:Numeric / count=Collection[T]→Integer regular-call parity; PROOF: 9 sections A-I (inventory 8 / source-scan 8 / ruby-diagnostics 8 / rust-diagnostics 6 / ruby-TC-source 8 / rust-TC-source 6 / signatures 6 / classification 6 / inline-fixtures 8); authority closed: no Ruby TC impl / no Rust impl / no VM / no lambda type-system / no inventory edits / no app fixture changes; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_stdlib_collection_p1.rb; doc: igniter-lab/lab-docs/governance/lab-stdlib-collection-pressure-readiness-v0.md; card: igniter-lab/.agents/work/cards/governance/LAB-STDLIB-COLLECTION-P1.md; next: LANG-STDLIB-COLLECTION-MAP-FILTER-PROP-P1 / LAB-STDLIB-FOLD-P1 / LAB-STDLIB-SUM-P1) | LAB-RUBY-FUNCTION-RECURSION-P2 CLOSED (BOUNDED RUBY IMPLEMENTATION — PROVED 58/58 PASS; SCC-based OOF-L4 for Ruby canon def functions; RULE: every member of a nontrivial SCC must declare decreases fuel; IMPLEMENTATION: replaced self-only fn_self_recursive? OOF-L4 loop with Tarjan's SCC gate in typechecker.rb; NEW PRIVATE METHODS: fn_extract_all_calls+fn_collect_calls_expr+fn_collect_calls_body+tarjan_sccs; PRESERVED: fn_self_recursive?/fn_body_has_call?/fn_expr_has_call? all kept; DETERMINISM: sorted input nodes + sorted adj + sorted SCC members; PURE MUTUAL BUG CLOSED: ping/pong mutual pair now both receive OOF-L4 (was silently accepted in P1); P1 PROOF UPDATED: E-04+G-05 inverted from gap-documenting to SCC-correct; P1 POST-UPDATE: 52/52 PASS; SPREADSHEET: SS-P02 minimal fix still sufficient; SS-P03 SCC-complete (eval_ref also needs fuel under SCC rule — proved E-07/E-08); AUTHORITY CLOSED: no Rust/parser/VM/new-OOF/keyword/spreadsheet/stdlib; 9 sections A-I (Tarjan 10 / non-recursive 4 / self-recursive 6 / pure mutual 8 / mixed/complex 8 / per-SCC rule 7 / OOF-L4 trigger 5 / call boundary 3 / P1 regression 7); fixtures: igniter-lang/experiments/function_recursion_proof/p2_fixtures/{pure_mutual_no_evidence,pure_mutual_partial_evidence,pure_mutual_full_evidence,disconnected_sccs}.ig; proof: igniter-lang/experiments/function_recursion_proof/verify_lab_ruby_function_recursion_p2.rb; doc: igniter-lab/lab-docs/lang/lab-ruby-function-recursion-scc-oof-l4-parity-proof-v0.md; card: igniter-lang/.agents/work/cards/lang/LAB-RUBY-FUNCTION-RECURSION-P2.md) | LAB-FUNCTION-RECURSION-P4 CLOSED (BOUNDED RUST IMPLEMENTATION — PROVED 80/80 PASS; SCC-based OOF-L4 gate live in igniter-lab/igniter-compiler/src/typechecker.rs; P2 CORRECTNESS BUG FIXED: pure mutual recursion A→B→A without decreases fuel now emits OOF-L4 on both members (was silently ok); FILE CHANGED: typechecker.rs only; NEW FUNCTIONS: collect_fn_calls+block_collect_calls+expr_collect_calls (call-graph builder; filters to known def fn names only; sorted Vec output) + TarjanScc struct+visit+tarjan_sccs (Tarjan's O(V+E); three determinism guarantees: sorted input nodes/sorted neighbors/sorted SCC members); NONTRIVIAL SCC: scc.len()>1 OR single-node self-loop; call graph filters to known def fn names — unknown calls (stdlib, types, contracts) create no false SCCs; BEHAVIORAL CHANGE TABLE: non-recursive=unchanged / self-recursive=unchanged / pure-mutual-no-fuel=FIXED (ok→oof) / pure-mutual-partial=FIXED (ok→oof on missing) / pure-mutual-all-fuel=unchanged / three/four-way-cycle=FIXED / disconnected-SCCs=each-checked-independently; P2 RUNNER UPDATED: 9 checks documented old buggy behavior → updated to document fixed behavior; P2=42/42 PASS (updated) / P3=60/60 PASS (unchanged) / P4=80/80 PASS; SS-P02 no longer sufficient under P4 — SS-P03 (both eval_expr and eval_ref annotated) required; cargo build --release PASS; AUTHORITY CLOSED: no Ruby impl / no parser changes / no new OOF codes / no VM / no spreadsheet edits; is_recursive preserved; NEXT: LAB-RUBY-FUNCTION-RECURSION-P2 Ruby SCC parity → LAB-SPREADSHEET-RECURSION-FOLLOWUP-P1; fixtures: p4_{unknown_calls,four_way_cycle,four_way_all_annotated,dag_with_helpers}.ig; proof: verify_lab_function_recursion_p4.rb (10 sections A-J); doc: lab-function-recursion-rust-scc-oof-l4-implementation-v0.md; card: LAB-FUNCTION-RECURSION-P4.md) | LAB-FUNCTION-RECURSION-P3 CLOSED (LAB PROOF + IMPLEMENTATION SPEC — PROVED 60/60 PASS; SCC detection spec for def function recursion; DECISION: ACCEPT per-SCC rule; IMPLEMENTATION-READY for P4; TOOLCHAIN CORRECTION (from P1): Ruby typechecker DOES have OOF-L4 for def functions at typechecker.rb:142-151 using fn_self_recursive? — self-only gap is SYMMETRIC across both Rust and Ruby; ALGORITHM: Tarjan's SCC with three determinism guarantees (sorted input nodes, sorted neighbors, sorted SCC members); OOF-L4 TRIGGER (per-SCC): "F is in a nontrivial SCC AND F lacks decreases fuel"; FULL CASE MATRIX (10 patterns) all proved; NO REGRESSION: C-06/F-06 prove per-SCC ≡ per-function for self-recursive-only; RUST INSERTION POINT: replace typechecker.rs:357-369 is_recursive loop; new fn extract_fn_calls+tarjan_sccs_sorted; RUBY INSERTION POINT: replace typechecker.rb:142-151 fn_self_recursive? loop; new methods fn_extract_all_calls+tarjan_sccs; SPREADSHEET: SS-P02=decreases fuel on eval_expr; SS-P03=eval_ref also; AUTHORITY CLOSED: no impl/parser/syntax/OOF/VM/spreadsheet; NEXT: LAB-FUNCTION-RECURSION-P4 Rust impl → LAB-RUBY-FUNCTION-RECURSION-P2 Ruby parity → LAB-SPREADSHEET-RECURSION-FOLLOWUP-P1; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_function_recursion_p3.rb; doc: lab-docs/governance/lab-function-scc-detection-implementation-spec-v0.md; card: LAB-FUNCTION-RECURSION-P3.md) | LANG-STDLIB-ENTRY-CONTRACT-P3 CLOSED (IMPLEMENTATION / DATA + PROOF ONLY — PROVED 76/76 PASS; hand-authored stdlib inventory JSON `igniter-lang/docs/spec/stdlib-inventory.json` with 24 entries; text×14 (production-implemented/design-locked/dual-toolchain; exact P2 set; all pure/authority_surface:none; byte/rune/grapheme length+slice triads) + map×4 (PROP-043; map.empty Unknown inference note recorded; OOF-MAP diagnostics lineage) + stdlib.option.or_else (sole legacy_sir="or_else"; semantic_ir_name="or_else" records current bare emission; P4 migration target) + stdlib.collection.count (experiment-pass; NUMERIC_MEASURE_BUILTINS/T3 dispatch path) + 4 orphans (stdlib.bool.and/stdlib.integer.gt via operator lowering; stdlib.option.wrap/stdlib.collection.concat Rust-only; all sketch/orphaned/triage-required); DIGEST ALGORITHM PROVED: stdlib_surface_digest = sha256(canonical_json(entries sorted by canonical_name, entry_digest stripped)) — deterministic across shuffled entry order and raw whitespace changes; alias-change sensitive; B-01..B-08 all PASS; NAMING INVARIANTS: exactly one non-null legacy_sir (stdlib.option.or_else); semantic_ir_name == canonical_name for all others; no short names as canonical; alias kinds within vocabulary; BIDIRECTIONAL DISPATCH (H-01..H-08): every Ruby TEXT_STDLIB_FNS/MAP_STDLIB_FNS/or_else/count dispatch key resolves to registry canonical or alias; every production entry has known dispatch site; operator-lowered orphans visible; SECTION I (live fixture): compile unavailable in runner context — skipped gracefully via compile_skipped? guard; H section covers dispatch consistency via source inspection; not a proof gap; NO COMPILER CODE CHANGES: parser/typechecker/emitter/assembler/VM all untouched; J-01..J-04 PASS; stdlib_surface_digest embedding deferred to P4; P4 scope: embed digest + or_else migration to qualified name with dual-accept window + Rust toolchain parity verification + live fixture regression; proof: igniter-lang/experiments/stdlib_entry_contract_proof/verify_stdlib_entry_contract_p3.rb; inventory: igniter-lang/docs/spec/stdlib-inventory.json; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-ENTRY-CONTRACT-P3.md; proof packet: igniter-lang/.agents/work/proposals/LANG-STDLIB-ENTRY-CONTRACT-P3-inventory-proof-v0.md) | LAB-RUBY-FUNCTION-RECURSION-P1 CLOSED (BOUNDED IMPLEMENTATION — PROVED 52/52 PASS; Ruby OOF-L4 parity for self-recursive `def` functions; THREE PIPELINE GAPS CLOSED: (1) parser did not parse `decreases fuel` on def function declarations; (2) classifier did not propagate `functions` array to classified result; (3) typechecker had no OOF-L4 check for def functions; ALL THREE NOW FIXED — Ruby emits OOF-L4 for self-recursive def functions missing `decreases fuel`, matching Rust typechecker.rs:357-370; PARSER: `decreases fuel` parsed between return type and `{`; `fuel` tokenizes as :ident (not in KEYWORDS); `decreases` key added to function AST node when present; CLASSIFIER: `functions` passed through to classified program hash (same pattern as contracts); TYPECHECKER: `fn_self_recursive?` + `fn_body_has_call?` + `fn_expr_has_call?` — self-call-only detection matching Rust `is_recursive()`; Ruby AST key conventions: `"fn"` (not `"fn_name"`), `"if_expr"/"then"/"else"` (not `"if"`); REGRESSION CONFIRMED: contract OOF-R1..R7 unaffected / loop OOF codes unaffected / non-recursive functions 0 errors; AUTHORITY CLOSED: no mutual recursion / no SCC detection / no VM / no Rust changes / no new OOF codes / no max_steps gate; OPEN FOR P3: SCC-level Ruby mutual recursion detection (parity with LAB-FUNCTION-RECURSION-P3) / OOF-L4-MUTUAL canonical code / max_steps for def functions; 4 fixtures (non_recursive/self_recursive_no_evidence/self_recursive_with_fuel/non_recursive_caller); 7 sections A-G (parser 8 / classifier 7 / typechecker 11 / regression 7 / parity 7 / traversal 7 / authority 5); proof: igniter-lang/experiments/function_recursion_proof/verify_lab_ruby_function_recursion_p1.rb; doc: igniter-lab/lab-docs/lang/lab-ruby-function-recursion-parity-proof-v0.md; card: igniter-lang/.agents/work/cards/lang/LAB-RUBY-FUNCTION-RECURSION-P1.md) | LAB-FUNCTION-RECURSION-P2 CLOSED (LAB PROOF — PROVED 42/42 PASS; mutual recursion SCC readiness proof; EMPIRICAL METHOD: Rust lab compiler invoked on 5 fixture files + 2 inline cases via Open3; JSON diagnostic output checked for OOF-L4; HEADLINE FINDING: pure mutual recursion A→B→A with NO self-calls compiles SILENTLY — status ok, zero diagnostics (Case 3 = CORRECTNESS BUG); ROOT CAUSE: is_recursive(body, fn_name) checks only if fn_name appears as direct self-call in body — pure mutual pairs have false/false → OOF-L4 loop never entered for either function; CASES 3/4/5 IDENTICAL: whether 0/1/2 mutual functions have `decreases fuel` annotation — all produce status:ok/zero-diagnostics; annotation on pure mutual-only functions is INERT (parsed, not validated); CLASSIFICATION: Case1=correct (self-rec gated) / Case2=correct (fuel accepted) / Case3=CORRECTNESS BUG (pure mutual silent, honesty violation) / Case4=bounded gap (partial annotation unvalidated) / Case5=correct-intent/wrong-reason (both annotated but unvalidated) / Mixed-no-dec=correct (self part gated) / Mixed-A-dec=bounded gap (partner ungated); SCC RECOMMENDATION: PER-SCC (Option B) — all members of any non-trivial SCC must have decreases fuel; algorithm Tarjan O(V+E); Option A per-function has correctness bug; Option C defer REJECTED as honesty violation (OOF-L4 gate exists but bypassable via mutual indirection = structural bypass of a safety gate); SPREADSHEET MAPPING: eval_expr↔eval_ref = MIXED SCC (not pure mutual) — eval_expr is self-recursive+cross-calls eval_ref, eval_ref calls eval_expr only; NOT Case 3 (bounded gap not bug); SS-P02 minimal fix = decreases fuel on eval_expr removes OOF-L4; SS-P03 under per-SCC = decreases fuel on eval_ref also; OPEN QUESTIONS FOR P3: Tarjan's vs Kosaraju's algorithm / OOF-L4 vs OOF-L4-MUTUAL code / cross-module SCCs deferred / max_steps orthogonal HOLD from P1; NEXT: LAB-FUNCTION-RECURSION-P3 SCC detection implementation design proof (Tarjan's proof-local model, full case matrix validation, Rust+Ruby typechecker change spec, ≥50 checks, implementation plan); fixtures: igniter-lab/igniter-view-engine/fixtures/function_recursion/p2_case{1..5}_*.ig; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_function_recursion_p2.rb; doc: lab-docs/governance/lab-function-mutual-recursion-scc-readiness-proof-v0.md; card: .agents/work/cards/governance/LAB-FUNCTION-RECURSION-P2.md) | LAB-FUNCTION-RECURSION-P1 CLOSED (LAB PROOF — PROVED 66/66 PASS; function-level managed recursion boundary proof; HEADLINE: OOF-L4 is the ACTUAL canonical diagnostic (already in Rust at typechecker.rs:357-370, NOT a new code) — `decreases fuel` required between return type and `{` in self-recursive def functions; SYNTAX: `def name(params) -> ReturnType decreases fuel { body }` (FunctionDecl.decreases already parsed, no parser changes needed); is_recursive() SELF-ONLY = mutual recursion gap confirmed (SS-P03): eval_ref calls eval_expr but does not call itself → Rust does NOT flag it → mutual cycle undetected; PROOF-LOCAL SCC MODEL: bidirectional BFS finds mutual SCCs; safe rule = ALL SCC members must declare `decreases fuel`; eval_expr+eval_ref form :mutual group, both must have evidence for SCC-completeness; TERMINATION EVIDENCE: :fuel accepted; nil → OOF-L4; :structural HOLD (T2 size-relation extension required); NO max_steps requirement for def functions (gap vs fuel_bounded contract which requires max_steps N — P2 decision); RUBY PARITY GAP: Ruby typechecker.rb has zero OOF-L4 check for def functions (only contract-level OOF-R* implemented) — separate authorized implementation work; DECISION TABLE: ACCEPT decreases fuel for self-recursive def / ACCEPT SCC-level evidence as safe model design recommendation / HOLD structural decrease (T2 extension) / HOLD max_steps requirement (P2 decision) / OOF-L4 canonical for self; OOF-L4-MUTUAL proof-local label for mutual gap; SPREADSHEET UNBLOCKING: SS-P02 minimal fix = add `decreases fuel` to eval_expr declaration (one keyword between return type and `{`); SS-P03 recommendation = also add `decreases fuel` to eval_ref for SCC-completeness (not currently required by Rust but recommended by safe model); 4 fixture files (non_recursive/self_recursive_fuel/mutual_recursive_fuel/spreadsheet_eval_pair); 10 sections A-J (inventory 6 / recursion-graph 8 / termination-evidence 8 / positive-self 7 / positive-mutual 7 / negative 8 / spreadsheet-mapping 7 / contract-recur-relation 5 / authority-closed 5 / decision 5); proof: igniter-lab/igniter-view-engine/proofs/verify_lab_function_recursion_p1.rb; fixtures: igniter-lab/igniter-view-engine/fixtures/function_recursion/; doc: lab-docs/governance/lab-function-managed-recursion-boundary-proof-v0.md; card: .agents/work/cards/governance/LAB-FUNCTION-RECURSION-P1.md; open questions for P2: max_steps for def functions / SCC-level is_recursive extension / canonical code for mutual gap / Ruby parity) | LANG-STDLIB-OUTCOME-PROP-P3 CLOSED (IMPLEMENTATION — PROVED 60/60 PASS; bounded Ruby TypeChecker implementation of stdlib.outcome helpers; THREE INSERTIONS to typechecker.rb only — OUTCOME_STDLIB_FNS constant (7 entries after MAP_STDLIB_FNS) + when *OUTCOME_STDLIB_FNS.keys dispatch case in infer_call (after MAP case) + infer_outcome_call shared private method (after infer_map_empty); ZERO OTHER FILE CHANGES — semanticir_emitter.rb/parser.rb/assembler.rb/VM/runtime all untouched; SIR NAMING: confirmed zero emitter changes via E-01..E-07 all PASS; TypeChecker sets fn=qualified_canonical_name; generic semantic_expr preserves it verbatim; DIAGNOSTICS: OOF-OUT1 fires for non-Map argument (first outcome-specific diagnostic code; message includes canonical name + "Map[String, String]"); OOF-TY0 for arity≠1 (consistent with TEXT/MAP pattern); is_retryable/route/outcome_is_retryable/outcome_route → OOF-TY0 "Unknown function" (G-01..G-04 all PASS); DOMAIN-LOCAL PRESERVATION: TypeChecker does not inspect Map content; any Map[String,String] accepted for all helpers; return type always Bool or String regardless of map content (H-01..H-06 all PASS); ENTRY CONSISTENCY: all 7 OUTCOME_STDLIB_FNS qualified_names exactly match PROP-P1 canonical_names; source aliases follow outcome_<fn> prefix pattern (I-01..I-06 all PASS); SECTION RESULTS: A regression 6/6 / B dispatch 7/7 / C types 7/7 / D input validation 8/8 / E SIR naming 7/7 / F fixtures 6/6 / G authority 7/7 / H domain-local 6/6 / I consistency 6/6; REGRESSIONS: string_core_proof 60/60 PASS + verify_typed_contract_ref_p5 71/71 PASS + section A inline 6/6 PASS; FIXTURES: 6 .ig files under experiments/stdlib_outcome_proof/fixtures/; proof runner: experiments/stdlib_outcome_proof/verify_stdlib_outcome_p3.rb; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-OUTCOME-PROP-P3.md; next: LANG-STDLIB-OUTCOME-PROP-P4 Rust parity or inventory integration) | LANG-STDLIB-OUTCOME-PROP-P2 CLOSED (IMPLEMENTATION PLANNING — READY FOR NARROW P3; SCOPE=Option A TypeChecker+SIR only (stdlib-inventory.json does not yet exist — LANG-STDLIB-ENTRY-CONTRACT-P3 independent parallel track, not a gate); OUTCOME_STDLIB_FNS constant (7 entries; source aliases outcome_kind/outcome_is_denied/outcome_is_unknown_external_state/outcome_is_timed_out/outcome_is_system_error/outcome_is_query_error/outcome_is_partial_success following map_get prefix pattern; qualified canonical names stdlib.outcome.*); dispatch: when *OUTCOME_STDLIB_FNS.keys in infer_call after MAP case (~line 833); shared infer_outcome_call private method (~line 2090+); SIR NAMING: zero emitter changes — generic semantic_expr (lines 338–368 of semanticir_emitter.rb) preserves fn field verbatim; TypeChecker emits fn=qualified_name; semantic_ir_name==canonical_name satisfied automatically; DIAGNOSTICS: OOF-TY0 arity mismatch / OOF-OUT1 non-Map argument; OOF-OUT2..OUT4 reserved (not triggered in P3); missing kind = runtime concern deferred to P4+; ACCEPTED INPUT: Map (any params) or Unknown → PASS; non-Map → OOF-OUT1; AUTHORITY CLOSED: is_retryable/outcome_is_retryable/route/outcome_route → OOF-TY0 unknown-function; no Outcome[T,E]; no parser/emitter/VM/runtime changes; DOMAIN-LOCAL PRESERVATION (TC level): TC does not inspect Map content; any Map with any kind string accepted; return type always Bool/String; runtime false-return semantics deferred to P4+; PROOF MATRIX: ≥60 checks / 9 sections — A regression 6 / B registration 7 / C return type 7 / D input validation 8 / E SIR naming 7 / F fixture compilation 6 / G authority closed 7 / H domain-local 6 / I entry consistency 6; PROOF RUNNER: experiments/stdlib_outcome_proof/verify_stdlib_outcome_p3.rb; AUTHORIZED FILES: lib/igniter_lang/typechecker.rb + proof runner only; ENTRY CONTRACTS: 7 records fully specified in planning doc §7 — ready for stdlib-inventory.json when LANG-STDLIB-ENTRY-CONTRACT-P3 runs (vm_lowering_status: "not-lowered" all 7); P3 GATE CONDITIONS: LANG-STDLIB-ENTRY-CONTRACT-P2 closed ✓ / LAB-STDLIB-OUTCOME-P1 66/66 PASS ✓ / LANG-STDLIB-OUTCOME-PROP-P1 authored ✓ / planning closed ✓; planning doc: igniter-lang/.agents/work/proposals/LANG-STDLIB-OUTCOME-P2-implementation-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-OUTCOME-PROP-P2.md) | LANG-STDLIB-OUTCOME-PROP-P1 CLOSED (PROPOSAL AUTHORED — pending review; stdlib.outcome v0 governance boundary; CORE PRINCIPLE: outcome helpers classify observed outcome data — do not decide recovery, schedule retries, execute routes, or grant authority; SCOPE: kind accessor + 6 PROP-047 stable-term predicates; input Map[String,String] (KDR-like); pure Bool/String output; no Outcome[T,E]; no variants; no routing; ACCEPTED (7): stdlib.outcome.kind (passthrough, convention stability), stdlib.outcome.is_denied (experiment-pass; axis-1; FC-1≠query_error), stdlib.outcome.is_unknown_external_state (axis-5; FC-2≠timed_out/FC-3≠system_error; Covenant P15 reconcile-not-retry), stdlib.outcome.is_timed_out (axis-4; FC-2; does NOT discriminate dispatch context), stdlib.outcome.is_system_error (axis-3; retry-with-backoff), stdlib.outcome.is_query_error (axis-2; FC-1; do-not-retry), stdlib.outcome.is_partial_success (axis-6; FC-4..FC-7; promoted by PROP-047-P2 after LAB-FAILURE-TAXONOMY-P4 54/54); all entries: purity:pure/deterministic:true/authority_surface:none/fragment_class:core/status:proof-local/lowering:none/compatibility:pre-v1-none/semantic_ir_name==canonical_name; CONDITIONAL (deferred to P2): stdlib.outcome.is_retryable — cannot be kind-only (timed_out pre-dispatch=retryable vs post-dispatch=unknown-state-path; requires dispatch context; 3 design options: A=read dispatch_started from same record/B=context param/C=split helpers; axis-9 boundary: denied/query_error/unknown_ext/partial_success=false; system_error=true; timed_out=context-dependent; domain-local=false); REJECTED: stdlib.outcome.route (encodes policy=runtime authority; permanently closed); DOMAIN-LOCAL PRESERVATION: is_* returns false for non-matching kinds (no fallback/fuzzy); kind() always passthrough; domain-local kinds (found/rows/empty/created/conflict/redirect/still_unknown/confirmed_succeeded/confirmed_failed etc.) never absorbed; OOF-OUT1..OUT4 reserved; missing kind behavior deferred to P2; KDR+variant coexist (variant arm names not stable terms; variant helpers separate future work); authority closed: implementation/parser/TC/SIR/VM/runtime/Outcome[T,E]/FailureKind/variant enforcement/retry execution/scheduling/routing/OOF-OUT impl all closed; next: LANG-STDLIB-OUTCOME-PROP-P2 implementation planning (gated on this proposal); optional parallel: LAB-STDLIB-OUTCOME-P2 retryability context proof; proposal: igniter-lang/.agents/work/proposals/LANG-STDLIB-OUTCOME-outcome-helper-predicates-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-OUTCOME-PROP-P1.md) | LAB-STDLIB-OUTCOME-P1 CLOSED (LAB PROOF — PROVED 66/66 PASS; stdlib outcome helper pressure proof; HEADLINE: stdlib.outcome.is_<term> helpers CAN reduce stringly `kind` handling without collapsing domain-specific outcomes or granting runtime authority; 3-domain fixture inventory (Network/HTTP, Storage/Query, Epistemic/Reconciliation) — all 6 PROP-047 stable terms appear in ≥2 domains; domain-local kinds (found/empty/rows/created/redirect/still_unknown/confirmed_succeeded etc.) domain-exclusive and not absorbed by any helper; VERDICT TABLE: ACCEPT is_denied/is_unknown_external_state/is_timed_out/is_system_error/is_query_error/is_partial_success/kind (7 total); CONDITIONAL ACCEPT is_retryable (axis-9 only — denied/unknown_ext/post-dispatch-timed_out/query_error=false; system_error/pre-dispatch-timed_out=true; domain-local always false; boundary MUST be documented in entry contract totality field); REJECT route(outcome,policy) — encodes caller policy = runtime authority (H-05); KEY FINDINGS: F-1 helpers centralize stable-term strings (typo-prevention); F-2 domain-local kinds NOT absorbed (no fallback unknown; kind() always passes through unchanged); F-3 is_retryable axis-9 boundary honoring PROP-047 FC-1 (denied≠retryable) and Covenant P15 (post-dispatch-timed_out → unknown-state path → reconcile not retry); F-4 route() rejected because policy-encoding = authority grant; F-5 KDR and variant separate (helpers on KDR hashes; variant arm names like ConfirmedSucceededReal not stable terms); AUTHORITY CLOSED: stdlib impl/parser/TC/SIR/assembler/VM/runtime/canon outcome type/Outcome[T,E]/FailureKind enum/OOF codes/public API all closed; ENTRY CONTRACT SKETCHES: 3 representative helpers sketched using LANG-STDLIB-ENTRY-CONTRACT-P1 schema (canonical_name/purity:pure/authority_surface:none/deterministic:true/input_signature:Map[String,String]/output_signature:Bool|String/totality); OPEN QUESTIONS FOR P2: implementation form (pattern-match dispatch vs stdlib-inventory entries), is_retryable boundary documentation requirements, input-type precision (Map[String,String] vs Map[String,Any] for dispatch_started boolean), is_partial_success proof_lineage must cite LAB-FAILURE-TAXONOMY-P4; 10 sections A-J (inventory/helper-model/positive-stable-terms/domain-local-preservation/retry-safety/stringly-reduction/kdr-variant-boundary/authority/entry-pressure/decision); proof: igniter-lab/igniter-view-engine/proofs/verify_lab_stdlib_outcome_p1.rb; fixtures: igniter-lab/igniter-view-engine/fixtures/stdlib_outcome/{http_client_outcome,storage_query_outcome,epistemic_reconciliation_outcome}.ig; doc: lab-docs/governance/lab-stdlib-outcome-helper-pressure-proof-v0.md; card: .agents/work/cards/governance/LAB-STDLIB-OUTCOME-P1.md; next: LAB-STDLIB-OUTCOME-P2 implementation planning) | LANG-FORM-VOCABULARY-PROP-P2 CLOSED (IMPLEMENTATION PLANNING — READY FOR NARROW P3; SCOPE DECISION = P3-C phased: declaration/import/validation layer only; invocation (form word use in compute/output nodes) deferred to P4 because form-trigger/function-call disambiguation requires type-directed post-classify pass absent from current pipeline; parser: `speaks VocPath` + `vocabulary VocPath { trigger -> ContractRef }` both top-level declarations added to parse_top_decl; KEYWORDS adds `speaks`/`vocabulary`; program hash adds `vocabulary_declarations`/`vocabulary_imports`; classifier: propagates both fields without fragment class change (V-8 structural); typechecker: vocabulary_registry: + per_module_speaks: kwargs (default {} single-file compat); validate_vocabulary_layer post-contract-typecheck; V-2 → OOF-FORM4 / V-3+V-4 → OOF-FORM3 (order-independent: same trigger detected under both import orderings) / V-5 → OOF-FORM1 (form word target must resolve via same_module_registry or cross_module_registry) / V-7 → OOF-FORM7 (primitive shadow) / V-8 → OOF-FORM8 (unknown vocabulary) / V-9 → OOF-FORM9 (duplicate trigger within vocab) / OOF-FORM2/5/6 deferred to P4; MultifileResolver: build_vocabulary_registry + build_per_module_speaks (same pattern as build_cross_module_registry from P5); SemanticIR: form_vocabularies top-level [] field; typed_nodes nil filter for vocabulary kind; assembler: form_vocabulary_imports manifest field (sorted deterministically; absent when no speaks); dependency_edges UNCHANGED in P3; CompilerOrchestrator: 2 new kwargs forwarded from resolver; proof 57 checks 8 sections (A regression 6 / B parser-vocab 7 / C parser-speaks 5 / D tc-vocab 8 / E tc-speaks 9 / F SIR 7 / G manifest 7 / H authority 8); ~200–280 lines 7 files; planning doc: igniter-lang/.agents/work/proposals/LANG-FORM-VOCABULARY-P2-implementation-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-FORM-VOCABULARY-PROP-P2.md) | LANG-FORM-VOCABULARY-PROP-P1 CLOSED (PROPOSAL AUTHORED — pending review; explicit form vocabulary and order-independent resolution; governance boundary for named form words as conservative elaboration over typed-ref substrate; OOF-REF2 substrate gate lifted (P5 closed); `speaks VocabularyPath` preferred syntax direction at module level; `vocabulary { }` declaration at module level in owning module; V-1..V-9 coherence rules: V-1 no ambient leakage / V-2 ownership (T's declaring module or recognized vocabulary owner; module-name matching insufficient) / V-3 no first-wins (OOF-FORM3 fail-closed for ambiguous same-trigger imports) / V-4 import-order independence / V-5 typed-ref anchor required (OOF-FORM1) / V-6 InvocationIntent equality (form-lowered ≡ explicit call) / V-7 no_form propagation / V-8 fragment class invariant / V-9 language primitive reservation; TH-1..TH-6 all mapped: TH-1 conservativity (lab-proved in-module; cross-module P2 proof required) / TH-2 order-independence (proved unconditionally post-P5) / TH-3 skeleton stable / TH-4 hygiene (F-01..F-06 + V-9) / TH-5 resugaring (ResugaringTrace mandatory per lowering) / TH-6 eliminability (InvocationIntent equality); OOF-FORM1..OOF-FORM9 namespace reserved; SIR design shapes: form_vocabularies / form_resolutions / resugaring_trace / lowered_from_form flag; manifest: form_vocabulary_imports (informational, not dependency data); Rust orphan lineage: form_registry.rs/form_resolver.rs/FormKind×7/F-01..F-06 KEEP as lab references; AccumulatorRef/FormShape inheritance DEFERRED; MultiKeyword restricted to System/Stdlib (C-7); 13 required explicit answers all provided; Gap-I Form Constructor NOT obsoleted (separate track); view/UI forms NOT obsoleted (separate domain); next LANG-FORM-VOCABULARY-PROP-P2 implementation planning (parser speaks+vocabulary/classifier/typechecker V-1..V-9+OOF-FORM/SemanticIR/assembler; ≥50 checks 8 sections); parser/typechecker/SemanticIR/VM/runtime/public syntax/package/capability/call_contract/macro all closed; proposal: igniter-lang/.agents/work/proposals/LANG-FORM-VOCABULARY-explicit-dictionary-and-resolution-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-FORM-VOCABULARY-PROP-P1.md) | LANG-TYPED-CONTRACT-REF-PROP-P5 CLOSED (IMPLEMENTATION — PROVED 71/71 PASS; cross-module `uses` resolution live in Ruby canon pipeline; OOF-REF2 gap from P3/P4 closed; 3-path resolution: PATH 1 qualified (`uses Mod.Contract` → `resolution_kind: "qualified"`) / PATH 2a local-first with per_contract_module attribution check (local shadows imported, no diagnostic) / PATH 2b scan import scope (0→OOF-REF1, 1→`resolution_kind: "imported"`, ≥2→OOF-REF2 narrowed to genuine ambiguity only); MultifileResolver adds build_cross_module_registry/build_per_module_imports/build_per_contract_module from pre-merge units (merge architecture destroys attribution — must build before merge); TypeChecker receives all three as per-call kwargs (default {} preserves single-file compat); SIR resolution_kind field + short contract_name from resolved_ref (not full dotted target) + module_name for resolved refs; assembler from_module/to_module/resolution_kind on dependency_edges; DFS cycle detection for OOF-REF4; attach_source_units! nil-guard for type-error path; OOF-DECL-DUP-CONTRACT constraint documented (ambiguity tests use direct TypeChecker invocation); 11 cross-module fixtures; sections A–L (71 checks); regressions PROP-ENTRYPOINT-P3 53/53 embedded; TH-2 conditionality from LAB-FORM-VOCABULARY-P1 now satisfiable unconditionally — cross-module typed-ref anchor (module_name + resolution_kind) is live; parser/VM/runtime/forms/call_contract/capability/CLI closed; proof: experiments/typed_contract_ref_proof/verify_typed_contract_ref_p5.rb; card: igniter-lang/.agents/work/cards/lang/LANG-TYPED-CONTRACT-REF-PROP-P5.md; proof packet: igniter-lang/.agents/work/proposals/LANG-TYPED-CONTRACT-REF-P5-cross-module-implementation-proof-v0.md) | LANG-TYPED-CONTRACT-REF-PROP-P4 CLOSED (IMPLEMENTATION PLANNING — READY FOR P5; cross-module `uses` resolution plan; OOF-REF2 gap bounded and answered; 3 resolution paths: qualified `uses Mod.Contract` / imported unqualified `import Mod` + `uses Contract` / local unchanged; OOF-REF2 narrowed to ambiguous-import-only; OOF-REF1 extended to unknown-module in qualified ref + unknown-contract in known module; OOF-REF4 extended to cross-module uses-cycles; new SIR field `resolution_kind: "local"/"qualified"/"imported"/"unresolved"`; new manifest fields `from_module`/`to_module`/`resolution_kind` on `dependency_edges`; cross-module registry built by MultifileResolver classifying each unit before merge; TypeChecker `cross_module_registry:` kwarg default `{}` preserves single-file compat; selective imports respected (`import Mod.{ A }` scopes to A only); local shadows imported (silent); cycle detection at assembler time; 5 authorized files (multifile_resolver/typechecker/semanticir_emitter/assembler/compiler_orchestrator); proof matrix: ≥71 top-level checks across 12 sections + 3 embedded regression suites (PROP-ENTRYPOINT-P3 53, PROP-IMPORT-RESOLUTION-P5 99, LAB-TYPED-CONTRACT-REF-P1 58); parser/VM/runtime/forms/call_contract/capability/CLI closed; planning doc: igniter-lang/.agents/work/proposals/LANG-TYPED-CONTRACT-REF-P4-cross-module-resolution-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-TYPED-CONTRACT-REF-PROP-P4.md) | LAB-FORM-VOCABULARY-P1 CLOSED (LAB PROOF — PROVED 61/61 PASS; cross-module form vocabulary coherence under explicit vocabulary import model; HEADLINE: vocabulary model is coherent and order-independent — V-1..V-8 all mechanised; TH-2 CONDITIONALLY PROVED: cross-module coherence holds for any pair of modules importing the same explicitly-declared vocabulary (G-02/05); TH-3 CONFIRMED BY DESIGN: vocabulary adds words over existing FormKind productions, never new grammar productions (H-01..05); NEW DIAGNOSTIC: E-FORM-VOCAB-AMBIG (fail-closed when two vocabularies export same trigger with compatible arity); NEW RULE: E-FORM-VOCAB-NO-IMPORT (trigger not visible in module that did not import vocabulary — V-1 no ambient leakage); OWNER RULE V-2: ownership checked via VocabularyOwner.owned_contracts/owned_vocabularies — module name matching is NOT sufficient (F-03/06); ORDER INDEPENDENCE PROVED: non-conflicting trigger receipts identical under [A,B] and [B,A] import orderings; conflicting triggers produce identical E-FORM-VOCAB-AMBIG naming same vocabularies under both orderings — no first-wins (D-01..07); OOF-REF2 GAP EXPLICIT: cross-module ProofLocalContractRef.cross_module=true flags gap; vocabulary model sound given resolved refs, gap is in substrate not vocabulary layer (G-04/C-05); SYNTAX EVALUATION: Option A `speaks Query.Forms` preferred (new keyword, cleanliness), Option B `uses vocabulary Query.Forms` acceptable — proof syntax-agnostic; 3 fixtures (alpha_module/beta_module/consumer_module); NEXT: proposal authoring after PROP-IMPORT-RESOLUTION + OOF-REF2; LAB-FORM-CONSTRUCTOR-P1 (T1 Gap-I independent clock); closed: all canon impl/parser/typechecker/SemanticIR/VM/form-syntax/macro/call_contract/capability/Rust-lab; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_form_vocabulary_p1.rb; doc: lab-docs/governance/lab-form-vocabulary-cross-module-coherence-proof-v0.md; card: .agents/work/cards/governance/LAB-FORM-VOCABULARY-P1.md) | LAB-FORM-INVOCATION-P1 CLOSED (LAB PROOF — PROVED 66/66 PASS; in-module Contract Invocation Forms conservative elaboration over typed-ref substrate (LANG-TYPED-CONTRACT-REF-PROP-P3 67/67 PASS); HEADLINE: proof-local model (ProofLocalContractRef/FormDeclaration/FormRegistry/FormResolution/InvocationIntent/LoweringReceipt/ResugaringTrace) demonstrates forms are non-execution metadata bindings; TH-1 MECHANISED: LoweringReceipt.conservative?=true — fragment class + authority surface identical with/without form; TH-4 MECHANISED: F-01/02/03/05 structural rules each produce diagnostics (E-FORM-STRUCT/E-FORM-BINDER/E-FORM-KW-SHADOW/E-FORM-KIND); TH-5 DEMONSTRATED: ResugaringTrace carries surface trigger + expanded contract + lowering metadata; TH-6 MECHANISED: explicit InvocationIntent == form-lowered InvocationIntent (same target, same args, same execution_dependency:false); C-1 ENFORCED: every valid form has resolved `uses T` anchor; C-5 ENFORCED: no_form target → E-FORM-NO-REF; AUTHORITY CLOSED: no execute/runtime_dispatch/capability_grant/macro_expansion/import_authority/profile_binding on any proof-local type (I-01..06); 5 fixtures (basic/effect/chain/multi/no_ref), all canon `uses ContractName` syntax; GAPS: TH-1 effect-modifier propagation (asserted via SIR, not derived), TH-2 cross-module (gates on import mainline), TH-3 golden-test, MultiKeyword arm-capture; NEXT: LAB-FORM-VOCABULARY-P1 (cross-module coherence; after OOF-REF2 + import mainline); closed: all canon impl/parser/typechecker/SemanticIR/VM/form-syntax/macro/call_contract/capability/Rust-lab/cross-module; proof: igniter-lab/igniter-view-engine/proofs/verify_lab_form_invocation_p1.rb; doc: lab-docs/governance/lab-contract-invocation-forms-in-module-proof-v0.md; card: .agents/work/cards/governance/LAB-FORM-INVOCATION-P1.md) | LAB-CONTRACT-FORMS-P2 CLOSED (LAB RECONCILIATION — SPLIT+KEEP; Contract Invocation Forms lineage reconciled against typed-ref substrate (LANG-TYPED-CONTRACT-REF-PROP-P3 67/67 PASS) and TH-1..TH-6 acceptance frame; HEADLINE: orphaned form_registry.rs/form_resolver.rs + PROP-Forms-Enhanced-v0 are NOT retired — they are a complete lab implementation (spec+two proofs) whose missing lowering substrate is now canon; DECISION=KEEP with typed-ref anchor: form declarations must be anchored to `uses T` (Rule C-1); C-2 form ownership (Rust-shaped orphan rule); C-3 ambiguity diagnostic not first-wins (already proved FTD-5/6); C-4 import-order independence; C-5 no_form propagates through typed-ref; C-6 fragment class of declaring contract unchanged (metadata precedent from P3); C-7 MultiKeyword restricted to System/Stdlib in v0; TH STATUS: TH-1 partially proved (lab lowering PASS; effect-modifier propagation gap); TH-2 in-module proved (FTD-5/6), cross-module OPEN (gates on import mainline); TH-3 confirmed by Strategy B design; TH-4 partially addressed (F-02/03; MultiKeyword arm capture open); TH-5 demonstrated (ResolvedExpr+lowered_from_form, strongest TH); TH-6 closed by design claims (no negative fixture yet); TRACKS: T2 Contract Invocation Forms → LAB-FORM-INVOCATION-P1 (in-module proof-local, TH-1/4/6, Rules C-1/5/6/7); T2 cross-module coherence → LAB-FORM-VOCABULARY-P1 (after invocation P1 + OOF-REF2 gate + import mainline); T1 Gap-I Form Constructor → LAB-FORM-CONSTRUCTOR-P1 (independent clock); T3 view/component forms = not a track (T2 consumer); closed: no impl/parser/typechecker/SemanticIR/VM/form-syntax/macro/public-API/call_contract/capability/Rust-refactor; doc: lab-docs/governance/lab-contract-forms-lineage-reconciliation-v0.md; card: .agents/work/cards/governance/LAB-CONTRACT-FORMS-P2.md) | LANG-TYPED-CONTRACT-REF-PROP-P3 CLOSED (IMPLEMENTATION — PROVED 67/67 PASS; `uses ContractName` live in Ruby canon pipeline across all 5 stages: parser 1-token lookahead + dotted-name read loop, classifier "metadata" fragment + contract_fragment_for exclusion, typechecker build_same_module_registry + typecheck_uses_contract + OOF-REF1/2/4, SemanticIR contract_refs per-contract (enters contract_ref hash), assembler dependency_edges manifest field (execution_dependency: false); 11 fixtures; regressions: PROP-ENTRYPOINT-P3 53/53 + PROP-IMPORT-RESOLUTION-P5 99/99 + LAB-TYPED-CONTRACT-REF-P1 58/58; cross-module/forms/VM/call_contract/capability/Rust-lab closed; proof: experiments/typed_contract_ref_proof/verify_typed_contract_ref_p3.rb; card: igniter-lang/.agents/work/cards/lang/LANG-TYPED-CONTRACT-REF-PROP-P3.md) | LANG-TYPED-CONTRACT-REF-PROP-P2 CLOSED (IMPLEMENTATION PLANNING — READY FOR P3; `uses ContractName` Ruby canon pipeline; 5 insertion points: parser `parse_uses_decl` 1-token lookahead branch, classifier `when "uses_contract"` passthrough with `contract_ref_declarations` array, typechecker same-module registry lookup + OOF-REF1/2/4, SemanticIR `contract_refs` per-contract field, assembler `dependency_edges` manifest field; ~110–160 lines across 5 files; same-module only in v0 (cross-module deferred to import module table gate); `contract_refs` enters `contract_ref` content hash; `dependency_edges` enters artifact_hash via manifest material; OOF-REF2 covers all dotted/cross-module targets in v0; `uses_contract` NOT in `typed_nodes` — no VM node; regression matrix: PROP-IMPORT-RESOLUTION-P5 (99/99) + PROP-ENTRYPOINT-P3 (53/53) + LAB-TYPED-CONTRACT-REF-P1 (58/58) + P10/P11; proof matrix target ≥60 checks in experiments/typed_contract_ref_proof/; call_contract/forms/VM/capability/visibility/Rust-lab/public-API all closed; planning doc: igniter-lang/.agents/work/proposals/LANG-TYPED-CONTRACT-REF-P2-implementation-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-TYPED-CONTRACT-REF-PROP-P2.md) | LANG-TYPED-CONTRACT-REF-PROP-P1 CLOSED (PROPOSAL AUTHORED — pending review; `uses ContractName` contract body declaration; follows `uses assumptions NAME` (PROP-032) syntactic precedent — `uses` keyword already in language, 1-token lookahead disambiguation; metadata-only in v0 (no runtime invocation/capability/fragment-class change); SemanticIR `contract_refs` field + manifest `dependency_edges` (enters artifact hash); OOF-REF1=unknown/OOF-REF2=ambiguous/OOF-REF4=cycle active; OOF-REF3/5 reserved; `call_contract` NOT deprecated (stringly pressure source with documented upgrade path); `uses` = lowering substrate for TH-1 conservativity path (LAB-CONTRACT-FORMS-P2); 10 binary design questions all explicitly answered; proof evidence mapped from LAB-TYPED-CONTRACT-REF-P1 (58/58 PASS → proposal claims); no parser/compiler/VM implementation authorized — LANG-TYPED-CONTRACT-REF-PROP-P2 required; proposal: igniter-lang/.agents/work/proposals/LANG-TYPED-CONTRACT-REF-typed-contract-reference-declaration-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-TYPED-CONTRACT-REF-PROP-P1.md; index: igniter-lang/.agents/work/proposals/README.md) | LAB-TYPED-CONTRACT-REF-P1 CLOSED (PINNED BACKGROUND, Track 1 of LAB-CONTRACT-FORMS-P1 SPLIT; 58/58 PASS; VERDICT=ACCEPT; HEADLINE: all data for typed contract refs already present in SemanticIR (contract_name/modifier/inputs/outputs/source_hash) — zero new emission required; proof-local model ContractRef/ContractSignature/ContractDependency/RefUseReceipt coherent; literal callee discriminant same as P10/P11 (args[0].kind=="literal" && type_tag=="String"); ContractRef has NO execute/runtime_dispatch/capability_grant — reference≠invocation; effect modifier preserved in resolved_signature (future gating possible); dependency DAG inspectable (ContractDependency.to_edge_label), serializable to JSON, acyclic in chain A→B→C; cross-file resolution order-independent (PROP-IMPORT-RESOLUTION-P3 fixture proven); import ≠ capability grant; ContractRef carries all fields LAB-CONTRACT-FORMS-P2 needs as lowering target (module_name/contract_name/contract_ref/modifier/input_count) = TH-1 conservativity substrate; OPEN GAP: cross-module typed refs gate on PROP-IMPORT-RESOLUTION-P3 module table (same-module case fully proven); NEXT: LANG-TYPED-CONTRACT-REF-PROP-P1 (canon proposal for `uses Contract` syntax) → LAB-CONTRACT-FORMS-P2 (PROP-Forms lineage reconciliation, now has typed-ref substrate) + LAB-FORM-CONSTRUCTOR-P1 (Gap-I independent); doc: lab-docs/governance/lab-typed-contract-reference-boundary-proof-v0.md; card: .agents/work/cards/governance/LAB-TYPED-CONTRACT-REF-P1.md) | LAB-FORM-LAYER-THEORY-P1 CLOSED (PINNED BACKGROUND theory continuation of LAB-CONTRACT-FORMS-P1; VERDICT=OPEN — strong hypothesis confirmed: forms = STRATIFICATION mechanism (fixed verifiable kernel + open surface vocabulary), not invocation sugar; THEORY JOINTS all land on standard foundations: kernel=traced symmetric monoidal category (ALREADY claimed by PROP-002:152-154/453-454) → "pictures over bricks" = string diagrams (Joyal–Street/Selinger), forms = named derived operations = DEFINITIONAL/CONSERVATIVE extension → Honesty becomes a PROVABLE theorem (Felleisen eliminability: forms add abbreviation power, NOT expressive power); grammar mechanism = fixed skeleton + open vocabulary (FormKind ×7 ALREADY implements it; vocabulary extension preserves skeleton unambiguity — conflicts move parse-time-undecidable → resolution-time-decidable fail-closed); execution layer = elaboration to trusted kernel (GHC Core/Lean 4/Racket lineage); tooling = resugaring (Pombrio–Krishnamurthi; ResolvedExpr already carries both ends); ONE new hard problem = COHERENCE/ownership (type-class coherence/Rust orphan rule; candidate: form declarable only by contract owner or vocabulary owner; order-independent resolution mandatory) — gates on import mainline, VALIDATES SPLIT sequencing; UNIT = form VOCABULARY (named, versioned, `speaks`-imported, no ambient dialects = P28 at language level); lab already invented 3 proto-vocabularies independently (view/query/decision); proof obligations TH-1..TH-6 named (conservativity receipt, coherence permutation, skeleton stability, hygiene, resugaring, eliminability boundary); NEXT: spine unchanged + LAB-CONTRACT-FORMS-P2 amended (TH-frame) + NEW LAB-FORM-VOCABULARY-P1 (proof-local 2 vocabularies, mechanize TH-1/2/3, after P2+import mainline); no impl/canon/grammar/speaks-authority; doc: lab-docs/governance/lab-form-layer-theory-and-grammar-stratification-v0.md; card: .agents/work/cards/governance/LAB-FORM-LAYER-THEORY-P1.md) | LAB-CONTRACT-FORMS-P1 CLOSED (PINNED BACKGROUND direction — NOT mainline; archaeology+formalization of Contract Invocation Forms; HEADLINE: idea was already BUILT and ORPHANED — form_registry.rs/form_resolver.rs = complete lab implementation of invocation forms (trigger→type-directed→ContractInvocation, FormKind ×7, priority, no_form, form_resolution_trace.json, F-01..F-06 fail-closed) specced in PROP-Forms-v0 (Agent-C archive)+PROP-Forms-Enhanced-v0, never entered canon governance; view-DSL exploration independently converged on same conclusion ("invocation alias → ContractInvocation, not runtime primitive"); TERMINOLOGY: 8 meanings of "form" disambiguated (report §3 normative) — keep "form" only for T1 Gap-I Form Constructor (Covenant P27/P28, value construction, zero impl, "highest-leverage sem gap") + T2 Invocation Form; typed contract-ref (`uses Other`) is NOT a form (dependency feature); composition-macro candidate REJECTED (macro risk; PROP-002 owns multi-invocation); HONESTY GATES: forms honesty-positive IFF static resolution + IR-preserved lowering (Path B precedent) + trace evidence/tooling expansion — all three non-negotiable; VM v0 impact = none (compile-time lowering); DECISION = SPLIT into 3 tracks: LAB-TYPED-CONTRACT-REF-P1 first (smallest, solves stringly call_contract pain) → LAB-CONTRACT-FORMS-P2 (PROP-Forms lineage reconciliation keep/reduce/retire) + LAB-FORM-CONSTRUCTOR-P1 (Gap-I, independent clock); closed: no impl/canon-PROP/grammar/call_contract-replacement/runtime-dispatch/visibility-import-changes; doc: lab-docs/governance/lab-contract-invocation-forms-formalization-v0.md; card: .agents/work/cards/governance/LAB-CONTRACT-FORMS-P1.md) | PROP-ENTRYPOINT-P4 CLOSED (64/64 PASS - Option C: Rust-lab parser/typechecker/SemanticIR/manifest parity plus proof-local manifest consumer; top-level `entrypoint ContractName` metadata produced outside Ruby P3; `OOF-EP1`, `OOF-EP2`, `OOF-EP5` proven in Rust lab; manifest consumer resolves display label/path/ref without VM execution; no CLI/VM/app framework/scheduler/visibility/package/import/capability/public API authority opened; entrypoint track may pause, next continue PROP-IMPORT-RESOLUTION-P3/P5 line if separately authorized) | PROP-ENTRYPOINT-P3 CLOSED (53/53 PASS - bounded Ruby single-file implementation; top-level `entrypoint ContractName` metadata carried parser -> classifier -> typechecker -> SemanticIR -> `.igapp` manifest; `OOF-EP1`, `OOF-EP2`, `OOF-EP5` proven; zero-entrypoint library valid; manifest entrypoint is evidence/metadata and enters artifact hash material; no CLI/VM/app framework/scheduler/visibility/package/import/capability/public API authority opened) | LANG-MODULE-IDENTITY-P1 CLOSED (governance/readiness audit — hash discipline and multi-file prerequisite; CONDITIONAL verdict; F1: SHA256 substrate sound — source_hash/contract_ref/artifact_hash all canonical; F2: program_id DIVERGENT blake3-Rust vs SHA256-Ruby = C24; F3: import INERT — parsed, zero classifier/TC resolution, undefined import→no error, root cause of 8+ copy-paste QueryResult; F4: module name = display label, no uniqueness enforcement; F5: authority does not flow through import — consumer-side binding is closed; DECISION=CONDITIONAL: two pre-conditions: C1 unify program_id to SHA256 + C2 enforce OOF-M1/M2; NEXT: LANG-MODULE-IDENTITY-P2 (program_id unification, small) → LAB-MULTIFILE-COMPILATION-P1 (≥15 fixture cases: cross-file import, OOF-M1/M2/M3, multi-contract .igapp); closed: registry/semver/visibility/cross-module-profiles/dynamic-loading/VM-bytecode-identity; docs: lab-docs/governance/lang-module-identity-hash-discipline-readiness-v0.md; card: .agents/work/cards/governance/LANG-MODULE-IDENTITY-P1.md) | LAB-LANGFORM-RESEARCH-P1 CLOSED (out-of-track — THREE proposal-readiness docs on language-form blind spots surfaced by LAB-PURSUIT-P1; lab/governance READINESS research, NO canon PROP authored; DOC1 stdlib: ~85% complete (Text/Option/Result/Collection/DateTime VM-executable, VM superset of canon for Option/Result; Map read-only) but NUMERIC MATH is the gap — only +−×÷, NO abs/min/max/clamp/compare/sign/isqrt/pow/sqrt/trig (LAB-PURSUIT-P1 forced sqrt-free integer fixed-point empirically); PRECONDITION = dual-toolchain OPERATOR PARITY (Float rejected-both, Decimal Rust-yes/Ruby-no, ==/< /|| Rust-yes/Ruby-no — STAB-P4) gates ALL numeric canon promotion; proposed N0 abs/min/max/clamp/compare/sign + N1 isqrt/ipow/imuldiv (Integer+Decimal pure CORE), N2 CORDIC-integer trig+sqrt DEFERRED; + Map from_pairs/empty VM handlers close PROP-043 v0; builds on PROP-013/042/043. DOC2 packaging: `import` PARSES but is INERT (zero classifier/TC resolution; undefined import→NO error; QueryResult re-declared 8+× "for lab independence") → reuse=copy-paste; 3 fused concerns (a) module-resolution KEYSTONE / (b) visibility-export / (c) distribution-deferred; DESIGN LEAN = content-addressed reuse Unison/Nix-style over semver (Igniter already hashes everything; fits determinism/replay/honesty); (a) alone ends copy-paste + unblocks docs 1&3. DOC3 application-form: flat plane CONFIRMED — only module→contract→io, contracts flat siblings, entrypoint LANGUAGE-ABSENT (Ch2 §2.2.1; CLI --entry/contracts[0] only), ZERO public/private, composition stringly-typed call_contract(\"Name\") (compiler knows DAG, source hides it); diagnosis = DAG compiler-real/source-invisible, undeclared public surface = hidden assumption → STRUCTURAL HONESTY gap (Covenant-aligned); 3 orthogonal needs entrypoint/visibility/grouping; proposed entry-marker+.igapp-entrypoint, public/internal default-internal (=content-addressed module interface), grouping+typed-contract-refs deferred. TRIAD SPINE: KEYSTONE PROP-IMPORT-RESOLUTION (doc2a) unblocks stdlib-as-import + cross-file visibility; PARALLEL STAB-P4-operator-parity + PROP-NUMERIC-CORE + PROP-ENTRYPOINT(standalone smallest first win, helps debugger-textbook); SHARED PROP-MODULE-VISIBILITY (docs 2&3); recommended first = PROP-ENTRYPOINT + PROP-IMPORT-RESOLUTION; NO canon/code/PROP; docs lab-docs/governance/igniter-{stdlib-numeric-coverage,packaging-and-library-reuse,application-structure-and-module-form}-proposal-readiness-v0.md) | LAB-PURSUIT-P1 CLOSED (45/45 PASS — hypothetical-app language-sufficiency probe, out-of-track: quadcopter pursuit/evasion = Kalman estimation + sqrt-free ZEM proportional-navigation interception + bang-bang evasion + closed-loop sim through the VM; HEADLINE arithmetic boundary: Float REJECTED by BOTH typecheckers, Decimal Rust-accepts/Ruby-rejects DIVERGENCE (STAB-P4 family), VM stdlib has NO sqrt/sin/cos/atan ⇒ forced INTEGER FIXED-POINT (mm/mm·s⁻¹/ms/milli-gain — embedded-grade, FPU-free, deterministic); scalar constant-velocity Kalman (no matrix inverse) + sqrt-free ZEM t_go=r²/−r·v by construction; VM Kalman ≡ integer reference EXACTLY incl. negative-residual truncation-toward-zero (Ruby floor-div would DISAGREE — embedded determinism detail); coast (sensor_lost⇒predict-only) grows covariance, filter converges, replay-grade determinism; sqrt-free PN exact (collision→ZEM0, offset hand-verified), command clamps ±amax, cannot_intercept = honest terminal NOT failed (PROP-047 namespace); closed-loop sim A non-evading INTERCEPTED @6.1s miss 1.70m from 111.8m est_err 0.62m@±0.3m noise, sim B evading NOT intercepted miss 62.7m (evasion effective), zero VM faults, deterministic replay (HYP-SIM-06 = frontier replay property concrete); epistemic: TrackObservation requires uncertainty_mm+evidence_kind (P11/P13), sensor_lost→coast (unknown≠failure), EngageGuard model-evidence-without-human-approval→escalate_human (SAME No-Upward-Coercion VM mechanism as EPISTEMIC-P4); guards Ruby-TC-blocked (==/< divergence)/Rust-VM-executed; EMBED VERDICT = CONDITIONAL — strong on honest-autonomy layer (estimate/decide/gate/replay, native to Igniter), weak on numerical-stdlib (no sqrt/trig + Decimal divergence) + real sensor/actuator I/O + hard-real-time/WCET (bounded conventional engineering, not language-philosophy gaps); composes with EPISTEMIC-OUTCOME P1-P4 + FRONTIER-DECISION (cannot_intercept/escalate_human = honest-terminal/escalation kinds) + deterministic-replay frontier property; harness = v0 simulator-host pattern (world in harness, avionics in VM → Gap-H SimulationReceipt for synthetic worlds); NO real-world targeting/weapons claim (textbook benchmark); next = LAB-MATH-STDLIB-READINESS keystone (isqrt/CORDIC + resolve Decimal divergence) / LAB-PURSUIT-P2 DecisionReceipt-over-engagement / FRONTIER-SYNTHETIC-P1 / real-time WCET probe)
LANG-STDLIB-IMPORT-SURFACE-P2 CLOSED (IMPLEMENTATION PLANNING — READY FOR P3; RUBY ONLY (Rust P4); THREE INSERTIONS in multifile_resolver.rb: (1) stdlib guard inside validate_imports before by_module lookup — OOF-IMP2 unknown module / OOF-IMP3 per-name; (2) four private helpers: stdlib_path? / stdlib_module_known? / stdlib_name_known? / stdlib_module_table (memoized JSON load from ../../docs/spec/stdlib-inventory.json); (3) OOF-IMP6 guard in resolve for user module declaring stdlib.* path; TABLE: stdlib.collection/{map,filter,count} / stdlib.text/{14} / stdlib.map/{4} / stdlib.option/{or_else} / stdlib.bool/{} / stdlib.numeric/{}; GAPS: fold/sum/append not importable (not in inventory); APP: advanced_logistics filter→OK; vector_editor map→OK/append→OOF-IMP3; decision_tree filter→OK/append→OOF-IMP3; IMPORT CLEARS BEFORE TC: already the case; RUST: include_str! embed in multifile.rs P4; proof ≥61 checks / 7 sections; no TC/SIR/assembler/inventory/VM changes)
LANG-STDLIB-IMPORT-SURFACE-P1 CLOSED (PROPOSAL / BOUNDARY DESIGN — stdlib import surface v0; SIX QUESTIONS ANSWERED: Q1=built-in stdlib module table (derived from inventory, consulted before user module table, stdlib.* namespace sealed to compiler, NOT package model / NOT compiler profile as resolver); Q2=source aliases only — import validates + clears, TypeChecker dispatch unchanged; Q3=NO capability authority — name availability ≠ authority, no fragment class change; Q4=stdlib_surface_digest is identity anchor for importable surface, profile pinning is future PROP-036/038 slot; Q5=OOF-IMP2 unknown stdlib module + OOF-IMP3 known module unknown name, no new OOF codes; Q6=NO SIR name change — canonical qualified names are entry-contract rule, not import rule; append=OOF-IMP3 today (not in inventory → LANG-STDLIB-COLLECTION-APPEND-P1); "import validates not gates" v0 stance; no parser/TC/SemanticIR/resolver/inventory/VM changes; pressure: AL-P01 advanced_logistics + VE-P01 vector_editor both blocked on OOF-IMP2 stdlib.collection; next LANG-STDLIB-IMPORT-SURFACE-P2 implementation planning)
**Scope:** Cross-repo state map for igniter-lab ↔ igniter-lang

---

**Recent canon planning update:** LANG-STDLIB-ENTRY-CONTRACT-P2 CLOSED / READY FOR P3 (implementation planning, no code; REGISTRY LOCATION: single hand-authored JSON igniter-lang/docs/spec/stdlib-inventory.json normatively referenced by Ch8 — YAML rejected (cross-language parse divergence threatens digest determinism), Ruby literal rejected (not Rust-consumable), generation rejected for v0 (inventory IS the truth); digest over canonical re-serialization never raw bytes; SCHEMA FINALIZED: envelope {kind, format_version, entries[], stdlib_surface_digest}; legacy_sir explicit field (exact current SIR string when ≠ canonical_name, must co-exist as legacy_sir-kind alias); global alias uniqueness = "aliases are not identities" made mechanical; purity:effect ⟺ capability authority ⟺ ESCAPE as schema consistency rule; orphan ⟹ triage-route note required; DIGESTS EXACT: entry_digest = sha256(canonical_json(entry minus itself)), surface_digest = sha256(canonical_json(entries sorted by canonical_name, digest-stripped)) — independently recomputable, file-order independent (IMPORT-P5 discipline); profile pinning = future stdlib_surface slot via separate PROP amendment, P3 does not touch profiles; DERIVED DISPATCH = OPTION A: tables stay manual, P3 proof verifies BOTH directions (every lowering claim has live dispatch site; every dispatch key resolves to canonical or active alias — stray name = FAIL = anti-accretion gate; B Ruby-derives deferred to P4/P5 after one stable cycle); FIRST 24 ENTRIES exhaustive: text×14 (reference category) + map×4 (PROP-043; map.empty Unknown = honest context-deferred inference, live note string recorded verbatim; from_pairs/empty VM lowering verified at proof time — registry records what proofs show) + stdlib.option.or_else (sole legacy_sir; counted-grandfather check = exactly one non-null in surface) + stdlib.collection.count + 4 orphan records (option.wrap/bool.and/integer.gt/collection.concat — visibility before judgment); or_else canonical = stdlib.option.or_else NOT unwrap_or (Ch8 §8.3 Option op vs §8.4 Result op, different input types; NEW DRIFT FOUND IN P2: Rust TC matches "unwrap_or"|"or_else" in one arm, typechecker.rs:2696, conflating Option/Result ops → compatibility_note + LAB-STDLIB-OPTION-P1); bare `length` = rejected alias on all three *_length entries (unit-model ambiguity by design); MIGRATION: fixtures never change (all 14 or_else uses source-level, source_alias permanent); emission switch = P4 separately authorized (SIR golden regeneration + Rust dual-accept window); CROSS-TOOLCHAIN: P3 Ruby-only runner experiments/stdlib_inventory_proof/verify_stdlib_inventory_p3.rb, P4 Rust parity reads same JSON; PROOF MATRIX 68 checks / 10 sections (schema 12 / digest determinism 8 incl shuffled-file + alias-append-negative / identity 8 / map 8 / text 10 / or_else 6 incl live-SIR==legacy_sir / dispatch coverage 8 / authority closure 6 / orphans 4 / closed surfaces 4); registry impl/dispatch generation/parser/TC/SIR/VM/renaming/packages closed in P2, P3 authors JSON + runner only with zero compiler-source changes; planning: igniter-lang/.agents/work/proposals/LANG-STDLIB-ENTRY-CONTRACT-P2-implementation-planning-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-ENTRY-CONTRACT-P2.md).

**Recent canon proposal update:** LANG-STDLIB-ENTRY-CONTRACT-P1 CLOSED / PROPOSAL AUTHORED (Stdlib Entry Contract v0, authored-pending-review; CORE PRINCIPLE: stdlib entry = sealed language-owned semantic claim — package sealed-claim discipline with inverted trust position (consumer recomputes packages, compiler vouches stdlib); NORMATIVE SHAPE: canonical_name (always qualified `stdlib.<category>.<fn>`, no exceptions) / typed aliases (source_alias|legacy_sir|lab_only|rejected, append-only churn ledger) / status × stability{semantic,lowering,compatibility} as INDEPENDENT axes / signature block / authority_surface ("none"|"capability: <Type>" — grant fields schema-absent) / semantic_ir_name MUST EQUAL canonical_name (single constraint kills bare-name drift permanently; bare `or_else` = sole grandfathered legacy_sir violation with P2 deadline) / lowering evidence per toolchain / proof_lineage / entry_digest; AXES make today's misleading cases expressible: Ch8 collection ops = canon+design-locked+kernel-only, stdlib.option.wrap = orphaned+dual-toolchain (orphans can run — fact worth recording); ALL 11 REQUIRED ANSWERS GIVEN: qualified always YES / short names = permanent source aliases not legacy / or_else = drift not exception / effectful entries YES (ESCAPE + capability input param + Result output) / authority grant NO (schema-absent) / proof-local use ≠ status (demand evidence only) / lab VM impl ≠ canon status (sets lowering axis only) / stdlib ∉ package model now (language-owned, profile-pinned; self-hosted convergence designed-for not opened) / outcome = category-yes-content-routed (LAB-STDLIB-OUTCOME-P1) / numeric NOT ready (STAB-P4 gate) / anchor = stdlib_surface_digest (canonical-JSON sorted records, IMPORT-P5 hash discipline, PROP-036 profile-pin slot — the concrete "stdlib is the package the compiler vouches for" object); DRIFT PROCEDURES per class: alias migration record-first code-in-P2, canon/lab mismatch canon-wins-by-default with proven-lab-vs-unproven-canon triage exception, orphans get immediate status:orphaned records (visibility before judgment), proof-local helpers get NO records; PROOF BAR cumulative per claim level up to dual-toolchain parity (per-entry STAB-P4 rule) + closed-authority assertion + surface digest update; Gemini inventory subtask evidence integrated (parallel bare-alias dispatch starts_with/map_get in both toolchains, parse_datetime/format_datetime hardcoded in Rust TC, stringly unknown_external_state/timed_out in fixtures); NEXT: LANG-STDLIB-ENTRY-CONTRACT-P2 implementation planning (inventory location, derived-dispatch strategy, D1/D2 SIR migration, first text/map records) + LAB-STDLIB-OUTCOME-P1 ungated; stdlib impl/renaming/parser/TC/SIR/VM/registry-build/package/compat-promise closed; proposal: igniter-lang/.agents/work/proposals/LANG-STDLIB-ENTRY-CONTRACT-stdlib-entry-contract-v0.md; card: igniter-lang/.agents/work/cards/lang/LANG-STDLIB-ENTRY-CONTRACT-P1.md).

**Recent governance update:** LAB-STDLIB-FOUNDATION-P1 CLOSED / SPLIT (stdlib surface inventory + entry contract foundation; HEADLINE 1: only stdlib.text (14 ops) has full three-way agreement (Ch8 canon + Ruby TEXT_STDLIB_FNS + Rust VM dispatch) — the reference category; stdlib.map (PROP-043, 4 ops) close behind; HEADLINE 2 — executable-truth inversion (D5): only fully executable stdlib is lab Rust VM (explicitly non-canonical per README) while canon kernel-proven collection surface (fold/map/filter/sum/avg/min/max/group_by/sort_by/take/first/last) does not run anywhere ("pending Slice A") — stability labels conflate accepted with executable; FIVE DRIFT DECISIONS surfaced: D1 SIR namespace inconsistency (`or_else` deliberately emitted bare, "no stdlib. prefix — v0 design" typechecker.rb:2092, vs everything else qualified), D2 Option constructor split (canon some/none vs VM-only stdlib.option.wrap), D3 temporal names disjoint sets (Ch8 §8.6 add_duration/diff/as_of vs proven datetime_extension diff_seconds/add_seconds/parse_datetime/format_datetime/is_before/is_after — zero overlap), D4 numeric HOLD (Ch8 lists stdlib.float.add rejected by BOTH TCs + stdlib.decimal.* Rust-yes/Ruby-no; STAB-P4 parity gates entire category; demand proven via LAB-PURSUIT N0 abs/min/max/clamp/compare/sign + N1 isqrt/ipow/imuldiv), D5 stability/lowering axes must separate; FALSE STDLIB partitioned 4 classes: proof-local helpers (PURSUIT fixed-point = demand evidence not entries), domain-local (query/storage/outcome ad-hoc kind: records), orphaned names (stdlib.bool.and, stdlib.integer.gt, stdlib.collection.concat — single-surface stdlib.-prefixed inventions), doc-sketches (igniter-stdlib/stdlib/*.ig uses non-canon `def` + stdlib.Math capitalization — zero surface authority); ENTRY CONTRACT concretized (extends RES-001 §3.1): canonical_name/source_name/aliases(churn ledger)/category/fragment_class/purity/signature/deterministic/totality/failure_behavior/diagnostics/authority_surface(capability-param-only, grant fields schema-absent)/lowering(per-toolchain+kernel_only)/stability/proof/lineage/since; inventory hash = stdlib_surface_digest (PROP-036 pattern; slots into compiler_profile pinning = the "stdlib is the package the compiler vouches for" object from PACKAGE-P1-a2); INCLUSION CRITERIA: ≥2-domain demand / tier-decidable / total-or-honestly-partial / deterministic-or-capability-gated / signature-expressible-today-else-reserved / proof bar / no hidden authority or domain leakage; CATEGORY VERDICTS: text READY, map READY, option/result READY-for-reconciliation, collection SPLIT, numeric HOLD(D4), datetime HOLD(D3), outcome OPEN-as-convention (biggest blind spot: canon Ch12+P15 unknown-state model has zero stdlib combinators — proofs flatten to ad-hoc kind: records; is_unknown/requires_reconciliation/partition_partial shippable convention-level pre-sum-types), query DOMAIN-LOCAL, io/net/storage L3-reserved, bool DO-NOT-CREATE; NEXT: LANG-STDLIB-ENTRY-CONTRACT-P1 (ready now; resolves D1/D2/D5) + LAB-STDLIB-OUTCOME-P1 + LAB-STDLIB-OPTION-P1/COLLECTION-P1/DATETIME-P1 (after entry-contract) + LAB-STDLIB-MATH-P1 (gated D4); stdlib impl/parser/TC/VM/package-distribution/registry/compatibility-promise closed; doc: lab-docs/governance/lab-stdlib-surface-inventory-and-entry-contract-v0.md; card: .agents/work/cards/governance/LAB-STDLIB-FOUNDATION-P1.md).

**Recent governance update:** LANG-CONTRACT-NAMESPACE-P1 CLOSED / READY FOR P2 (prerequisite research for LAB-PACKAGE-MODEL-P2; blocker analysis of global contract name uniqueness constraint; HEADLINE: OOF-DECL-DUP-CONTRACT and OOF-DECL-DUP-TYPE both use flat short-name scan across ALL source units — not module-scoped; two modules each declaring `ValidateInput` fail even though their identities are distinct (`Users.ValidateInput` ≠ `Orders.ValidateInput`); INVENTORY: 6 package-hostile surfaces identified — duplicate_declaration check (multifile_resolver.rb:261), contract_index_for keys (assembler.rb:416), contract_refs manifest field (assembler.rb:246), contract_ref path label (semanticir_emitter.rb:862), entrypoint resolution by short name (typechecker.rb:188), no disambiguation for multifile same-short-name entrypoint; WHAT IS CORRECT: OOF-IMP4 (module-level dup) unchanged; P5 cross_module_registry + per_contract_module already implement (module, contract_name) identity internally — gap is declaration check and manifest output only; RECOMMENDED IDENTITY MODEL: source syntax = short local names (unchanged), canonical = module_name.contract_name, manifest surfaces use qualified form; DUPLICATE RULE: same name + same module = OOF-DECL-DUP-CONTRACT (unchanged); same name + different modules = ALLOWED (change); OOF-IMP4 unchanged; entrypoint short-name ambiguous in multifile → OOF-EP2 with qualified disambiguation hints; MIGRATION: pre-v1 breaking correction; single-file transparent; multifile manifest contract_index keys change from short to module-qualified; no source syntax change required; PROOF REQUIREMENTS FOR P2: ≥16 checks covering co-compile success, OOF-DECL-DUP-CONTRACT still fires within-module, qualified contract_index keys, no file-path collision, entrypoint disambiguation, all TYPED-REF-P5 71/71 and IMPORT-P5 99/99 regressions; doc: lab-docs/governance/lang-contract-namespace-boundary-v0.md; card: .agents/work/cards/governance/LANG-CONTRACT-NAMESPACE-P1.md).
