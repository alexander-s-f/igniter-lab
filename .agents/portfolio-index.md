...
**Last updated:** LAB-LANGFORM-RESEARCH-P1 CLOSED (out-of-track — THREE proposal-readiness docs on language-form blind spots surfaced by LAB-PURSUIT-P1; lab/governance READINESS research, NO canon PROP authored; DOC1 stdlib: ~85% complete (Text/Option/Result/Collection/DateTime VM-executable, VM superset of canon for Option/Result; Map read-only) but NUMERIC MATH is the gap — only +−×÷, NO abs/min/max/clamp/compare/sign/isqrt/pow/sqrt/trig (LAB-PURSUIT-P1 forced sqrt-free integer fixed-point empirically); PRECONDITION = dual-toolchain OPERATOR PARITY (Float rejected-both, Decimal Rust-yes/Ruby-no, ==/< /|| Rust-yes/Ruby-no — STAB-P4) gates ALL numeric canon promotion; proposed N0 abs/min/max/clamp/compare/sign + N1 isqrt/ipow/imuldiv (Integer+Decimal pure CORE), N2 CORDIC-integer trig+sqrt DEFERRED; + Map from_pairs/empty VM handlers close PROP-043 v0; builds on PROP-013/042/043. DOC2 packaging: `import` PARSES but is INERT (zero classifier/TC resolution; undefined import→NO error; QueryResult re-declared 8+× "for lab independence") → reuse=copy-paste; 3 fused concerns (a) module-resolution KEYSTONE / (b) visibility-export / (c) distribution-deferred; DESIGN LEAN = content-addressed reuse Unison/Nix-style over semver (Igniter already hashes everything; fits determinism/replay/honesty); (a) alone ends copy-paste + unblocks docs 1&3. DOC3 application-form: flat plane CONFIRMED — only module→contract→io, contracts flat siblings, entrypoint LANGUAGE-ABSENT (Ch2 §2.2.1; CLI --entry/contracts[0] only), ZERO public/private, composition stringly-typed call_contract(\"Name\") (compiler knows DAG, source hides it); diagnosis = DAG compiler-real/source-invisible, undeclared public surface = hidden assumption → STRUCTURAL HONESTY gap (Covenant-aligned); 3 orthogonal needs entrypoint/visibility/grouping; proposed entry-marker+.igapp-entrypoint, public/internal default-internal (=content-addressed module interface), grouping+typed-contract-refs deferred. TRIAD SPINE: KEYSTONE PROP-IMPORT-RESOLUTION (doc2a) unblocks stdlib-as-import + cross-file visibility; PARALLEL STAB-P4-operator-parity + PROP-NUMERIC-CORE + PROP-ENTRYPOINT(standalone smallest first win, helps debugger-textbook); SHARED PROP-MODULE-VISIBILITY (docs 2&3); recommended first = PROP-ENTRYPOINT + PROP-IMPORT-RESOLUTION; NO canon/code/PROP; docs lab-docs/governance/igniter-{stdlib-numeric-coverage,packaging-and-library-reuse,application-structure-and-module-form}-proposal-readiness-v0.md) | LAB-PURSUIT-P1 CLOSED (45/45 PASS — hypothetical-app language-sufficiency probe, out-of-track: quadcopter pursuit/evasion = Kalman estimation + sqrt-free ZEM proportional-navigation interception + bang-bang evasion + closed-loop sim through the VM; HEADLINE arithmetic boundary: Float REJECTED by BOTH typecheckers, Decimal Rust-accepts/Ruby-rejects DIVERGENCE (STAB-P4 family), VM stdlib has NO sqrt/sin/cos/atan ⇒ forced INTEGER FIXED-POINT (mm/mm·s⁻¹/ms/milli-gain — embedded-grade, FPU-free, deterministic); scalar constant-velocity Kalman (no matrix inverse) + sqrt-free ZEM t_go=r²/−r·v by construction; VM Kalman ≡ integer reference EXACTLY incl. negative-residual truncation-toward-zero (Ruby floor-div would DISAGREE — embedded determinism detail); coast (sensor_lost⇒predict-only) grows covariance, filter converges, replay-grade determinism; sqrt-free PN exact (collision→ZEM0, offset hand-verified), command clamps ±amax, cannot_intercept = honest terminal NOT failed (PROP-047 namespace); closed-loop sim A non-evading INTERCEPTED @6.1s miss 1.70m from 111.8m est_err 0.62m@±0.3m noise, sim B evading NOT intercepted miss 62.7m (evasion effective), zero VM faults, deterministic replay (HYP-SIM-06 = frontier replay property concrete); epistemic: TrackObservation requires uncertainty_mm+evidence_kind (P11/P13), sensor_lost→coast (unknown≠failure), EngageGuard model-evidence-without-human-approval→escalate_human (SAME No-Upward-Coercion VM mechanism as EPISTEMIC-P4); guards Ruby-TC-blocked (==/< divergence)/Rust-VM-executed; EMBED VERDICT = CONDITIONAL — strong on honest-autonomy layer (estimate/decide/gate/replay, native to Igniter), weak on numerical-stdlib (no sqrt/trig + Decimal divergence) + real sensor/actuator I/O + hard-real-time/WCET (bounded conventional engineering, not language-philosophy gaps); composes with EPISTEMIC-OUTCOME P1-P4 + FRONTIER-DECISION (cannot_intercept/escalate_human = honest-terminal/escalation kinds) + deterministic-replay frontier property; harness = v0 simulator-host pattern (world in harness, avionics in VM → Gap-H SimulationReceipt for synthetic worlds); NO real-world targeting/weapons claim (textbook benchmark); NO canon/VM/compiler/PROP/real-IO; next = LAB-MATH-STDLIB-READINESS keystone (isqrt/CORDIC + resolve Decimal divergence) / LAB-PURSUIT-P2 DecisionReceipt-over-engagement / FRONTIER-SYNTHETIC-P1 / real-time WCET probe; fixtures/pursuit_guidance/pursuit_guidance.ig + proofs/verify_pursuit_guidance_p1.rb + lab-docs/lang/lab-pursuit-evasion-guidance-hypothetical-app-sufficiency-v0.md) | FRONTIER-DECISION-P2 CLOSED (72/72 PASS first full run — query-plan selection Decision KDR proof; Gap-J first lab pressure; DecisionReceipt 12-field (nested ChosenAction + 4 Collections + Map) VM-constructed/carried/routed; P24 all SEVEN exposures incl. expected_outcome+uncompensatable; satisfied constraints recorded not only violated; constraint-driven rejections (no_include_all/row_budget) w/ reasons+dispositions; FC-D1..D9 behaviorally: capability-denied→denied_upstream EXCLUDED from rejected (FC-D3) while capability-permitted plan still decision-rejected; in-VM kind guard — none-approval→escalated (FC-D1), model+agent→escalated (FC-D6), model+SYSTEM→escalated (v0-conservative, system-over-model parked on FRONTIER-HUMAN-GATE), model+human→decided, real+agent→decided, nvo wins over pending; routing execute_plan/wait/human_review/stop + unknown→hold fail-closed, output=data (FC-D7); no_viable_option = empty chosen ≠failed ≠deferred (FC-D9); waiver recorded+still-listed w/ named authority (FC-D4), empty-authority waiver→invalid_waiver_escalate; argmax+empty-rationale→insufficient_justification (FC-D5); decision kinds ≠ PROP-047 outcome kinds (no collision); guards Ruby-TC-blocked (== divergence, STAB-P4)/Rust-VM-executed as in EPISTEMIC-P4; regression EPISTEMIC-P2 54/54 + P4 46/46; NO grammar/constraints{}/StrategyDecision-canon/compiler-VM-changes/PROP/ethics-engine/agent-autonomy/audit-semantics/real-IO; next = FRONTIER-DECISION-PROP-READINESS (surface clean, P3 hardening NOT needed) + parallel FRONTIER-HUMAN-GATE-P1; fixtures/frontier_decision/query_plan_decision.ig + proofs/verify_frontier_decision_p2.rb + lab-docs/governance/frontier-decision-query-plan-selection-kdr-proof-v0.md) | LAB-FILE-IO-P1 CLOSED (78/78 PASS - File/Text IO mocked read snapshot boundary; FileCapability-shaped root/path/encoding/size/symlink/traversal gates; explicit MockFileRegistry fixture data; FileReadResult content/not_found/denied/file_error/decode_error/size_error; receipt=evidence not authority; no real filesystem reads/writes/public API/canon; next LAB-FILE-IO-P2 or parallel Clock/Host IPC; real filesystem HOLD) | FRONTIER-DECISION-P1 CLOSED (Decision Honesty surface design boundary, Gap-J/P24/P25, NO IMPLEMENTATION — DecisionReceipt KDR with 4 kinds (decided/decision_deferred/decision_escalated/no_viable_option — last = decision analog of denial-as-data); canon-faithful SEVEN P24 exposures (+expected_outcome/+uncompensatable); ConstraintApplication (declaration≠application, satisfied recorded, waiver⇐named authority), RejectedAlternative (rejected=evaluated, denied excluded, disposition final|deferred|escalated), AuthorityLink (decision authority≠capability authority; proposed|recommended|approved|vetoed; agent-only approval over model evidence→escalated per P13/No-Upward-Coercion), EvidenceRef.evidence_kind (P4-proven mechanism); forbidden collapses FC-D1..FC-D9; decision kinds = separate namespace from PROP-047 outcome kinds; audit_obligation carrier-only (Gap-N deferred); all field types VM-proven; next = FRONTIER-DECISION-P2 query-plan selection under cost/safety constraints (recommended over agent-review/retry/storage-adapter domains); grammar/StrategyDecision-canon-type/compiler/VM/PROP/ethical-reasoning/agent-autonomy all CLOSED; lab-docs/governance/frontier-decision-honesty-surface-gap-j-boundary-v0.md) | LAB-STORAGE-ADAPTER-P2 CLOSED (89/89 PASS - replay verifier + canonical digests; tamper rejected; fixture/capability/plan/version drift classified; receipt-only replay => insufficient_evidence; real storage HOLD) | LAB-FRONTIER-EXPEDITION-P1 CLOSED (frontier report, read-only — Accountability Loop half-built: Decide/Gap-J + Audit/Gap-N spec-only; agent accountability substrate ~70%; receipt=evidence design-law + deterministic-replay latent; white-space: constraints{}/StrategyDecision/SimulationReceipt/PostAuditReceipt zero lab pressure; 6 breakthrough candidates; FRONTIER-DECISION (Gap-J) recommended first; nothing authorized; lab-docs/governance/igniter-frontier-expedition-insights-and-breakthrough-concepts-v0.md) | LAB-STORAGE-ADAPTER-P1 CLOSED (80/80 PASS - mocked Storage adapter boundary; Query v0 semantics reused; missing mock source => system_error; real DB/SQL/ORM HOLD) | LAB-IGV-TAILMIX-P3 CLOSED (70/70 PASS — Sidebar+FileTreeRow bundle dedup, slot values, nested compose, per-instance isolation) | LAB-IGV-TAILMIX-P4 CLOSED (47/47 PASS — proof-local .igv→definition compiler; compiled hashes match hand-authored; 7 error categories fail-closed; compiled bundle compatible with P3 render/oracle/interpreter)
