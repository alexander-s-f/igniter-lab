...
*   **Remaining gap:** compaction's temp-WAL `rename` is still un-fsynced (audit B4) — but compaction is disabled by default (§D safety note), so it is off the ack path for any ledger/shadow daemon.
