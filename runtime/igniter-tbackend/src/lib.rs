...
fn init(ruby: &Ruby) -> Result<(), Error> {
    ...
    let playground_mod = igniter.define_module("TBackendPlayground")?;

    // ── Fact ──────────────────────────────────────────────────────────────────
    let fact_class = playground_mod.define_class("Fact", ruby.class_object())?;
    ...
    // ── FactLog ───────────────────────────────────────────────────────────────
    let log_class = playground_mod.define_class("FactLog", ruby.class_object())?;
    ...
    log_class.define_method(
        "latest_for_native",
        method!(FactLog::rb_latest_for_native, 3),
    )?;
    log_class.define_method("facts_for_native", method!(FactLog::rb_facts_for_native, 4))?;
    log_class.define_method(
        "query_scope_native",
        method!(FactLog::rb_query_scope_native, 3),
    )?;
    log_class.define_method("size", method!(FactLog::rb_size, 0))?;

    // ── FileBackend ───────────────────────────────────────────────────────────
    let fb_class = playground_mod.define_class("FileBackend", ruby.class_object())?;
    ...
    // ── Server ───────────────────────────────────────────────────────────
    let server_class = playground_mod.define_class("Server", ruby.class_object())?;
    server_class.define_singleton_method("start", function!(server::Server::rb_start, 4))?;
    server_class.define_method("stop", method!(server::Server::rb_stop, 0))?;
    server_class.define_method("metrics", method!(server::Server::rb_metrics, 0))?;
