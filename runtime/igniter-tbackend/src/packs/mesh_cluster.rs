...
impl BackgroundService for MeshSyncService {
    fn start(&self, kernel: Arc<ServerKernel>) -> Result<(), String> {
        ...
        println!(
            "[TBackend Mesh] Booting P2P Gossip Sync thread. Peers: {:?}",
            peers
        );
    ...
    for (store_name, peer_time_val) in peer_stores {
        ...
        let local_time = kernel
            ...
            .and_then(|engine| {
                engine
                    .log
                    .facts_for_store(store_name, None, None)
                    .last()
...
fn pull_facts_from_peer(
    ...
    let engine = kernel
        .get_or_create_engine(store_name)
        ...
        registry.register(
            ...
            Arc::new(|_req, kernel| {
                ...
                for (name, engine) in map.iter() {
                    let latest_time = engine
                        .log
                        .facts_for_store(name, None, None)
                        .last()
                        .map(|f| f.transaction_time)
                        .unwrap_or(0.0);
