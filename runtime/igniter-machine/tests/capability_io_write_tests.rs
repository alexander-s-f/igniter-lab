...
fn commit_lifecycle_writes_two_phase_receipt() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("k1", json!({"v": 1})), RunMode::Live)
            .await
            .unwrap();
        ...
        let facts = store.facts_for(RECEIPTS_STORE, "IO.WriteCapability:k1", None, None).await.unwrap();
        assert_eq!(facts.len(), 2, "two-phase: a prepared and a committed receipt");
        let latest = store.read_as_of(RECEIPTS_STORE, "IO.WriteCapability:k1", f64::MAX).await.unwrap().unwrap();
...
fn duplicate_same_payload_replays_no_second_write() {
    rt().block_on(async {
        ...
        let a = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("dup", json!({"v": 1})), RunMode::Live).await.unwrap();
        let b = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("dup", json!({"v": 1})), RunMode::Live).await.unwrap();
...
fn duplicate_different_payload_refused() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("k", json!({"v": 1})), RunMode::Live).await.unwrap();
        let conflict = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("k", json!({"v": 999})), RunMode::Live).await.unwrap();

        assert_eq!(conflict.state, WriteState::Denied);
        assert!(conflict.detail.unwrap().contains("different payload"));
        assert_eq!(exec.attempts(), 1, "the conflicting write must not reach the executor");
        assert_eq!(exec.applied_count(), 1);
        // the original committed receipt is intact
        let latest = store.read_as_of(RECEIPTS_STORE, "IO.WriteCapability:k", f64::MAX).await.unwrap().unwrap();
        assert_eq!(latest.value["state"], json!("committed"));
    });
}
...
fn executor_denial_is_denied_state_with_receipt() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("d1", json!({"v": 1})), RunMode::Live).await.unwrap();
        assert_eq!(out.state, WriteState::Denied);
        assert_eq!(exec.attempts(), 1);
        assert_eq!(exec.applied_count(), 0, "a denied write applies no mutation");

        let latest = store.read_as_of(RECEIPTS_STORE, "IO.WriteCapability:d1", f64::MAX).await.unwrap().unwrap();
...
fn timeout_is_unknown_and_not_blindly_retried() {
    rt().block_on(async {
        ...
        let first = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("t1", json!({"v": 1})), RunMode::Live).await.unwrap();
        assert_eq!(first.state, WriteState::UnknownExternalState);
        assert_eq!(exec.attempts(), 1);

        let latest = store.read_as_of(RECEIPTS_STORE, "IO.WriteCapability:t1", f64::MAX).await.unwrap().unwrap();
        assert_eq!(latest.value["state"], json!("unknown_external_state"));

        // a second identical call MUST NOT blindly retry the mutation
        let second = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("t1", json!({"v": 1})), RunMode::Live).await.unwrap();
        assert_eq!(second.state, WriteState::UnknownExternalState);
        assert!(second.detail.unwrap().contains("no blind retry"));
        assert_eq!(exec.attempts(), 1, "an unknown write is never blindly retried");
    });
}
...
impl TBackend for WriteFailBackend {
    ...
    async fn facts_for(&self, _: &str, _: &str, _: Option<f64>, _: Option<f64>) -> Result<Vec<Fact>, EngineError> {
        Ok(vec![])
    }
...
fn prepare_receipt_failure_blocks_executor() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("g1", json!({"v": 1})), RunMode::Live).await.unwrap();
        ...
        assert_eq!(exec.attempts(), 0, "the executor must not be called if the gate cannot be written");
    });
}
...
fn authority_refused_writes_no_receipt() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("a1", json!({"v": 1})), RunMode::Live).await.unwrap();
        assert_eq!(out.state, WriteState::Denied);
        assert_eq!(exec.attempts(), 0);
        assert!(store.read_as_of(RECEIPTS_STORE, "IO.WriteCapability:a1", f64::MAX).await.unwrap().is_none());
    });
}
...
fn replay_with_different_authority_refused() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &a, "write", &write_req("k", json!({"v": 1})), RunMode::Live).await.unwrap();
        let mism = run_write_effect(&reg, &store, &clock(), &b, "write", &write_req("k", json!({"v": 1})), RunMode::Live).await.unwrap();
...
fn replay_mode_without_receipt_is_unknown() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &p, "write", &write_req("r1", json!({"v": 1})), RunMode::Replay).await.unwrap();
        assert_eq!(out.state, WriteState::UnknownExternalState);
        assert_eq!(exec.attempts(), 0);
        assert!(store.read_as_of(RECEIPTS_STORE, "IO.WriteCapability:r1", f64::MAX).await.unwrap().is_none());
    });
}
