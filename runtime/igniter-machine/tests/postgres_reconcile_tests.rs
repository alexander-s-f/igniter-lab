...
fn registry(adapter: Arc<FakePostgresWriteAdapter>) -> CapabilityExecutorRegistry {
    let policy = PostgresWritePolicy::new().allow_target("leads").allow_ops(&["insert", "upsert"]);
...
fn unknown_with_pg_receipt_found_resolves_committed() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k1"), RunMode::Live).await.unwrap();
        ...
        let r = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "k1").await.unwrap();
        ...
        let f = store.read_as_of(RECEIPTS_STORE, &format!("{CAP}:k1"), f64::MAX).await.unwrap().unwrap();
...
fn unknown_with_no_pg_receipt_resolves_permanent_failure() {
    rt().block_on(async {
        ...
        let out = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k2"), RunMode::Live).await.unwrap();
        assert_eq!(out.state, WriteState::UnknownExternalState);
        assert!(!adapter.has_effect_receipt("k2"));

        let r = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "k2").await.unwrap();
        assert_eq!(r, PostgresReconcileResult::ResolvedPermanentFailure);
        assert_eq!(receipt_state(&store, "k2").await, WriteState::PermanentFailure);
...
fn resolver_unavailable_keeps_unknown() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k3"), RunMode::Live).await.unwrap();
        adapter.set_resolver_down(true);

        let r = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "k3").await.unwrap();
        assert_eq!(r, PostgresReconcileResult::StillUnknown);
        assert_eq!(receipt_state(&store, "k3").await, WriteState::UnknownExternalState, "stays unknown");
    });
}
...
fn recovered_committed_replays_without_re_executing() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k4"), RunMode::Live).await.unwrap();
        let r = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "k4").await.unwrap();
        assert_eq!(r, PostgresReconcileResult::ResolvedCommitted);
        ...
        let replay = run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("k4"), RunMode::Live).await.unwrap();
        assert_eq!(replay.state, WriteState::Committed);
        assert_eq!(adapter.attempts(), 1, "reconciled-committed receipt replays, no re-execution");
        assert_eq!(adapter.business_row_count(), 1);
    });
}
...
fn same_value_different_key_no_false_positive() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("landed"), RunMode::Live).await.unwrap();
        ...
        let r = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "notlanded").await.unwrap();
        assert_eq!(r, PostgresReconcileResult::ResolvedPermanentFailure);
        assert_eq!(receipt_state(&store, "notlanded").await, WriteState::PermanentFailure);

        // while the genuinely-landed key resolves committed.
        let r2 = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "landed").await.unwrap();
...
fn committed_receipt_is_not_applicable_and_missing_is_no_receipt() {
    rt().block_on(async {
        ...
        run_write_effect(&reg, &store, &clock(), &passport(), "write", &write_req("done"), RunMode::Live).await.unwrap();
        let r = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "done").await.unwrap();
        assert_eq!(r, PostgresReconcileResult::NotApplicable(WriteState::Committed));

        let r2 = reconcile_postgres_unknown_write(&store, adapter.as_ref(), &clock(), CAP, "ghost").await.unwrap();
