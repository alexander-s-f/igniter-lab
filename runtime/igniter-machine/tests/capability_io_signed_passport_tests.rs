...
use igniter_machine::capability::{
    ...
    CapabilityExecutorRegistry, CapabilityPassport, EchoCapabilityExecutor, EffectRequest,
    OutcomeKind, PassportVerifier, RunMode, RECEIPTS_STORE,
};
...
fn valid_signed_passport_authorizes() {
    rt().block_on(async {
        ...
        let out = run_effect_with_verified_passport(
            &reg,
            &store,
            &clock_at(10.0),
            &verifier(),
        ...
        assert!(store
            .read_as_of(RECEIPTS_STORE, "IO.SignedCapability:k1", f64::MAX)
            .await
            .unwrap()
            .is_some());
    });
...
fn untrusted_signature_refused() {
    rt().block_on(async {
        ...
        let out2 = run_effect_with_verified_passport(
            &reg,
            &store,
            &clock_at(10.0),
            &verifier(),
        ...
        assert_eq!(
            echo.call_count(),
            0,
            "no executor reached for an unauthentic passport"
        );
        assert!(store
            .read_as_of(RECEIPTS_STORE, "IO.SignedCapability:u1", f64::MAX)
            .await
            .unwrap()
            .is_none());
    });
...
fn tampered_passport_cannot_escalate_scope() {
    rt().block_on(async {
        ...
        assert_eq!(
            out.kind,
            OutcomeKind::Denied,
            "the signature binds the scope set — no escalation"
        );
...
fn signed_but_expired_revoked_or_wrong_scope_still_refused() {
    rt().block_on(async {
        ...
        let r = run_effect_with_verified_passport(
            &reg,
            &store,
            &clock_at(200.0),
            &verifier(),
...
fn verify_passport_signed_unit() {
    ...
    let foreign = signed(&OTHER_ISSUER, "svc", &["read"], Some(1_000_000.0));
    assert_eq!(
        verify_passport_signed(&v, &foreign, CAP, "read", &c).unwrap_err(),
        AuthRefusal::Untrusted
    );

    // authentic but wrong capability / scope
    assert_eq!(
        verify_passport_signed(&v, &good, "other-cap", "read", &c).unwrap_err(),
        AuthRefusal::WrongCapability
    );
    assert_eq!(
        verify_passport_signed(&v, &good, CAP, "write", &c).unwrap_err(),
