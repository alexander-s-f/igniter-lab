...
fn recipe(digest: &str, pool_sizing: u32) -> ServiceRecipe {
    ServiceRecipe {
        ...
        receipt_policy: "audit".into(), retry_policy_ref: None, pool_sizing,
...
async fn setup(h: &mut CoordinationHub, n: usize, last_labels: Vec<String>) -> String {
    ...
    for i in 0..n {
        let labels = if i == n - 1 { last_labels.clone() } else { vec![] };
        let cref = h.add_capsule(&passport("alice"), "svc", bytes.clone(), labels).await.unwrap();
        digest = cref.capsule_id;
    }
    h.accept_recipe(&passport("dev"), "svc", recipe(&digest, n as u32)).await.unwrap();
...
fn n_homogeneous_replicas_one_image() {
    rt().block_on(async {
        ...
        assert_eq!(h.content_count(), 1, "N refs share ONE stored byte image (no copy)");
    });
}
...
fn deterministic_selection() {
    assert_eq!(select_replica("hash_key", 3, "E1"), select_replica("hash_key", 3, "E1"));
...
fn invoke_replica_output_invariant() {
    rt().block_on(async {
        let (mut h, _a) = hub();
        setup(&mut h, 3, vec![]).await;
        let r0 = h.invoke_replica(&passport("vendor:acme"), "svc", json!({"a": 2, "b": 3}), 0).await.unwrap();
        let r1 = h.invoke_replica(&passport("vendor:acme"), "svc", json!({"a": 2, "b": 3}), 1).await.unwrap();
        let r2 = h.invoke_replica(&passport("vendor:acme"), "svc", json!({"a": 2, "b": 3}), 7).await.unwrap(); // 7 % 3 = 1
        assert_eq!(r0, json!(5));
        assert_eq!(r1, json!(5));
...
fn fanout_identical_output() {
    rt().block_on(async {
        let (mut h, _a) = hub();
        setup(&mut h, 3, vec![]).await;
        let results = h.invoke_fanout(&passport("vendor:acme"), "svc", json!({"a": 10, "b": 7})).await.unwrap();
        assert_eq!(results.len(), 3);
        assert!(results.iter().all(|(_, r)| r.as_ref().unwrap() == &json!(17)), "all replicas give the same output");
    });
}
...
fn audit_records_replica_and_fanout() {
    rt().block_on(async {
        ...
        h.invoke_replica(&passport("vendor:acme"), "svc", json!({"a": 1, "b": 1}), 0).await.unwrap();
        h.invoke_fanout(&passport("vendor:acme"), "svc", json!({"a": 1, "b": 1})).await.unwrap();
        let evs = audit_events(&audit).await;
        assert!(evs.iter().any(|e| e["operation"] == "invoke" && e["reason"].as_str().map(|s| s.starts_with("replica:")).unwrap_or(false)));
        assert!(evs.iter().any(|e| e["operation"] == "invoke_fanout" && e["reason"] == json!("fanout:2")));
    });
}
...
fn non_production_cannot_fanout() {
    rt().block_on(async {
        ...
        register(&mut h, "alice", AgentKind::Agent).await;
        register(&mut h, "vendor:acme", AgentKind::RuntimeActor).await;
        h.create_pool(&passport("alice"), "draft", "d", PoolVisibility::Private).await.unwrap();
        let bytes = add_bytes().await;
        h.add_capsule(&passport("alice"), "draft", bytes, vec![]).await.unwrap();
        // never signed into production → fanout refused
        assert!(h.invoke_fanout(&passport("vendor:acme"), "draft", json!({"a": 1, "b": 1})).await.is_err());
    });
}
...
fn failure_isolation_in_fanout() {
    rt().block_on(async {
        ...
        let results = h.invoke_fanout(&passport("vendor:acme"), "svc", json!({"a": 4, "b": 4})).await.unwrap();
        assert_eq!(results.len(), 3);
        assert_eq!(results[0].1.as_ref().unwrap(), &json!(8));
        assert_eq!(results[1].1.as_ref().unwrap(), &json!(8));
        assert!(results[2].1.is_err(), "the disabled replica is isolated + reported, others succeed");
    });
}
