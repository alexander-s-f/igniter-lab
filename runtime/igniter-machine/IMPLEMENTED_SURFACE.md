...
> P1→P8 complete (fake P1–P4, gate P5, real read P6, wire-atomic P7, real write P8). **Durable
> multi-process exactly-once is now PROVEN — durable-CAS P2 (`LAB-MACHINE-DURABLE-CAS-PG-EXACTLY-ONCE-P2`):
> two concurrent writers on separate connections + separate receipt stores (= two processes) on the
> same idempotency key resolve to ONE durable mutation/receipt via the DB-native
> `effect_receipts(idempotency_key)` UNIQUE CAS — the in-process `single_flight` lock is not relied
> on; canonical `EFFECT_RECEIPTS_DDL` is code-anchored; DDL/config drift fails loud as
> `PermanentConfig`; see `…-durable-cas-pg-exactly-once-p2-v0.md`.** P23 adds an
...
| WAL (`wal::WALWriter`) | ✅ **explicit durability + non-silent recovery** (`LAB-MACHINE-WAL-FSYNC-NONSILENT-RECOVERY-P2`, `../lab-docs/lang/lab-machine-wal-fsync-nonsilent-recovery-p2-v0.md`). Append-only `len|msgpack(Fact)|crc32`. Each `append` flushes then (default `WalDurability::Sync`) `File::sync_data` (fdatasync); `WalDurability::Flush` opts out. Recovery: `replay_reported() -> WalReplay` distinguishes a benign torn **tail** (`truncated_tail`, healthy prefix recovered) from mid-stream **corruption** (`corrupt: Vec<WalCorruption>` = `CrcMismatch` stops the scan / `Deserialize` continues, both with byte offsets). Boot-facing `replay()` tolerates a torn tail but **fails closed** with `EngineError::Corruption` on mid-stream corruption (used at `machine.rs` boot). Same non-claim as the `.mpk` store: fsync-to-OS, **no power-loss claim** (macOS needs `F_FULLFSYNC`); deterministic on-disk truncation/byte-flip stands in for a torn write. |
